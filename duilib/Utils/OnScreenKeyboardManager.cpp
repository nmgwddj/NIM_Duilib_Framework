...
void OnScreenKeyboardManager::ShowOSK(bool show) {
    ...
    if (bNeedOsk) {
        m_bShow = show;
        auto callback =
            nbase::Bind(&OnScreenKeyboardManager::TimerCallback, this);
        TimerManager::GetInstance()->AddCancelableTimer(this->GetWeakFlag(),
                                                        callback, 10, 1);
    }
}
...
void OnScreenKeyboardManager::StartTabTip() {
    ...
    if (bufferSize) {
        ...
        GetEnvironmentVariable(L"CommonProgramW6432", &tabTipPath[0],
                               bufferSize);
    ...
    if (tabTipPath.empty()) {
        tabTipPath =
            LR"(C:\\Program Files\Common Files\microsoft shared\ink\TabTip.exe)";
    }
