...
size_t StringReplaceAllT(const std::basic_string<CharType>& find,
    ...
    if (find_length < replace_length) {
        ...
        size_t newsize =
            output.size() + replaced * (replace_length - find_length);
    ...
    if (replace_length < find_length)
        output.resize(output.size() -
                      replaced * (find_length - replace_length));
...
inline int vsnprintfT(wchar_t* dst,
                      size_t count,
...
template <typename CharType>
void StringAppendVT(const CharType* format,
                    va_list ap,
                    std::basic_string<CharType>& output) {
...
size_t StringHelper::ReplaceAll(const std::wstring& find,
...
bool StringHelper::MBCSToUnicode(const char* input,
                                 std::wstring& output,
                                 int code_page) {
    output.clear();
...
bool StringHelper::MBCSToUnicode(const std::string& input,
                                 std::wstring& output,
                                 int code_page) {
    output.clear();
...
bool StringHelper::UnicodeToMBCS(const wchar_t* input,
                                 std::string& output,
                                 int code_page) {
    output.clear();
    int length =
...
bool StringHelper::UnicodeToMBCS(const std::wstring& input,
                                 std::string& output,
                                 int code_page) {
    output.clear();
...
std::list<std::string> StringHelper::Split(const std::string& input,
...
std::list<std::wstring> StringHelper::Split(const std::wstring& input,
