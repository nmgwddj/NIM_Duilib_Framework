    ...
    while (*pStrKey != _T('\0')) {
        ...
        if (!sValue.empty()) {
            if (sItem == _T("color") || sItem == _T("res")) {
                ...
                m_cpOffset.x = _tcstol(sValue.c_str(), &pstr, 10);
                ASSERT(pstr);
                m_cpOffset.y = _tcstol(pstr + 1, &pstr, 10);
                ASSERT(pstr);
            } else if (sItem == _T("blurradius")) {
                m_nBlurRadius = _tcstol(sValue.c_str(), &pstr, 10);
                ASSERT(pstr);
            } else if (sItem == _T("blursize")) {
                m_nBlurSize = _tcstol(sValue.c_str(), &pstr, 10);
                ASSERT(pstr);
            } else if (sItem == _T("spreadsize")) {
                m_nSpreadSize = _tcstol(sValue.c_str(), &pstr, 10);
                ASSERT(pstr);
