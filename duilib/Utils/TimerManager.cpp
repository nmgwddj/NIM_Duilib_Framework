...
TimerManager::TimerManager()
    ...
    m_hMessageWnd = ::CreateWindowW(L"UI_ANIMATION_TIMERMANAGER_H_", 0, 0, 0, 0,
                                    0, 0, HWND_MESSAGE, 0, hinst, 0);
}

LRESULT TimerManager::WndProcThunk(HWND hwnd,
                                   UINT message,
                                   WPARAM wparam,
                                   LPARAM lparam) {
...
void TimerManager::TimeCallback(UINT uTimerID,
                                UINT uMsg,
                                DWORD_PTR dwUser,
                                DWORD_PTR dw1,
                                DWORD_PTR dw2) {
...
bool TimerManager::AddCancelableTimer(
    const std::weak_ptr<nbase::WeakFlag>& weakFlag,
    ...
    if (m_nOldTimerId == 0 || m_bMinPause == false) {
        ...
        m_nOldTimerId =
            ::timeSetEvent(TIMER_INTERVAL, TIMER_PRECISION,
                           &TimerManager::TimeCallback, NULL, TIME_PERIODIC);
...
void TimerManager::Poll() {
    ...
    while (!m_aTimers.empty()) {
        LONGLONG detaTime =
            m_aTimers.top().dwTimeEnd.QuadPart - currentTime.QuadPart;
        if (detaTime <= 0) {
            ...
            if (!it.weakFlag.expired()) {
                ...
                if (rePush) {
                    ...
                    rePushTimerInfo.dwTimeEnd.QuadPart =
                        currentTime.QuadPart + it.uPause;
            ...
            if (!m_bMinPause) {
                timeKillEvent(m_nOldTimerId);
                m_nOldTimerId = ::timeSetEvent(TIMER_INTERVAL, TIMER_PRECISION,
                                               &TimerManager::TimeCallback,
                                               NULL, TIME_PERIODIC);
                ASSERT(m_nOldTimerId);
                m_bMinPause = true;
            }
            ...
            double newDetaTime =
                double(detaTime) * 1000 / m_timeFrequency.QuadPart;
            timeKillEvent(m_nOldTimerId);
            m_nOldTimerId = ::timeSetEvent(
                int(newDetaTime + 2 * TIMER_PRECISION), TIMER_PRECISION,
                &TimerManager::TimeCallback, NULL, TIME_PERIODIC);
            ASSERT(m_nOldTimerId);
            m_bMinPause = false;
            break;
