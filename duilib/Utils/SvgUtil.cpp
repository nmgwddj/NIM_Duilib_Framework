...
std::unique_ptr<ui::ImageInfo> SvgUtil::LoadSvg(
...
std::unique_ptr<ui::ImageInfo> SvgUtil::LoadImageBySvg(
    void* data,
    const std::wstring& strImageFullPath) {
    ...
    std::unique_ptr<NSVGrasterizer, RasterizerDeleter> rast(
    ...
    HDC hdc = GetDC(NULL);
    HBITMAP hBitmap =
        GdiBitmap::CreateDIBBitmap(hdc, w, h, true, (LPVOID*)&pBmpBits);
    ReleaseDC(NULL, hdc);
