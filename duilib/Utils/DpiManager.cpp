...
UINT DpiManager::GetMonitorDPI(HMONITOR hMonitor) {
    ...
    if (IsWindows8OrGreater()) {
        if (!GetDpiForMonitorWrapper(hMonitor, MDT_EFFECTIVE_DPI, &dpix,
                                     &dpiy)) {
            dpix = 96;
        }
    } else {
...
UINT DpiManager::GetDPIFromRegistry() {
    ...
    ret = ::RegOpenKeyEx(HKEY_CURRENT_USER,
                         L"CONTROL PANEL\\DESKTOP\\WINDOWMETRICS", 0,
                         KEY_ALL_ACCESS, &hKEY);
    if (ret != ERROR_SUCCESS) {
        ...
        ret = ::RegQueryValueEx(hKEY, L"AppliedDPI", NULL, &type, (LPBYTE)&data,
                                &data_size);
        if (ret != ERROR_SUCCESS) {
            ::RegCloseKey(hKEY);
        } else {
