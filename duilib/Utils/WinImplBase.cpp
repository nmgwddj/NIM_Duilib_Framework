...
LRESULT WindowImplBase::OnWindowPosChanging(UINT uMsg,
                                            WPARAM wParam,
                                            LPARAM lParam,
                                            BOOL& bHandled) {
    bHandled = FALSE;
    if (IsZoomed(m_hWnd)) {
        ...
        if (lpPos->flags &
            SWP_FRAMECHANGED)  // 第一次最大化，而不是最大化之后所触发的WINDOWPOSCHANGE
        {
            POINT pt = {0, 0};
            HMONITOR hMontorPrimary =
            ...
            if (rc.left < -10000 && rc.top < -10000 && rc.right < -10000 &&
                rc.right < -10000) {
                WINDOWPLACEMENT wp = {sizeof(WINDOWPLACEMENT)};
                GetWindowPlacement(m_hWnd, &wp);
                hMonitorTo = MonitorFromRect(&wp.rcNormalPosition,
...
LRESULT WindowImplBase::OnNcHitTest(UINT uMsg,
    ...
    if (pt.x >= rcClient.left + rcCaption.left &&
        ...
        pt.y >= rcClient.top + rcCaption.top &&
        pt.y < rcClient.top + rcCaption.bottom) {
        Control* pControl = FindControl(pt);
        if (pControl) {
            if (dynamic_cast<Button*>(pControl) ||
                dynamic_cast<ButtonBox*>(pControl) ||
                dynamic_cast<RichEdit*>(pControl) ||
                dynamic_cast<Combo*>(pControl))
                return HTCLIENT;
            else
                return HTCAPTION;
        }
...
LRESULT WindowImplBase::OnSize(UINT uMsg,
                               WPARAM wParam,
                               LPARAM lParam,
                               BOOL& bHandled) {
...
LRESULT WindowImplBase::OnChar(UINT uMsg,
                               WPARAM wParam,
                               LPARAM lParam,
                               BOOL& bHandled) {
    bHandled = FALSE;
...
LRESULT WindowImplBase::OnCreate(UINT uMsg,
    ...
    if (pRoot == NULL) {
        ...
        _stprintf_s(szErrMsg, L"Failed to load xml file %s",
                    strSkinFile.c_str());
    ...
    if (pControl) {
        ...
        ASSERT(pCloseBtn);
        pCloseBtn->AttachClick(nbase::Bind(&WindowImplBase::OnButtonClick, this,
                                           std::placeholders::_1));
    }
    ...
    if (pControl) {
        ...
        ASSERT(pMinBtn);
        pMinBtn->AttachClick(nbase::Bind(&WindowImplBase::OnButtonClick, this,
                                         std::placeholders::_1));
    }
    ...
    if (pControl) {
        ...
        ASSERT(pMaxBtn);
        pMaxBtn->AttachClick(nbase::Bind(&WindowImplBase::OnButtonClick, this,
                                         std::placeholders::_1));
    }
    ...
    if (pControl) {
        ...
        ASSERT(pRestoreBtn);
        pRestoreBtn->AttachClick(nbase::Bind(&WindowImplBase::OnButtonClick,
