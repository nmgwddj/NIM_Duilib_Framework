...
void AnimationPlayerBase::StartTimer() {
    ...
    TimerManager::GetInstance()->AddCancelableTimer(
        m_weakFlagOwner.GetWeakFlag(), playCallback, m_elapseMillSeconds,
...
void AnimationPlayerBase::Play() {
    ...
    m_palyedMillSeconds +=
        (double(newCurrentTime.QuadPart - m_currentTime.QuadPart) * 1000) /
...
int AnimationPlayer::GetCurrentValue() {
    ...
    if (m_palyedMillSeconds <= m_speedUpMillSeconds) {
        detaValue =
            int(m_speedUpfactorA * m_palyedMillSeconds * m_palyedMillSeconds);
    } else if (m_palyedMillSeconds <=
               (m_speedUpMillSeconds + m_linerMillSeconds)) {
        double linerTime = m_palyedMillSeconds - m_speedUpMillSeconds;
        detaValue =
            int(m_speedUpfactorA * m_speedUpMillSeconds * m_speedUpMillSeconds +
                m_linearSpeed * linerTime);
    } else if (m_palyedMillSeconds <= m_totalMillSeconds) {
        double speedDownTime =
            m_palyedMillSeconds - m_speedUpMillSeconds - m_linerMillSeconds;
        detaValue =
            int(m_speedUpfactorA * m_speedUpMillSeconds * m_speedUpMillSeconds +
                m_linearSpeed * m_linerMillSeconds +
                m_speedDownfactorA * speedDownTime * speedDownTime +
                m_speedDownfactorB * speedDownTime);
    } else {
        ASSERT(FALSE);
    }
...
void AnimationPlayer::InitFactor() {
    ...
    if (m_speedUpRatio == 0 && m_speedDownRatio == 0) {  // liner
        ASSERT(m_totalMillSeconds == DUI_NOSET_VALUE && m_linearSpeed != 0 ||
               m_totalMillSeconds != DUI_NOSET_VALUE && m_linearSpeed == 0);
        ...
        if (m_totalMillSeconds != DUI_NOSET_VALUE) {
            if (m_speedUpRatio != 0) {
                m_speedUpfactorA =
                    s / ((m_speedUpRatio * m_speedUpRatio +
                          (1 - m_speedUpRatio - m_speedDownRatio) * 2 *
                              m_speedUpRatio +
                          m_speedUpRatio * m_speedDownRatio) *
                         m_totalMillSeconds * m_totalMillSeconds);
            } else if (m_speedDownRatio != 0) {
                m_speedDownfactorA =
                    -s / ((m_speedDownRatio * m_speedDownRatio +
                           (1 - m_speedUpRatio - m_speedDownRatio) * 2 *
                               m_speedDownRatio +
                           m_speedUpRatio * m_speedDownRatio) *
                          m_totalMillSeconds * m_totalMillSeconds);
        ...
        if (m_speedUpfactorA != 0 && m_speedUpRatio != 0) {
            ...
            if (m_speedDownRatio != 0) {
                m_speedDownfactorA =
                    -m_speedUpfactorA * m_speedUpRatio / m_speedDownRatio;
            }
        } else if (m_speedDownfactorA != 0 && m_speedDownRatio != 0) {
            tmpValue = -m_speedDownfactorA * m_speedDownRatio;
            if (m_speedUpRatio != 0) {
                m_speedUpfactorA =
                    -m_speedDownfactorA * m_speedDownRatio / m_speedUpRatio;
            }
        } else {
            ASSERT(FALSE);
        }
        if (m_totalMillSeconds == DUI_NOSET_VALUE) {
            m_totalMillSeconds = int(sqrt(
                s /
                (m_speedUpfactorA * m_speedUpRatio * m_speedUpRatio +
                 (1 - m_speedUpRatio - m_speedDownRatio) * 2 * tmpValue -
        ...
        m_linerMillSeconds =
            m_totalMillSeconds - m_speedUpMillSeconds - m_speedDownMillSeconds;
    }
}
