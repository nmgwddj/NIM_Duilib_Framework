...
AnimationPlayer* AnimationManager::GetAnimationPlayer(
...
AnimationPlayer* AnimationManager::SetFadeHot(bool bFadeHot) {
    ...
    if (bFadeHot) {
        ...
        std::function<void(int)> playCallback = nbase::Bind(
            &Control::SetHotAlpha, m_pControl, std::placeholders::_1);
...
AnimationPlayer* AnimationManager::SetFadeAlpha(bool bFadeVisible) {
    ...
    if (bFadeVisible) {
        ...
        std::function<void(int)> playCallback =
            nbase::Bind(&Control::SetAlpha, m_pControl, std::placeholders::_1);
        animationArgs->SetCallback(playCallback);
...
AnimationPlayer* AnimationManager::SetFadeWidth(bool bFadeWidth) {
    ...
    if (bFadeWidth) {
        ...
        animationArgs->SetSpeedDownRatio(0.7);
        std::function<void(int)> playCallback =
            nbase::Bind(&Control::SetFixedWidth, m_pControl,
                        std::placeholders::_1, true, false);
        animationArgs->SetCallback(playCallback);
...
AnimationPlayer* AnimationManager::SetFadeHeight(bool bFadeHeight) {
    ...
    if (bFadeHeight) {
        ...
        animationArgs->SetSpeedDownRatio(0.7);
        std::function<void(int)> playCallback = nbase::Bind(
            &Control::SetFixedHeight, m_pControl, std::placeholders::_1, false);
        animationArgs->SetCallback(playCallback);
...
AnimationPlayer* AnimationManager::SetFadeInOutX(bool bFade,
                                                 bool bIsFromRight) {
    AnimationPlayer* animationArgs = nullptr;
    if (bFade) {
        animationArgs = new AnimationPlayer();
        ...
        animationArgs->SetSpeedDownRatio(0.7);
        std::function<void(int)> playCallback = nbase::Bind(
            &Control::SetRenderOffsetX, m_pControl, std::placeholders::_1);
        animationArgs->SetCallback(playCallback);
...
AnimationPlayer* AnimationManager::SetFadeInOutY(bool bFade,
                                                 bool bIsFromBottom) {
    AnimationPlayer* animationArgs = nullptr;
    if (bFade) {
        animationArgs = new AnimationPlayer();
        ...
        animationArgs->SetSpeedDownRatio(0.7);
        std::function<void(int)> playCallback = nbase::Bind(
            &Control::SetRenderOffsetY, m_pControl, std::placeholders::_1);
        animationArgs->SetCallback(playCallback);
...
void AnimationManager::Disappear() {
    ...
    CompleteCallback completeCallback =
        nbase::Bind(&Control::SetVisible_, m_pControl, false);
    if (GetAnimationPlayer(kAnimationAlpha)) {
