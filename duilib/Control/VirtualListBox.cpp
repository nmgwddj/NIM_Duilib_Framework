...
CSize VirtualVLayout::ArrangeChild(const std::vector<Control*>& items,
                                   UiRect rc) {
...
CSize VirtualHLayout::ArrangeChild(const std::vector<Control*>& items,
                                   UiRect rc) {
...
void VirtualListBox::SetDirection(ListDirection direction) {
    ...
    ReSetLayout(kListVertical == direction
                    ? dynamic_cast<Layout*>(new VirtualVLayout)
                    : dynamic_cast<Layout*>(new VirtualHLayout));
}
...
void VirtualListBox::GetDisplayCollection(std::vector<int>& collection) {
    ...
    int length = (kListVertical == m_eDirection) ? rcThis.GetWidth()
                                                 : rcThis.GetHeight();
    int scroll_pos =
        (kListVertical == m_eDirection) ? GetScrollPos().cy : GetScrollPos().cx;
...
void VirtualListBox::EnsureVisible(int iIndex, bool bToTop) {
    ...
    int nPos =
        (m_eDirection == kListVertical) ? GetScrollPos().cy : GetScrollPos().cx;
    ...
    if (bToTop) {
        ...
        if (iIndex > nTopIndex) {
            ...
            nNewPos = length - ((m_eDirection == kListVertical)
                                    ? m_rcItem.GetHeight()
                                    : m_rcItem.GetWidth());
        } else {
            // 向上
...
void VirtualListBox::ReArrangeChild(bool bForce) {
    ...
    if (direction == kScrollDown) {
        ...
        // 向上滚动
        int nDisplayCount =
            ((m_eDirection == kListVertical) ? m_rcItem.GetHeight()
                                             : m_rcItem.GetWidth()) /
                m_nElementHeight +
            1;
...
void VirtualListBox::HandleMessage(ui::EventArgs& event) {
    if (!IsMouseEnabled() && event.Type > ui::kEventMouseBegin &&
        event.Type < ui::kEventMouseEnd) {
        if (m_pParent != NULL)
...
void VirtualListBox::SetAttribute(const std::wstring& strName,
...
bool VirtualListBox::IsElementDisplay(int iIndex) {
    ...
    if (nElementPos >= nPos) {
        int nLength = (m_eDirection == kListVertical) ? this->GetHeight()
                                                      : this->GetWidth();
...
bool VirtualListBox::NeedReArrange(ScrollDirection& direction) {
    ...
    // 针对情况二，解决方法是，列表宽度变化 或者
    // 拖动写信窗口右侧，列表position改变， 此时，在SetPos中强制重新布局
    ...
    if (nPos >= m_nOldYScrollPos) {
        ...
        if (kListVertical == m_eDirection) {
            int nSub =
                (rcItem.bottom - rcThis.top) - (nPos + rcThis.GetHeight());
            if (nSub < 0) {
            ...
            int nSub =
                (rcItem.right - rcThis.left) - (nPos + rcThis.GetWidth());
            if (nSub < 0) {
                direction = kScrollDown;
                return true;
            }
