...
ui::CSize VirtualTileLayout::ArrangeChild(
    const std::vector<ui::Control*>& items,
    ui::UiRect rc) {
...
ui::CSize VirtualTileLayout::AjustSizeByChild(
    const std::vector<ui::Control*>& items,
...
bool VirtualTileLayout::SetAttribute(const std::wstring& strName,
...
void VirtualTileLayout::LazyArrangeChild() {
    ...
    for (auto pControl : pList->m_items) {
        // Determine size
        ui::UiRect rcTile(ptTile.x, ptTile.y, ptTile.x + m_szItem.cx,
                          ptTile.y + m_szItem.cy);
...
int VirtualTileLayout::AjustMaxItem() {
    ...
    if (m_bAutoCalcColumn) {
        if (m_szItem.cx > 0)
            m_nColumns =
                (rc.right - rc.left) / (m_szItem.cx + m_iChildMargin / 2);
        if (m_nColumns == 0)
            m_nColumns = 1;
    }
...
void VirtualTileBox::SetDataProvider(VirtualTileInterface* pProvider) {
    ...
    pProvider->RegNotifys(
        nbase::Bind(&VirtualTileBox::OnModelDataChanged, this,
