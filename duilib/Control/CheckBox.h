...
public:
    ...
    virtual void SetAttribute(const std::wstring& strName,
                              const std::wstring& strValue) override;
     ...
     * @param[in] bTriggerEvent 是否发送状态改变事件，true 为发送，否则为
     * false。默认为 false
     * @return 无
     */
     ...
     * @return 无
     */
    void SetSelectedStateImage(ControlStateType stateType,
     ...
     * @return 无
     */
    void SetSelectedStateTextColor(ControlStateType stateType,
                                   const std::wstring& dwTextColor);
     ...
     * @return 返回颜色字符串，该值在 global.xml 中定义
     */
    std::wstring GetPaintSelectedStateTextColor(
        ControlStateType buttonStateType,
     ...
     * @return 无
     */
    void SetSelectedStateColor(ControlStateType stateType,
                               const std::wstring& stateColor);
     ...
     * @return 无
     */
    void SetSelectedForeStateImage(ControlStateType stateType,
                                   const std::wstring& pStrImage);
    ...
    void AttachSelect(const EventCallback& callback) {
        this->OnEvent[kEventSelect] += callback;
    }
    ...
    void AttachUnSelect(const EventCallback& callback) {
        this->OnEvent[kEventUnSelect] += callback;
    }
...
inline UIAControlProvider* CheckBoxTemplate<InheritType>::GetUIAProvider() {
    ...
    if (this->m_pUIAProvider == nullptr) {
        this->m_pUIAProvider = static_cast<UIAControlProvider*>(
            new (std::nothrow) UIACheckBoxProvider(this));
    }
...
template <typename InheritType>
void CheckBoxTemplate<InheritType>::Selected(bool bSelected,
                                             bool bTriggerEvent) {
    ...
    if (this->m_pUIAProvider != nullptr && UiaClientsAreListening()) {
        ...
        UiaRaiseAutomationPropertyChangedEvent(this->m_pUIAProvider,
...
bool CheckBoxTemplate<InheritType>::HasHotState() {
    return m_selectedColorMap.HasHotColor() ||
           m_selectedTextColorMap.HasHotColor() || __super::HasHotState();
}
...
Image* CheckBoxTemplate<InheritType>::GetEstimateImage() {
    ...
    if (!estimateImage) {
        estimateImage =
            this->m_imageMap.GetEstimateImage(kStateImageSelectedBk);
    }
...
void CheckBoxTemplate<InheritType>::PaintStatusColor(IRenderContext* pRender) {
    ...
    if (IsPaintNormalFirst() && !m_selectedColorMap.HasColor())
        this->m_colorMap.PaintStatusColor(pRender, this->m_rcPaint,
                                          this->m_uButtonState);
    else
        m_selectedColorMap.PaintStatusColor(pRender, this->m_rcPaint,
                                            this->m_uButtonState);
}
...
void CheckBoxTemplate<InheritType>::PaintStatusImage(IRenderContext* pRender) {
    ...
    if (IsPaintNormalFirst() &&
        !this->m_imageMap.HasImageType(kStateImageSelectedBk))
        this->m_imageMap.PaintStatusImage(pRender, kStateImageBk,
                                          this->m_uButtonState);
    else
        this->m_imageMap.PaintStatusImage(pRender, kStateImageSelectedBk,
                                          this->m_uButtonState);

    if (IsPaintNormalFirst() &&
        !this->m_imageMap.HasImageType(kStateImageSelectedFore))
        this->m_imageMap.PaintStatusImage(pRender, kStateImageFore,
                                          this->m_uButtonState);
    else
        this->m_imageMap.PaintStatusImage(pRender, kStateImageSelectedFore,
                                          this->m_uButtonState);
}
...
void CheckBoxTemplate<InheritType>::PaintText(IRenderContext* pRender) {
    ...
    std::wstring clrColor =
        GetPaintSelectedStateTextColor(this->m_uButtonState, stateType);
    if (clrColor.empty())
        clrColor =
            m_dwSelectedTextColor.empty()
                ? this->GetPaintStateTextColor(this->m_uButtonState, stateType)
                : m_dwSelectedTextColor;
    ...
    if (this->m_animationManager.GetAnimationPlayer(kAnimationHot)) {
        if ((stateType == kControlStateNormal ||
             stateType == kControlStateHot) &&
            !GetSelectedStateTextColor(kControlStateHot).empty()) {
            std::wstring clrColor =
                GetSelectedStateTextColor(kControlStateNormal);
            if (!clrColor.empty()) {
                DWORD dwClrColor = this->GetWindowColor(clrColor);
                pRender->DrawText(rc, this->GetText(), dwClrColor,
                                  this->m_sFontId, this->m_uTextStyle, 255,
                                  this->m_bLineLimit);
            }

            if (this->m_nHotAlpha > 0) {
                std::wstring clrColor =
                    GetSelectedStateTextColor(kControlStateHot);
                if (!clrColor.empty()) {
                    DWORD dwClrColor = this->GetWindowColor(clrColor);
                    pRender->DrawText(rc, this->GetText(), dwClrColor,
                                      this->m_sFontId, this->m_uTextStyle,
    ...
    pRender->DrawText(rc, this->GetText(), dwClrColor, this->m_sFontId,
                      this->m_uTextStyle, 255, this->m_bLineLimit);
}

template <typename InheritType>
std::wstring CheckBoxTemplate<InheritType>::GetSelectedStateImage(
    ControlStateType stateType) {
...
void CheckBoxTemplate<InheritType>::SetSelectedStateImage(
    ...
    if (this->GetFixedWidth() == DUI_LENGTH_AUTO ||
        this->GetFixedHeight() == DUI_LENGTH_AUTO) {
...
template <typename InheritType>
void CheckBoxTemplate<InheritType>::SetSelectedTextColor(
    const std::wstring& dwTextColor) {
...
template <typename InheritType /*= Control*/>
std::wstring ui::CheckBoxTemplate<InheritType>::GetSelectedStateTextColor(
    ControlStateType stateType) {
...
template <typename InheritType /*= Control*/>
std::wstring ui::CheckBoxTemplate<InheritType>::GetPaintSelectedStateTextColor(
    ControlStateType buttonStateType,
    ControlStateType& stateType) {
    stateType = buttonStateType;
    if (stateType == kControlStatePushed &&
        GetSelectedStateTextColor(kControlStatePushed).empty()) {
        stateType = kControlStateHot;
    }
...
template <typename InheritType>
std::wstring CheckBoxTemplate<InheritType>::GetSelectStateColor(
    ControlStateType stateType) {
...
template <typename InheritType>
std::wstring CheckBoxTemplate<InheritType>::GetSelectedForeStateImage(
    ControlStateType stateType) {
