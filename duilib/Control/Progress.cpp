...
UIAControlProvider* Progress::GetUIAProvider() {
    ...
    if (m_pUIAProvider == nullptr) {
        m_pUIAProvider = static_cast<UIAControlProvider*>(
            new (std::nothrow) UIAProgressProvider(this));
    }
...
void Progress::SetValue(double nValue) {
    ...
    if (m_pUIAProvider != nullptr && UiaClientsAreListening()) {
        ...
        UiaRaiseAutomationPropertyChangedEvent(
            m_pUIAProvider, UIA_RangeValueValuePropertyId, vtOld, vtNew);
    }
#endif
...
void Progress::SetAttribute(const std::wstring& srName,
                            const std::wstring& strValue) {
    if (srName == _T("hor"))
        SetHorizontal(strValue == _T("true"));
    else if (srName == _T("min"))
        SetMinValue(_ttoi(strValue.c_str()));
    else if (srName == _T("max"))
        SetMaxValue(_ttoi(strValue.c_str()));
    else if (srName == _T("value"))
        SetValue(_ttoi(strValue.c_str()));
    else if (srName == _T("progressimage"))
        SetProgressImage(strValue);
    else if (srName == _T("isstretchfore"))
    ...
    else if (srName == _T("marqueewidth"))
        SetMarqueeWidth(_ttoi(strValue.c_str()));
    else if (srName == _T("marqueestep"))
        SetMarqueeStep(_ttoi(strValue.c_str()));
...
void Progress::PaintStatusImage(IRenderContext* pRender) {
    ...
    if (!m_progressImage.imageAttribute.simageString.empty()) {
        ...
        if (m_bStretchForeImage)
            m_sProgressImageModify =
                StringHelper::Printf(_T("destscale='false' dest='%d,%d,%d,%d'"),
            ...
            m_sProgressImageModify = StringHelper::Printf(
                _T("destscale='false' dest='%d,%d,%d,%d' source='%d,%d,%d,%d'"),
                rc.left, rc.top, rc.right, rc.bottom, m_rcSrc.left, m_rcSrc.top,
                m_rcSrc.right, m_rcSrc.bottom);
        }
        ...
        if (IsHorizontal()) {
            if (corner.left != 0 && corner.left >= rc.right) {
                m_sProgressImageModify +=
                    StringHelper::Printf(_T(" corner='%d,%d,%d,%d'"), rc.right,
                                         corner.top, 0, corner.bottom);
            }
        } else {
            if (corner.top != 0 && corner.top >= rc.bottom) {
                m_sProgressImageModify += StringHelper::Printf(
                    _T(" corner='%d,%d,%d,%d'"), corner.left, corner.bottom,
                    corner.right, 0);
            }
        }
...
UiRect Progress::GetProgressPos() {
    ...
    if (m_bHorizontal) {
        if (m_bReverse) {
            ...
            rc.left =
                ...
                static_cast<int>(std::floor(
                    static_cast<double>((m_nValue - m_nMin) *
                                        (m_rcItem.right - m_rcItem.left)) /
                    static_cast<double>(m_nMax - m_nMin)));
        } else {
        ...
        if (m_bReverse) {
            rc.bottom = static_cast<int>(std::floor(
                static_cast<double>((m_nMax - m_nValue) *
...
void Progress::Play() {
    ...
    if (m_bHorizontal) {
        if (m_nMarqueePos > rc.right - rc.left)
            m_nMarqueePos =
                (m_nMarqueePos - (rc.right - rc.left)) - m_nMarqueeWidth;
    } else {
        if (m_nMarqueePos > rc.bottom - rc.top)
            m_nMarqueePos =
                (m_nMarqueePos - (rc.bottom - rc.top) - m_nMarqueeWidth);
    }
    ...
    if (!m_sProgressColor.empty()) {
        ...
        if (dwProgressColor != 0) {
            ...
            if (m_bHorizontal) {
                ...
                rc.right = rc.left + (m_nMarqueePos >= 0
                                          ? m_nMarqueeWidth
                                          : (m_nMarqueeWidth + m_nMarqueePos));
            } else {
                rc.top = max(m_nMarqueePos, 0) + rc.top;
                rc.bottom = rc.top + (m_nMarqueePos >= 0
                                          ? m_nMarqueeWidth
                                          : (m_nMarqueeWidth + m_nMarqueePos));
            }
...
void Progress::SetMarquee(bool bMarquee) {
    ...
    if (m_bMarquee) {
        ...
        TimerManager::GetInstance()->AddCancelableTimer(
            m_timer.GetWeakFlag(), playCallback, m_nMarqueeElapsed,
