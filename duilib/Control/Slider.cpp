...
UiRect Slider::GetProgressPos() {
    UiRect rc;
    if (m_bHorizontal) {
        rc.right = int((m_nValue - m_nMin) *
                           (m_rcItem.right - m_rcItem.left - m_szThumb.cx) /
                           (m_nMax - m_nMin) +
                       m_szThumb.cx / 2 + 0.5);
        rc.bottom = m_rcItem.bottom - m_rcItem.top;
    } else {
        rc.top = int((m_nMax - m_nValue) *
            ...
            if (m_bHorizontal) {
                ...
                if (event.ptMouse.y >= m_rcItem.bottom - m_szThumb.cy / 2)
                    ...
                    m_nValue =
                        m_nMin +
                        double((m_nMax - m_nMin) *
                               (m_rcItem.bottom - event.ptMouse.y -
                                m_szThumb.cy / 2)) /
                            (m_rcItem.bottom - m_rcItem.top - m_szThumb.cy);
            }
...
void Slider::SetAttribute(const std::wstring& strName,
                          const std::wstring& strValue) {
    if (strName == _T("step"))
        SetChangeStep(_ttoi(strValue.c_str()));
        ...
        CSize szXY;
        LPTSTR pstr = NULL;
        szXY.cx = _tcstol(strValue.c_str(), &pstr, 10);
        ASSERT(pstr);
...
void Slider::PaintStatusImage(IRenderContext* pRender) {
    ...
    if (IsMouseFocused()) {
        ...
        m_sImageModify = StringHelper::Printf(
            _T("destscale='false' dest='%d,%d,%d,%d'"), rcThumb.left,
            rcThumb.top, rcThumb.right, rcThumb.bottom);
        if (!DrawImage(pRender, m_thumbStateImage[kControlStatePushed],
                       m_sImageModify)) {
        } else
            return;
        ...
        m_sImageModify = StringHelper::Printf(
            _T("destscale='false' dest='%d,%d,%d,%d'"), rcThumb.left,
            rcThumb.top, rcThumb.right, rcThumb.bottom);
        if (!DrawImage(pRender, m_thumbStateImage[kControlStateHot],
                       m_sImageModify)) {
        } else
            return;
    }
    ...
    m_sImageModify.clear();
    m_sImageModify = StringHelper::Printf(
        _T("destscale='false' dest='%d,%d,%d,%d'"), rcThumb.left, rcThumb.top,
        rcThumb.right, rcThumb.bottom);
    if (!DrawImage(pRender, m_thumbStateImage[kControlStateNormal],
                   m_sImageModify)) {
    } else
        return;
}
...
UiRect Slider::GetThumbRect() const {
    if (m_bHorizontal) {
        ...
        int top = int(m_rcItem.bottom - m_szThumb.cy -
                      (m_rcItem.bottom - m_rcItem.top - m_szThumb.cy) *
                          (m_nValue - m_nMin) / (m_nMax - m_nMin));
        return UiRect(left, top, left + m_szThumb.cx, top + m_szThumb.cy);
    }
...
void Slider::SetThumbStateImage(ControlStateType stateType,
