...
UIAControlProvider* ListBox::GetUIAProvider() {
    ...
    if (m_pUIAProvider == nullptr) {
        m_pUIAProvider = static_cast<UIAControlProvider*>(
            new (std::nothrow) UIAListBoxProvider(this));
    }
...
void ListBox::SetAttribute(const std::wstring& strName,
...
bool ListBox::SelectItem(int iIndex, bool bTakeFocus, bool bTrigger) {
    ...
    if (m_iCurSel >= 0) {
        ...
        if (pControl != NULL) {
            ListContainerElement* pListItem =
...
void ListBox::EnsureVisible(const UiRect& rcItem) {
    ...
    if (rcNewItem.left >= rcList.left && rcNewItem.top >= rcList.top &&
        ...
        if (m_pParent &&
            dynamic_cast<ListContainerElement*>(m_pParent) != NULL) {
            dynamic_cast<ListContainerElement*>(m_pParent)
                ->GetOwner()
                ->EnsureVisible(rcNewItem);
        }
        return;
    }
    ...
    for (auto it = m_items.begin(); it != m_items.end(); it++) {
        if ((*it)->GetName() == strItemName) {
            if (GetScrollRange().cy != 0) {
                ...
                scrollPos.cy =
                    (*it)->GetPos().top - m_pLayout->GetInternalPos().top;
                if (scrollPos.cy >= 0) {
...
Control* ListBox::GetTopItem() {
    int listTop =
        GetPos().top + m_pLayout->GetPadding().top + GetScrollPos().cy;
    for (auto it = m_items.begin(); it != m_items.end(); it++) {
        if ((*it)->IsVisible() && !(*it)->IsFloat() &&
            (*it)->GetPos().bottom >= listTop) {
            return (*it);
        }
    }
...
bool ListBox::SetItemIndex(Control* pControl, std::size_t iIndex) {
    ...
    if (m_iCurSel >= 0)
        pSelectedListItem =
            dynamic_cast<ListContainerElement*>(GetItemAt(m_iCurSel));
...
bool ListBox::RemoveAt(std::size_t iIndex) {
    ...
    for (int i = iIndex; i < GetCount(); ++i) {
        Control* p = GetItemAt(i);
        ListContainerElement* pListItem =
            dynamic_cast<ListContainerElement*>(p);
        if (pListItem != NULL)
            pListItem->SetIndex(i);
    }
...
bool ListBox::SortItems(PULVCompareFunc pfnCompare, UINT_PTR dwData) {
    ...
    qsort_s(&(*m_items.begin()), m_items.size(), sizeof(Control*),
            ListBox::ItemComareFunc, this);
    ListContainerElement* pItem = NULL;
    for (int i = 0; i < (int)m_items.size(); ++i) {
        pItem = dynamic_cast<ListContainerElement*>(
            static_cast<Control*>(m_items[i]));
        if (pItem) {
            pItem->SetIndex(i);
...
int __cdecl ListBox::ItemComareFunc(void* pvlocale,
                                    const void* item1,
                                    const void* item2) {
...
int __cdecl ListBox::ItemComareFunc(const void* item1, const void* item2) {
    ...
    return m_pCompareFunc((UINT_PTR)pControl1, (UINT_PTR)pControl2,
                          m_pCompareData);
}
...
ListContainerElement::ListContainerElement()
    ...
    , m_pOwner(nullptr) {
    m_uTextStyle =
        DT_LEFT | DT_VCENTER | DT_END_ELLIPSIS | DT_NOCLIP | DT_SINGLELINE;
...
UIAControlProvider* ListContainerElement::GetUIAProvider() {
    ...
    if (m_pUIAProvider == nullptr) {
        m_pUIAProvider = static_cast<UIAControlProvider*>(
            new (std::nothrow) UIAListBoxItemProvider(this));
    }
    return m_pUIAProvider;
#else
    return nullptr;
...
void ListContainerElement::HandleMessage(EventArgs& event) {
    if (!IsMouseEnabled() && event.Type > kEventMouseBegin &&
        ...
        if (event.chKey == VK_RETURN) {
            if (IsActivatable()) {
                if (m_pWindow != NULL)
                    m_pWindow->SendNotify(this, kEventReturn);
            }
            return;
        }
    ...
    // if( m_pOwner != NULL ) m_pOwner->HandleMessage(event); else
