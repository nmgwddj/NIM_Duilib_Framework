...
void MenuManager::RegisterMenu(const std::wstring strMenuName,
                               MenuWndEx* menu) {
...
MenuBox::MenuBox() {
    ...
    this->SetMaxHeight(MulDiv(GetSystemMetrics(SM_CYFULLSCREEN), 100,
                              ui::DpiManager::GetInstance()->GetScale()));
...
void MenuBox::SetAttribute(const std::wstring& strName,
...
void GetMenuPopupInfo(Control* control, CPoint& point, int& align) {
    ...
    point = StringToMenuPopup(control->GetPosWithScrollOffset(),
                              control->GetMenuPopup());
...
MenuWndEx* MenuWndEx::CreateMenu(STRINGorID xml,
                                 CPoint point,
...
void MenuWndEx::InitMenu(MenuElement* pOwner,
    ...
    if (!m_pOwner) {
        ...
        m_skinFloder =
            (dynamic_cast<MenuWndEx*>(m_pOwner->GetWindow()))->m_skinFloder;
    }

    MenuManager::GetInstance()->GetSubject().AddObserver(this);

    std::wstring title =
        MutiLanSupport::GetInstance()->GetStringViaID(L"STRID_MENU_TITLE");
    Create((m_pOwner == NULL) ? hParent : m_pOwner->GetWindow()->GetHWND(),
           title.c_str(), WS_POPUP, WS_EX_TOOLWINDOW | WS_EX_TOPMOST, true);
...
void MenuWndEx::ResizeMenu() {
    ...
    ::GetMonitorInfo(::MonitorFromPoint(m_BasedPoint, MONITOR_DEFAULTTOPRIMARY),
                     &oMonitor);
    ...
    SetPos(rc, false,
           SWP_SHOWWINDOW | (CheckFlag(kNoFocus) ? SWP_NOACTIVATE : 0),
           HWND_TOPMOST, true);
...
void MenuWndEx::ResizeSubMenu() {
    MONITORINFO oMonitor = {};
    oMonitor.cbSize = sizeof(oMonitor);
    ::GetMonitorInfo(::MonitorFromWindow(m_pOwner->GetWindow()->GetHWND(),
...
void MenuWndEx::AttachSelect(const std::wstring& strName,
                             const ui::EventCallback& callback) {
...
void MenuWndEx::AttachPopupSubMenu(const std::wstring& strName,
    ...
    if (m_pOwner != NULL) {
        for (int i = 0; i < m_pOwner->GetCount(); i++) {
            ...
            if (pMenuItem) {
                ...
                pMenuItem->SetOwner(
                    dynamic_cast<MenuBox*>(m_pOwner->GetParent()));
...
LRESULT MenuWndEx::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam) {
    if (m_pOwner != NULL) {
        ...
        if (!m_pOwner->GetSubMenuShadowImage().empty()) {
            ...
            m_shadow.SetShadowCorner(m_pOwner->GetWindow()->GetShadowCorner(),
                                     false);
            AttachDialog(m_shadow.AttachShadow(pRoot));
        } else {
            AttachDialog(pRoot);
        ...
        auto callback = nbase::Bind(&WindowImplBase::CreateControl, this,
                                    std::placeholders::_1);
        auto pRoot = (Box*)builder.Create(
            (GetWindowResourcePath() + GetSkinFile()).c_str(), callback, this);

        ASSERT(pRoot);
        if (pRoot == NULL) {
            // MessageBox(NULL, _T("加载资源文件失败"), _T("Duilib"), MB_OK |
            // MB_ICONERROR);
            return -1;
        }
    ...
    if (pRoot->GetFixedWidth() == DUI_LENGTH_AUTO ||
        pRoot->GetFixedHeight() == DUI_LENGTH_AUTO) {
...
LRESULT MenuWndEx::OnKillFocus(UINT uMsg,
                               WPARAM wParam,
                               LPARAM lParam,
                               BOOL& bHandled) {
...
LRESULT MenuWndEx::OnClose(UINT uMsg,
                           WPARAM wParam,
                           LPARAM lParam,
                           BOOL& bHandled) {
    if (m_pOwner != NULL) {
        m_pOwner->SetWindow(m_pOwner->GetWindow(), m_pOwner->GetParent(),
                            false);
...
void MenuWndEx::SetWidth(const int width) {
    ...
    int shadow_width =
        this->GetShadowCorner().left + this->GetShadowCorner().right;
...
ui::Control* MenuElement::FindControl(FINDCONTROLPROC Proc,
                                      LPVOID pData,
                                      UINT uFlags,
...
void MenuElement::SetAttribute(const std::wstring& strName,
                               const std::wstring& strValue) {
...
void MenuElement::CreateMenuWnd() {
    ...
    m_pSubMenuWindow->InitMenu(static_cast<MenuElement*>(this), NULL, _T(""),
                               CPoint(), kRight | kBottom);
