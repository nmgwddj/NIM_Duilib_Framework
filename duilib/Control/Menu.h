...
struct ContextMenuParam {
    enum RemoveType { kRemoveAllMenu, kRemoveSubMenu };
    RemoveType type;
    HWND hWnd;
};
...
public:
    ...
    virtual BOOL OnNotify(ContextMenuParam param) { return BOOL(); }
...
public:
     ...
     */
    static MenuWndEx* CreateMenu(STRINGorID xml,
                                 CPoint point,
     ...
     * @return void 无返回值
     */
    void AttachSelect(const std::wstring& strName,
     ...
     * @return void 无返回值
     */
    void AttachPopupSubMenu(const std::wstring& strName,
...
private:
    ...
    void InitMenu(MenuElement* pOwner,
                  HWND hParent,
                  STRINGorID xml,
                  CPoint point,
                  int align,
...
public:
    ...
    /** @brief
     * 向已经弹出的子菜单窗口里添加临时菜单项，子菜单窗口销毁后菜单项也销毁
     ...
     * @return void 无返回值
     */
    void AttachPopupSubMenu(const EventCallback& callback) {
        OnEvent[kEventNotify] += callback;
    }
...
protected:
    virtual Control* FindControl(FINDCONTROLPROC Proc,
                                 LPVOID pData,
                                 UINT uFlags,
                                 CPoint scrollPos = CPoint()) override;
