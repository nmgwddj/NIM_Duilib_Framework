...
void CircleProgress::SetAttribute(const std::wstring& srName,
                                  const std::wstring& strValue) {
    if (srName == _T("circular"))
        SetCircular(strValue == _T("true"));
    else if (srName == _T("circlewidth"))
        SetCircleWidth(_ttoi(strValue.c_str()));
...
void CircleProgress::PaintStatusImage(IRenderContext* pRender) {
    ...
    if (m_bCircular) {
        ...
        //以后可能会调整：需实现1、DrawArc  2、Pen增加brush(渐变)入参
        //3、可以自由设置Graphics属性
        ...
        // 控件矩形内的最大正方形的边界
        int side =
            min(m_rcItem.right - m_rcItem.left, m_rcItem.bottom - m_rcItem.top);
        // UiRect rcBorder;  仍然存在UiRect 到
        // RectF的转换，所以直接用gdi的RectF了
        ...
        if (m_pIndicator) {
            outer.Inflate(-1.0F * m_pIndicator->GetWidth() / 2,
                          -1.0F * m_pIndicator->GetWidth() / 2);
        } else {
        ...
        if (m_dwGradientColor == 0) {
            ...
            graphics.DrawArc(
                &bgPen, outer, 270,
                360);  // 270从最上面开始递增，设为0的话，是最右边开始
            graphics.DrawArc(
                &fgPen, outer, 270,
                direction * 360 * (m_nValue - m_nMin) / (m_nMax - m_nMin));
        } else {
            Gdiplus::REAL factors[4] = {0.0f, 0.4f, 0.6f, 1.0f};
            Gdiplus::REAL positions[4] = {0.0f, 0.2f, 0.8f, 1.0f};

            Gdiplus::LinearGradientBrush lgbrush(
                rcBorder, m_dwForegroundColor, m_dwGradientColor,
        ...
        if (m_pIndicator) {
            ...
            matrix.RotateAt(
                direction * 360 * (m_nValue - m_nMin) / (m_nMax - m_nMin),
                Gdiplus::PointF(center.x, center.y),
...
void CircleProgress::SetIndicator(const std::wstring& sIndicatorImage) {
    if (m_sIndicatorImage != sIndicatorImage) {
        ...
        if (!::PathFileExistsW(imagepath.c_str())) {
            imagepath = GlobalManager::GetResourcePath() +
                        m_pWindow->GetWindowResourcePath() + imagepath;
        }
        if (!::PathFileExistsW(imagepath.c_str())) {
            return;
        }
        ...
        if (Gdiplus::Ok == state) {
            // 假定图片指向上
            m_pIndicator->RotateFlip(m_bClockwise ? Gdiplus::Rotate90FlipNone
