...
UIAControlProvider* ScrollBar::GetUIAProvider() {
    ...
    if (m_pUIAProvider == nullptr) {
        m_pUIAProvider = static_cast<UIAControlProvider*>(
            new (std::nothrow) UIAScrollBarProvider(this));
    }
    ...
    if (m_bHorizontal) {
        ...
        if (cy > m_cxyFixed.cx) {
            ...
            if (m_nRange > 0) {
                ...
                m_rcThumb.top = m_nScrollPos * (cy - cyThumb) / m_nRange +
                                m_rcButton1.bottom;
...
void ScrollBar::HandleMessage(EventArgs& event) {
    ...
    if (event.Type == kEventInternalSetFocus) {
        ...
        TimerManager::GetInstance()->AddCancelableTimer(
            m_weakFlagOwner.GetWeakFlag(), callback, 50,
        ...
        if (IsMouseFocused()) {
            if (!m_bHorizontal) {
                int vRange = m_rcItem.bottom - m_rcItem.top - m_rcThumb.bottom +
                             m_rcThumb.top;
                if (m_bShowButton1) {
                ...
                if (vRange != 0)
                    m_nLastScrollOffset =
                        (event.ptMouse.y - m_ptLastMouse.y) * m_nRange / vRange;
            } else {
                ...
                if (hRange != 0)
                    m_nLastScrollOffset =
                        (event.ptMouse.x - m_ptLastMouse.x) * m_nRange / hRange;
            }
        }
...
void ScrollBar::SetButton1StateImage(ControlStateType stateType,
...
void ScrollBar::SetButton2StateImage(ControlStateType stateType,
                                     const std::wstring& pStrImage) {
...
void ScrollBar::SetThumbStateImage(ControlStateType stateType,
                                   const std::wstring& pStrImage) {
...
void ScrollBar::SetRailStateImage(ControlStateType stateType,
                                  const std::wstring& pStrImage) {
...
void ScrollBar::SetBkStateImage(ControlStateType stateType,
                                const std::wstring& pStrImage) {
        ...
        if (!m_bHorizontal) {
            if (m_pOwner != NULL)
                m_pOwner->SetScrollPos(
                    CSize(m_pOwner->GetScrollPos().cx,
            ...
            if (m_pOwner != NULL)
                m_pOwner->SetScrollPos(
                    CSize(m_nLastScrollPos + m_nLastScrollOffset,
...
void ScrollBar::PaintButton1(IRenderContext* pRender) {
    ...
    m_sImageModify = StringHelper::Printf(
        ...
        m_rcButton1.left - m_rcItem.left, m_rcButton1.top - m_rcItem.top,
        m_rcButton1.right - m_rcItem.left, m_rcButton1.bottom - m_rcItem.top);

    if (m_uButton1State == kControlStateDisabled) {
        if (!DrawImage(pRender, m_button1StateImage[kControlStateDisabled],
                       m_sImageModify)) {
        } else
            return;
    } else if (m_uButton1State == kControlStatePushed) {
        if (!DrawImage(pRender, m_button1StateImage[kControlStatePushed],
                       m_sImageModify)) {
        } else
            return;

        if (!DrawImage(pRender, m_button1StateImage[kControlStateHot],
                       m_sImageModify)) {
        } else
            return;
    } else if (m_uButton1State == kControlStateHot ||
               m_uThumbState == kControlStatePushed) {
        if (!DrawImage(pRender, m_button1StateImage[kControlStateHot],
                       m_sImageModify)) {
        } else
    ...
    if (!DrawImage(pRender, m_button1StateImage[kControlStateNormal],
                   m_sImageModify)) {
    } else
        return;
}
...
void ScrollBar::PaintButton2(IRenderContext* pRender) {
    ...
    m_sImageModify = StringHelper::Printf(
        _T("destscale='false' dest='%d,%d,%d,%d'"),
        m_rcButton2.left - m_rcItem.left, m_rcButton2.top - m_rcItem.top,
        m_rcButton2.right - m_rcItem.left, m_rcButton2.bottom - m_rcItem.top);

    if (m_uButton2State == kControlStateDisabled) {
        if (!DrawImage(pRender, m_button2StateImage[kControlStateDisabled],
                       m_sImageModify)) {
        } else
            return;
    } else if (m_uButton2State == kControlStatePushed) {
        if (!DrawImage(pRender, m_button2StateImage[kControlStatePushed],
                       m_sImageModify)) {
        } else
            return;

        if (!DrawImage(pRender, m_button2StateImage[kControlStateHot],
                       m_sImageModify)) {
        } else
            return;
    } else if (m_uButton2State == kControlStateHot ||
               m_uThumbState == kControlStatePushed) {
        if (!DrawImage(pRender, m_button2StateImage[kControlStateHot],
                       m_sImageModify)) {
        } else
    ...
    if (!DrawImage(pRender, m_button2StateImage[kControlStateNormal],
                   m_sImageModify)) {
    } else
        return;
}
...
void ScrollBar::PaintThumb(IRenderContext* pRender) {
    ...
    m_sImageModify = StringHelper::Printf(
        _T("destscale='false' dest='%d,%d,%d,%d'"),
        m_rcThumb.left - m_rcItem.left, m_rcThumb.top - m_rcItem.top,
...
void ScrollBar::PaintRail(IRenderContext* pRender) {
    ...
    if (!m_bHorizontal) {
        m_sImageModify = StringHelper::Printf(
            ...
            (m_rcThumb.top + m_rcThumb.bottom) / 2 - m_rcItem.top -
                m_cxyFixed.cx / 2,
        ...
        m_sImageModify = StringHelper::Printf(
            _T("destscale='false' dest='%d,%d,%d,%d'"),
            (m_rcThumb.left + m_rcThumb.right) / 2 - m_rcItem.left -
                m_cxyFixed.cy / 2,
    ...
    if (m_uThumbState == kControlStateDisabled) {
        if (!DrawImage(pRender, m_railStateImage[kControlStateDisabled],
                       m_sImageModify)) {
        } else
            return;
    } else if (m_uThumbState == kControlStatePushed) {
        if (!DrawImage(pRender, m_railStateImage[kControlStatePushed],
                       m_sImageModify)) {
        } else
            return;

        if (!DrawImage(pRender, m_railStateImage[kControlStateHot],
                       m_sImageModify)) {
        } else
            return;
    ...
    if (!DrawImage(pRender, m_railStateImage[kControlStateNormal],
                   m_sImageModify)) {
    } else
        return;
}
