...
EXTERN_C const IID IID_ITextHost = {/* c5bdd8d0-d26e-11ce-a89e-00aa006cadc5 */
                                    ...
                                    0xd26e,
                                    0x11ce,
                                    {0xa8, 0x9e, 0x00, 0xaa, 0x00, 0x6c, 0xad,
                                     0xc5}};
...
#else
    // need to thunk pcf->szFaceName to a standard char string.in this case it's
    // easy because our thunk is also our copy
    MultiByteToWideChar(CP_ACP, 0, lf.lfFaceName, LF_FACESIZE, pcf->szFaceName,
                        LF_FACESIZE);
#endif
...
BOOL CTxtWinHost::Init(RichEdit* re, const CREATESTRUCT* pcs) {
    ...
    if (pcs && pcs->lpszName) {
        ...
        ::MultiByteToWideChar(CP_ACP, 0, pcs->lpszName, -1, (LPWSTR)lpText,
                              iLen);
...
BOOL CTxtWinHost::TxShowScrollBar(INT fnBar, BOOL fShow) {
    ...
    //    if( pVerticalScrollBar ) pVerticalScrollBar->SetVisible(fShow ==
    //    TRUE); if( pHorizontalScrollBar )
    //    pHorizontalScrollBar->SetVisible(fShow == TRUE);
    //}
    return TRUE;
}
...
void CTxtWinHost::SetFont(HFONT hFont) {
    ...
    // need to thunk pcf->szFaceName to a standard char string.in this case it's
    // easy because our thunk is also our copy
    MultiByteToWideChar(CP_ACP, 0, lf.lfFaceName, LF_FACESIZE, cf.szFaceName,
                        LF_FACESIZE);
#endif
...
BOOL CTxtWinHost::DoSetCursor(UiRect* prc, POINT* pt) {
    ...
    if (PtInRect(&rc, newPt)) {
        ...
        pserv->OnTxSetCursor(DVASPECT_CONTENT, -1, NULL, NULL,
                             m_re->GetWindowDC(), NULL, prcClient, newPt.x,
                             newPt.y);
...
void CTxtWinHost::GetControlRect(LPRECT prc) {
    ...
    if (alignType != kVerAlignTop) {
        ...
        GetTextServices()->TxGetNaturalSize(
            DVASPECT_CONTENT, m_re->GetWindowDC(), NULL, NULL,
            TXTNS_FITTOCONTENT, &szExtent, &iWidth, &iHeight);
...
void RichEdit::SetFont(const std::wstring& pStrFontName,
                       int nSize,
                       bool bBold,
                       bool bUnderline,
                       bool bStrikeout,
...
#ifdef _UNICODE
    TxSendMessage(EM_REPLACESEL, (WPARAM)bCanUndo, (LPARAM)lpszNewText.c_str(),
                  0);
#else
    std::wstring strOut;
...
int RichEdit::InsertText(long nInsertAfterChar,
                         LPCTSTR lpstrText,
                         bool bCanUndo) {
...
bool RichEdit::SetWordCharFormat(CHARFORMAT2& cf) {
    ...
    cf.cbSize = sizeof(CHARFORMAT2);
    LRESULT lResult;
    TxSendMessage(EM_SETCHARFORMAT, SCF_SELECTION | SCF_WORD, (LPARAM)&cf,
                  &lResult);
    return (BOOL)lResult == TRUE;
}
...
void RichEdit::PasteSpecial(UINT uClipFormat,
                            DWORD dwAspect /* = 0*/,
                            HMETAFILE hMF /* = 0*/) {
    if (NULL == m_pTwh)
        return;
...
HRESULT RichEdit::TxSendMessage(UINT msg,
                                WPARAM wParam,
                                LPARAM lParam,
                                LRESULT* plresult) const {
    if (m_pTwh) {
        LRESULT lr = m_pTwh->GetTextServices()->TxSendMessage(msg, wParam,
                                                              lParam, plresult);
        return lr;
    }
    return S_FALSE;
    ...
    TxSendMessage(
        ...
        ENM_DROPFILES |
            ENM_LINK,  // ENM_CHANGE| ENM_CORRECTTEXT | ENM_DRAGDROPDONE |
                       ...
                       // ENM_OBJECTPOSITIONS | ENM_PROTECTED |
                       // ENM_REQUESTRESIZE | ENM_SCROLL | ENM_SELCHANGE |
                       // ENM_UPDATE,
        &lResult);
    return (BOOL)lResult == FALSE;
}
    ...
    switch (iNotify) {
        ...
        case EN_DRAGDROPDONE: {
            if (pv) {  // Fill out NMHDR portion of pv
                ...
                SendMessage(this->GetWindowHandle(), WM_NOTIFY, (WPARAM)nId,
                            (LPARAM)pv);
            }
        } break;
    }
...
CSize RichEdit::GetNaturalSize(LONG width, LONG height) {
    ...
    if (m_pTwh) {
        m_pTwh->GetTextServices()->TxGetNaturalSize(
            DVASPECT_CONTENT, GetWindowDC(), NULL, NULL, TXTNS_FITTOCONTENT,
            &szExtent, &lWidth, &lHeight);
    }
    sz.cx = (int)lWidth;
    sz.cy = (int)lHeight;
    return sz;
...
void RichEdit::SetTimer(UINT idTimer, UINT uTimeout) {
    ...
    TimerManager::GetInstance()->AddCancelableTimer(
        m_timeFlagMap[idTimer].GetWeakFlag(), callback, uTimeout,
...
UIAControlProvider* RichEdit::GetUIAProvider() {
    ...
    if (m_pUIAProvider == nullptr) {
        m_pUIAProvider = static_cast<UIAControlProvider*>(
            new (std::nothrow) UIARichEditProvider(this));
    }
...
void RichEdit::DoInit() {
    ...
    if (m_pTwh) {
        ...
        LRESULT lResult;
        m_pTwh->GetTextServices()->TxSendMessage(EM_SETLANGOPTIONS, 0, 0,
                                                 &lResult);
        m_pTwh->GetTextServices()->TxSendMessage(EM_SETEVENTMASK, 0, ENM_CHANGE,
                                                 &lResult);
...
CSize RichEdit::EstimateSize(CSize szAvailable) {
    ...
    if (size.cx == DUI_LENGTH_AUTO || size.cy == DUI_LENGTH_AUTO) {
        ...
        m_pTwh->GetTextServices()->TxGetNaturalSize(
            DVASPECT_CONTENT, GetWindowDC(), NULL, NULL, TXTNS_FITTOCONTENT,
            &szExtent, &iWidth, &iHeight);

        if (size.cx == DUI_LENGTH_AUTO) {
            size.cx = iWidth + m_pLayout->GetPadding().left +
                      m_pLayout->GetPadding().right;
        } else if (size.cy == DUI_LENGTH_AUTO) {
            size.cy = iHeight + m_pLayout->GetPadding().top +
                      m_pLayout->GetPadding().bottom;
        }
    }
    return size;
}
...
CSize RichEdit::EstimateText(CSize szAvailable) {
    ...
    SIZEL szExtent = {-1, -1};
    m_pTwh->GetTextServices()->TxGetNaturalSize(
        DVASPECT_CONTENT, GetWindow()->GetPaintDC(), NULL, NULL,
        TXTNS_FITTOCONTENT, &szExtent, &iWidth, &iHeight);
...
void RichEdit::SetPos(UiRect rc) {
    ...
    if (m_pTwh) {
        ...
        if (bVScrollBarVisiable &&
            (!m_pVerticalScrollBar->IsValid() || m_bVScrollBarFixing)) {
            LONG lWidth =
    ...
    if (event.Type == kEventMouseButtonDown || event.Type == kEventPointDown) {
        if (m_linkInfo.size() > 0) {
            ...
            if (HittestCustomLink(event.ptMouse, strLink)) {
                ...
                if (m_pWindow != NULL)
                    m_pWindow->SendNotify((Control*)this,
...
void RichEdit::OnSetCursor(EventArgs& event) {
    ...
    if (m_pTwh && !IsReadOnly() && m_pTwh->DoSetCursor(NULL, &event.ptMouse)) {
        ...
        ::SetCursor(::LoadCursor(
            NULL, MAKEINTRESOURCE(IsReadOnly() ? IDC_ARROW : IDC_IBEAM)));
    }
}
...
void RichEdit::OnKeyDown(EventArgs& event) {
    if (event.wParam == VK_RETURN && ::GetAsyncKeyState(VK_SHIFT) >= 0) {
    ...
    } else if (m_bNumberOnly && event.wParam == 'V' &&
        ...
        if (!strClipText.empty()) {
            ...
            nbase::ThreadManager::PostTask([strClipText]() {
...
void RichEdit::SetAttribute(const std::wstring& strName,
    ...
    if (strName == _T("vscrollbar")) {
    ...
    } else if (strName == _T("prompttext"))
        SetPromptText(strValue);
    else if (strName == _T("prompttextid"))
        SetPromptTextId(strValue);
    else if (strName == _T("focusedimage"))
        ...
        SetTextId(strValue.c_str());
    else if (strName == _T("wanttab"))
        SetWantTab(strValue == _T("true"));
    else if (strName == _T("wantreturnmsg"))
        SetNeedReturnMsg(strValue == _T("true"));
    else if (strName == _T("returnmsgwantctrl"))
        SetReturnMsgWantCtrl(strValue == _T("true"));
    else if (strName == _T("rich"))
        SetRich(strValue == _T("true"));
    else if (strName == _T("maxchar"))
        SetLimitText(_ttoi(strValue.c_str()));
    else
...
BOOL RichEdit::ShowCaret(BOOL fShow) {
    if (fShow) {
        ...
        std::function<void()> closure =
            nbase::Bind(&RichEdit::ChangeCaretVisiable, this);
        TimerManager::GetInstance()->AddCancelableTimer(
            m_drawCaretFlag.GetWeakFlag(), closure, 500,
            TimerManager::REPEAT_FOREVER);
    } else {
        m_bIsCaretVisiable = false;
        m_drawCaretFlag.Cancel();
...
RECT RichEdit::GetCaretRect() {
    RECT rc = {m_iCaretPosX, m_iCaretPosY, m_iCaretPosX + m_iCaretWidth,
               m_iCaretPosY + m_iCaretHeight};
    return rc;
}
...
void RichEdit::PaintCaret(IRenderContext* pRender, const UiRect& rcPaint) {
    ...
    if (m_bIsCaretVisiable && !m_bIsComposition) {
        UiRect rect(m_iCaretPosX, m_iCaretPosY, m_iCaretPosX,
                    m_iCaretPosY + m_iCaretHeight);
...
std::wstring RichEdit::GetPromptText() const {
    ...
    if (strText.empty() && !m_sPromptTextId.empty()) {
        strText =
            MutiLanSupport::GetInstance()->GetStringViaID(m_sPromptTextId);
    }
...
void RichEdit::AddColorText(const std::wstring& str,
...
void RichEdit::AddLinkColorText(const std::wstring& str,
                                const std::wstring& color,
                                const std::wstring& linkInfo) {
    if (!m_bRich || str.empty() || color.empty()) {
        ASSERT(FALSE);
        return;
    }
...
void RichEdit::AddLinkColorTextEx(const std::wstring& str,
                                  const std::wstring& color,
                                  const std::wstring& linkInfo,
    ...
    static std::string link_format =
        "{\\rtf1%s%s\\f0\\fs%d{\\field{\\*\\fldinst{HYPERLINK "
        "\"%s\"}}{\\fldrslt{\\cf1 %s}}}}";
    char sfont[255];
    sprintf(sfont, font_format.c_str(), lf.lfCharSet, font_face.c_str());
    char scolor[255];
    sprintf(scolor, color_format.c_str(), GetBValue(dwTextColor),
            GetGValue(dwTextColor), GetRValue(dwTextColor));
    char slinke[1024];
    sprintf(slinke, link_format.c_str(), sfont, scolor,
            ((int)(-lf.lfHeight * 1.5)) / 2 * 2, link.c_str(), text.c_str());
    std::wstring temp;
...
void RichEdit::RaiseUIAValueEvent(const std::wstring oldText,
    ...
    if (m_pUIAProvider != nullptr && UiaClientsAreListening()) {
        ...
        UiaRaiseAutomationPropertyChangedEvent(
            m_pUIAProvider, UIA_ValueValuePropertyId, vtOld, vtNew);
    }
#endif
}
...
void SetClipBoardText(const std::wstring& str) {
    ...
    ::memcpy(lpStr, str.c_str(),
             len * sizeof(wchar_t));  //把数据拷贝考全局内存中
