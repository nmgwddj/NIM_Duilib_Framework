...
public:
    ...
    /**
     * @brief 设置控件为密码控件（显示 ***）
     * @param[in] bPassword 设置为 true 让控件显示内容为 ***，false
     * 为显示正常内容
     * @return 无
     */
     ...
     * @param[in] bWordWrap 为 true 时并且在多行模式下内容被换行显示，false
     * 则表示截断显示
     * @return 无
     */
     ...
     * @return 无
     */
    void SetFont(const std::wstring& pStrFontName,
                 int nSize,
                 bool bBold,
                 bool bUnderline,
                 bool bStrikeout,
    ...
    /**
     * @brief 获取内容的长度
     * @param[in] dwFlags 指定用以确定文本长度的方式，参考
    ...
    /**
     * @brief 设置控件的文本对应 ID
     * @param[in] strTextId 要设置的 UTF8 格式 ID，该 ID
     * 必须在加载的语言文件中存在
     * @return 无
     */
    ...
    /**
     * @brief 设置修改标志
     * @param[in] bModified 设置为 true 表示文本已经被修改，false
     * 为未修改，默认为 true
     * @return 无
     */
     ...
     * @brief 获取控件的选择类型
     * @return
     * 返回控件的选择类型，参考：https://docs.microsoft.com/en-us/windows/desktop/controls/em-selectiontype
     */
     ...
     * @param[in] bAutoDetect 设置为 true 表示检测 URL，false 为不检测，默认为
     * true
     ...
     * @return 返回插入后的文本位置
     */
    int InsertText(long nInsertAfterChar,
                   LPCTSTR lpstrText,
                   bool bCanUndo = false);
     ...
     * @param[in] nFormat
     * 指定数据格式的替换选项，见：https://docs.microsoft.com/en-us/windows/desktop/controls/em-streamin
    ...
    virtual void PaintChild(IRenderContext* pRender,
                            const UiRect& rcPaint) override;
    virtual void SetAttribute(const std::wstring& pstrName,
                              const std::wstring& pstrValue) override;
    ...
    /**
     * @brief 设置提示文字 ID
     * @param[in] strText 要设置的 UTF8 格式提示文字 ID，该 ID
     * 必须在加载的语言文件中存在
     * @return 无
     */
     ...
     * @return 无
     */
    void AddLinkColorText(const std::wstring& str,
                          const std::wstring& color,
                          const std::wstring& linkInfo = L"");
     ...
     * @return 无
     */
    void AddLinkColorTextEx(const std::wstring& str,
                            const std::wstring& color,
                            const std::wstring& linkInfo = L"",
                            const std::wstring& strFontId = L"");
    ...
    /**
     * @brief 添加一个范围用于 hittest
     * 判断是否是链接信息,并将该范围内文字样式改为系统链接样式
     * @param[in] str 文字内容
     * @param[in] cr 范围的起始位置和结束位置
     * @param[in] linkInfo 自定义 link 属性
     * @return 无
     ...
     * @return 无
     */
    void RaiseUIAValueEvent(const std::wstring oldText,
