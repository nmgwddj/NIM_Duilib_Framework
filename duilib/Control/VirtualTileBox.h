...
public:
    ...
    void RegNotifys(const DataChangedNotify& dcNotify,
...
public:
    VirtualTileLayout();
    virtual ui::CSize ArrangeChild(const std::vector<ui::Control*>& items,
                                   ui::UiRect rc) override;
    virtual ui::CSize AjustSizeByChild(const std::vector<ui::Control*>& items,
                                       ui::CSize szAvailable) override;
...
public:
    ...
    /**
     * @brief 设置数据代理对象
     * @param[in] pProvider 开发者需要重写 VirtualListInterface
     * 的接口来作为数据代理对象
     * @return 无
     */
...
private:
    enum ScrollDirection { kScrollUp = -1, kScrollDown = 1 };
