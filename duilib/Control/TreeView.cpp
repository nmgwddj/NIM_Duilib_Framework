...
UIAControlProvider* TreeNode::GetUIAProvider() {
    ...
    if (m_pUIAProvider == nullptr) {
        m_pUIAProvider = static_cast<UIAControlProvider*>(
            new (std::nothrow) UIATreeNodeProvider(this));
    }
...
bool TreeNode::IsVisible() const {
    return ListContainerElement::IsVisible() &&
           (!m_pParentTreeNode ||
            (m_pParentTreeNode && m_pParentTreeNode->IsExpand() &&
             m_pParentTreeNode->IsVisible()));
}
...
bool TreeNode::AddChildNodeAt(TreeNode* pTreeNode, std::size_t iIndex) {
    ...
    pTreeNode->OnEvent[kEventClick] +=
        nbase::Bind(&TreeNode::OnClickItem, this, std::placeholders::_1);
    ...
    return m_pTreeView->ListBox::AddAt(pTreeNode,
                                       (int)(nodeIndex + nGlobalIndex + 1));
}
...
void TreeNode::SetExpand(bool bExpand, bool bTriggerEvent) {
    ...
    if (m_pWindow != NULL) {
        if (bTriggerEvent) {
            m_pWindow->SendNotify(this,
                                  m_bExpand ? kEventExpand : kEventUnExpand);
        }
    }
...
void TreeView::SetAttribute(const std::wstring& strName,
