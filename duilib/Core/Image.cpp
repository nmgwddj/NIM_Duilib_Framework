...
std::unique_ptr<ImageInfo> ImageInfo::LoadImage(
    ...
    std::unique_ptr<Gdiplus::Bitmap> gdiplusBitmap(
        Gdiplus::Bitmap::FromFile(strImageFullPath.c_str()));
...
std::unique_ptr<ImageInfo> ImageInfo::LoadImageByBitmap(
    std::unique_ptr<Gdiplus::Bitmap>& pGdiplusBitmap,
    const std::wstring& strImageFullPath) {
    ...
    if (iFrameCount > 1) {
        ...
        Gdiplus::PropertyItem* pPropertyItem =
            (Gdiplus::PropertyItem*)malloc(iSize);
        status = pGdiplusBitmap->GetPropertyItem(PropertyTagFrameDelay, iSize,
                                                 pPropertyItem);
        ASSERT(status == Gdiplus::Ok);
        if (status != Gdiplus::Ok) {
            return nullptr;
    ...
    for (int i = 0; i < iFrameCount; i++) {
        status =
            pGdiplusBitmap->SelectActiveFrame(&Gdiplus::FrameDimensionTime, i);
        ASSERT(status == Gdiplus::Ok);
        if (status != Gdiplus::Ok) {
            return nullptr;
        }
    ...
    if ((format & PixelFormatIndexed) != 0) {
        ...
        if (nPalSize > 0) {
            Gdiplus::ColorPalette* palette =
                (Gdiplus::ColorPalette*)malloc(nPalSize);
            ;
            status = pGdiplusBitmap->GetPalette(palette, nPalSize);
            if (status == Gdiplus::Ok) {
                imageInfo->SetAlpha(
                    (palette->Flags & Gdiplus::PaletteFlagsHasAlpha) != 0);
            }
            free(palette);
        }
    }
...
void ImageAttribute::ModifyAttribute(ImageAttribute& imageAttribute,
    ...
    while (*pStrImage != _T('\0')) {
        ...
        if (!sValue.empty()) {
            if (sItem == _T("file") || sItem == _T("res")) {
                ...
                imageAttribute.bFade =
                    (BYTE)_tcstoul(sValue.c_str(), &pstr, 10);
            } else if (sItem == _T("xtiled")) {
                imageAttribute.bTiledX =
                    (_tcscmp(sValue.c_str(), _T("true")) == 0);
            } else if (sItem == _T("fullxtiled")) {
                imageAttribute.bFullTiledX =
                    (_tcscmp(sValue.c_str(), _T("true")) == 0);
            } else if (sItem == _T("ytiled")) {
                imageAttribute.bTiledY =
                    (_tcscmp(sValue.c_str(), _T("true")) == 0);
            } else if (sItem == _T("fullytiled")) {
                imageAttribute.bFullTiledY =
                    (_tcscmp(sValue.c_str(), _T("true")) == 0);
            } else if (sItem == _T("tiledmargin")) {
                imageAttribute.nTiledMargin =
                    _tcstol(sValue.c_str(), &pstr, 10);
                ASSERT(pstr);
            } else if (sItem == _T("playcount")) {
                imageAttribute.nPlayCount = _tcstol(sValue.c_str(), &pstr, 10);
                ASSERT(pstr);
            }
        }
        if (*pStrImage++ != _T(' '))
...
bool StateImage::HasHotImage() {
    return !m_stateImageMap[kControlStateHot]
                .imageAttribute.simageString.empty();
}

bool StateImage::HasImage() {
    return !m_stateImageMap[kControlStateNormal]
                .imageAttribute.simageString.empty() ||
           !m_stateImageMap[kControlStateHot]
                .imageAttribute.simageString.empty() ||
           !m_stateImageMap[kControlStatePushed]
                .imageAttribute.simageString.empty() ||
           !m_stateImageMap[kControlStateDisabled]
                .imageAttribute.simageString.empty();
}

bool StateImage::PaintStatusImage(IRenderContext* pRender,
                                  ControlStateType stateType,
                                  const std::wstring& sImageModify /*= L""*/) {
    if (m_pControl) {
        bool bFadeHot = m_pControl->GetAnimationManager().GetAnimationPlayer(
                            kAnimationHot) != nullptr;
        int nHotAlpha = m_pControl->GetHotAlpha();
        if (bFadeHot) {
            if (stateType == kControlStateNormal ||
                stateType == kControlStateHot) {
                std::wstring strNormalImagePath =
                    m_stateImageMap[kControlStateNormal]
                        .imageAttribute.sImageName;
                std::wstring strHotImagePath =
                    m_stateImageMap[kControlStateHot].imageAttribute.sImageName;

                if (strNormalImagePath.empty() || strHotImagePath.empty() ||
                    strNormalImagePath != strHotImagePath ||
                    !m_stateImageMap[kControlStateNormal]
                         .imageAttribute.rcSource.Equal(
                             m_stateImageMap[kControlStateHot]
                                 .imageAttribute.rcSource)) {
                    m_pControl->DrawImage(pRender,
                                          m_stateImageMap[kControlStateNormal],
                                          sImageModify);
                    int nHotFade =
                        m_stateImageMap[kControlStateHot].imageAttribute.bFade;
                    nHotFade = int(nHotFade * (double)nHotAlpha / 255);
                    return m_pControl->DrawImage(
                        pRender, m_stateImageMap[kControlStateHot],
                        sImageModify, nHotFade);
                } else {
                    int nNormalFade = m_stateImageMap[kControlStateNormal]
                                          .imageAttribute.bFade;
                    int nHotFade =
                        m_stateImageMap[kControlStateHot].imageAttribute.bFade;
                    int nBlendFade =
                        int((1 - (double)nHotAlpha / 255) * nNormalFade +
                            (double)nHotAlpha / 255 * nHotFade);
                    return m_pControl->DrawImage(
                        pRender, m_stateImageMap[kControlStateHot],
                        sImageModify, nBlendFade);
                }
            }
        }
    }
    ...
    return m_pControl->DrawImage(pRender, m_stateImageMap[stateType],
                                 sImageModify);
}

Image* StateImage::GetEstimateImage() {
    Image* pEstimateImage = nullptr;
    if (!m_stateImageMap[kControlStateNormal]
             .imageAttribute.sImageName.empty()) {
        pEstimateImage = &m_stateImageMap[kControlStateNormal];
    } else if (!m_stateImageMap[kControlStateHot]
                    .imageAttribute.sImageName.empty()) {
        pEstimateImage = &m_stateImageMap[kControlStateHot];
    } else if (!m_stateImageMap[kControlStatePushed]
                    .imageAttribute.sImageName.empty()) {
        pEstimateImage = &m_stateImageMap[kControlStatePushed];
    } else if (!m_stateImageMap[kControlStateDisabled]
                    .imageAttribute.sImageName.empty()) {
...
void StateImageMap::SetImage(StateImageType stateImageType,
                             ControlStateType stateType,
                             const std::wstring& strImagePath) {
    m_stateImageMap[stateImageType][stateType].SetImageString(strImagePath);
}

std::wstring StateImageMap::GetImagePath(StateImageType stateImageType,
                                         ControlStateType stateType) {
    return m_stateImageMap[stateImageType][stateType]
        .imageAttribute.simageString;
}
...
bool StateImageMap::PaintStatusImage(
    IRenderContext* pRender,
...
void StateColorMap::PaintStatusColor(IRenderContext* pRender,
                                     UiRect rcPaint,
                                     ControlStateType stateType) {
    if (m_pControl) {
        bool bFadeHot = m_pControl->GetAnimationManager().GetAnimationPlayer(
        ...
        if (bFadeHot) {
            if ((stateType == kControlStateNormal ||
                ...
                if (!strColor.empty()) {
                    pRender->DrawColor(rcPaint,
                                       m_pControl->GetWindowColor(strColor));
                }
                if (nHotAlpha > 0) {
                    pRender->DrawColor(rcPaint,
                                       m_pControl->GetWindowColor(
                                           m_stateColorMap[kControlStateHot]),
                                       nHotAlpha);
                }
                return;
            }
    ...
    if (!strColor.empty()) {
        DWORD color = m_pControl ? m_pControl->GetWindowColor(strColor)
                                 : GlobalManager::GetTextColor(strColor);
