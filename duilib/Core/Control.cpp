...
void Control::SetStateColor(ControlStateType stateType,
                            const std::wstring& strColor) {
    ASSERT(this->GetWindowColor(strColor) != 0);
    if (m_colorMap[stateType] == strColor)
        return;
...
std::string Control::GetUTF8BkImage() const {
    std::string strOut;
    StringHelper::UnicodeToMBCS(m_bkImage.imageAttribute.simageString.c_str(),
                                strOut, CP_UTF8);
    return strOut;
}
...
void Control::SetBkImage(const std::wstring& strImage) {
    ...
    if (GetFixedWidth() == DUI_LENGTH_AUTO ||
        GetFixedHeight() == DUI_LENGTH_AUTO) {
        ArrangeAncestor();
    } else {
        Invalidate();
    }
...
void Control::SetStateImage(ControlStateType stateType,
                            const std::wstring& strImage) {
    if (stateType == kControlStateHot) {
        m_animationManager.SetFadeHot(true);
    }
...
void Control::SetForeStateImage(ControlStateType stateType,
                                const std::wstring& strImage) {
    if (stateType == kControlStateHot) {
        m_animationManager.SetFadeHot(true);
    }
...
std::wstring Control::GetToolTipText() const {
    ...
    if (strText.empty() && !m_sToolTipTextId.empty()) {
        strText =
            MutiLanSupport::GetInstance()->GetStringViaID(m_sToolTipTextId);
    }
...
Control* Control::FindControl(FINDCONTROLPROC Proc,
                              LPVOID pData,
                              UINT uFlags,
                              CPoint scrollPos) {
    ...
    if (GetFixedWidth() == DUI_LENGTH_AUTO ||
        ...
        if (image) {
            ...
            if (imageAttribute.rcSource.left != DUI_NOSET_VALUE &&
                ...
                if ((GetFixedWidth() != imageAttribute.rcSource.right -
                                            imageAttribute.rcSource.left)) {
                    SetFixedWidth(imageAttribute.rcSource.right -
                                  imageAttribute.rcSource.left);
                }
                if ((GetFixedHeight() != imageAttribute.rcSource.bottom -
                                             imageAttribute.rcSource.top)) {
                    SetFixedHeight(imageAttribute.rcSource.bottom -
                                   imageAttribute.rcSource.top);
                }
                return m_cxyFixed;
            }
...
void Control::HandleMessageTemplate(EventType eventType,
                                    WPARAM wParam,
                                    LPARAM lParam,
                                    TCHAR tChar,
                                    CPoint mousePos,
...
void Control::HandleMessageTemplate(EventArgs& msg) {
    if (msg.Type == kEventInternalDoubleClick ||
        msg.Type == kEventInternalMenu || msg.Type == kEventInternalSetFocus ||
...
void Control::HandleMessage(EventArgs& msg) {
    if (!IsMouseEnabled() && msg.Type > kEventMouseBegin &&
        ...
        if (IsContextMenuUsed()) {
            m_pWindow->SendNotify(this, kEventMouseMenu, msg.wParam,
                                  msg.lParam);
            return;
        }
        ...
        if (m_pWindow != NULL)
            m_pWindow->SendNotify(this, kEventMouseDoubleClick);
        return;
    }
...
void Control::SetAttribute(const std::wstring& strName,
    ...
    if (strName == _T("class")) {
        ...
        UiRect rcMargin;
        LPTSTR pstr = NULL;
        rcMargin.left = _tcstol(strValue.c_str(), &pstr, 10);
        ASSERT(pstr);
        ...
        if (nValue.find(',') == std::wstring::npos) {
            ...
            UiRect rcBorder;
            LPTSTR pstr = NULL;
            rcBorder.left = _tcstol(strValue.c_str(), &pstr, 10);
            ASSERT(pstr);
        ...
        CSize cxyRound;
        LPTSTR pstr = NULL;
        cxyRound.cx = _tcstol(strValue.c_str(), &pstr, 10);
        ASSERT(pstr);
        cxyRound.cy = _tcstol(pstr + 1, &pstr, 10);
        ASSERT(pstr);
        SetBorderRound(cxyRound);
    } else if (strName == _T("boxshadow"))
        ...
        CPoint renderOffset;
        LPTSTR pstr = NULL;
        renderOffset.x = _tcstol(strValue.c_str(), &pstr, 10);
        ASSERT(pstr);
    ...
    } else if (strName == _T("normalcolor"))
        SetStateColor(kControlStateNormal, strValue);
    else if (strName == _T("hotcolor"))
        SetStateColor(kControlStateHot, strValue);
    else if (strName == _T("pushedcolor"))
        SetStateColor(kControlStatePushed, strValue);
    else if (strName == _T("disabledcolor"))
        SetStateColor(kControlStateDisabled, strValue);
    else if (strName == _T("bordercolor"))
        SetBorderColor(strValue);
    else if (strName == _T("leftbordersize"))
        SetLeftBorderSize(_ttoi(strValue.c_str()));
    else if (strName == _T("topbordersize"))
        SetTopBorderSize(_ttoi(strValue.c_str()));
    else if (strName == _T("rightbordersize"))
        SetRightBorderSize(_ttoi(strValue.c_str()));
    else if (strName == _T("bottombordersize"))
        SetBottomBorderSize(_ttoi(strValue.c_str()));
    else if (strName == _T("bkimage"))
        SetBkImage(strValue);
    else if (strName == _T("minwidth"))
        SetMinWidth(_ttoi(strValue.c_str()));
    else if (strName == _T("minheight"))
        SetMinHeight(_ttoi(strValue.c_str()));
    else if (strName == _T("name"))
        SetName(strValue);
    else if (strName == _T("tooltiptext"))
        SetToolTipText(strValue);
    else if (strName == _T("tooltiptextid"))
        SetToolTipTextId(strValue);
    else if (strName == _T("dataid"))
        SetDataID(strValue);
    else if (strName == _T("menupopup"))
        SetMenuPopup(strValue);
    else if (strName == _T("menualign"))
        SetMenuAlign(strValue);
    else if (strName == _T("enabled"))
        SetEnabled(strValue == _T("true"));
    else if (strName == _T("mouse"))
        SetMouseEnabled(strValue == _T("true"));
    else if (strName == _T("keyboard"))
        SetKeyboardEnabled(strValue == _T("true"));
    else if (strName == _T("visible"))
        SetVisible_(strValue == _T("true"));
    else if (strName == _T("fadevisible"))
        SetVisible(strValue == _T("true"));
    else if (strName == _T("float"))
        SetFloat(strValue == _T("true"));
    else if (strName == _T("menu"))
        SetContextMenuUsed(strValue == _T("true"));
    else if (strName == _T("cache"))
        SetUseCache(strValue == _T("true"));
    else if (strName == _T("nofocus"))
        SetNoFocus();
    else if (strName == _T("alpha"))
        SetAlpha(_ttoi(strValue.c_str()));
    else if (strName == _T("normalimage"))
        SetStateImage(kControlStateNormal, strValue);
    else if (strName == _T("hotimage"))
        SetStateImage(kControlStateHot, strValue);
    else if (strName == _T("pushedimage"))
        SetStateImage(kControlStatePushed, strValue);
    else if (strName == _T("disabledimage"))
        SetStateImage(kControlStateDisabled, strValue);
    else if (strName == _T("forenormalimage"))
        SetForeStateImage(kControlStateNormal, strValue);
    else if (strName == _T("forehotimage"))
        SetForeStateImage(kControlStateHot, strValue);
    else if (strName == _T("forepushedimage"))
        SetForeStateImage(kControlStatePushed, strValue);
    else if (strName == _T("foredisabledimage"))
        SetForeStateImage(kControlStateDisabled, strValue);
    else if (strName == _T("fadealpha"))
        m_animationManager.SetFadeAlpha(strValue == _T("true"));
    else if (strName == _T("fadehot"))
        m_animationManager.SetFadeHot(strValue == _T("true"));
    else if (strName == _T("fadewidth"))
        m_animationManager.SetFadeWidth(strValue == _T("true"));
    else if (strName == _T("fadeheight"))
        m_animationManager.SetFadeHeight(strValue == _T("true"));
    else if (strName == _T("fadeinoutxfromleft"))
        m_animationManager.SetFadeInOutX(strValue == _T("true"), false);
    else if (strName == _T("fadeinoutxfromright"))
        m_animationManager.SetFadeInOutX(strValue == _T("true"), true);
    else if (strName == _T("fadeinoutyfromtop"))
        m_animationManager.SetFadeInOutY(strValue == _T("true"), false);
    else if (strName == _T("fadeinoutyfrombottom"))
        m_animationManager.SetFadeInOutY(strValue == _T("true"), true);
    else if (strName == _T("receivepointer"))
        SetReceivePointerMsg(strValue == _T("true"));
    else if (strName == _T("tabstop"))
        SetTabStop(strValue == _T("true"));
    else if (strName == _T("loadingimage"))
        SetLoadingImage(strValue);
    else if (strName == _T("loadingbkcolor"))
...
void Control::SetClass(const std::wstring& strClass) {
    ...
    for (auto it = splitList.begin(); it != splitList.end(); it++) {
        std::wstring pDefaultAttributes =
            GlobalManager::GetClassAttributes((*it));
...
bool Control::OnApplyAttributeList(const std::wstring& strReceiver,
                                   const std::wstring& strList,
                                   EventArgs* eventArgs) {
    Control* pReceiverControl;
    if (strReceiver.substr(0, 2) == L".\\" ||
        strReceiver.substr(0, 2) == L"./") {
...
void Control::GetImage(Image& duiImage) const {
    ...
    std::wstring imageFullPath = GlobalManager::GetResPath(
        sImageName, m_pWindow->GetWindowResourcePath());
...
bool Control::DrawImage(IRenderContext* pRender,
                        Image& duiImage,
    ...
    if (m_bkImage.imageCache && m_bkImage.imageCache->IsGif() && m_bGifPlay &&
        !m_bkImage.IsPlaying()) {
        GifPlay();
    } else {
        ...
        pRender->DrawImage(
            m_rcPaint, duiImage.GetCurrentHBitmap(), imageInfo->IsAlpha(),
            rcNewDest, rcNewSource, newImageAttribute.rcCorner,
            imageInfo->IsSvg(), iFade, newImageAttribute.bTiledX,
            newImageAttribute.bTiledY, newImageAttribute.bFullTiledX,
            newImageAttribute.bFullTiledY, newImageAttribute.nTiledMargin);
    }
...
void Control::AlphaPaint(IRenderContext* pRender, const UiRect& rcPaint) {
    ...
    UiRect rcUnion;
    if (!::IntersectRect(&rcUnion, &rcPaint, &m_rcItem))
        return;
    ...
    if (IsAlpha()) {
        ...
        if (pCacheRender) {
            ...
            if (m_bCacheDirty) {
                pCacheRender->Clear();
                int scaleOffset = m_boxShadow.HasShadow()
                                      ? (m_boxShadow.m_nBlurSize * 2 +
                                         abs(m_boxShadow.m_cpOffset.x))
                                      : 0;
                UiRect rcClip = {0, 0, size.cx + scaleOffset,
                                 size.cy + scaleOffset};

                AutoClip alphaClip(pCacheRender, rcClip, IsClip());
                AutoClip roundAlphaClip(pCacheRender, rcClip,
                                        m_cxyBorderRound.cx,
                                        m_cxyBorderRound.cy, bRoundClip);

                pCacheRender->SetRenderTransparent(true);
                CPoint ptOffset(m_rcItem.left + m_renderOffset.x,
                                m_rcItem.top + m_renderOffset.y);
            ...
            pRender->AlphaBlend(
                rcUnion.left, rcUnion.top, rcUnion.right - rcUnion.left,
                rcUnion.bottom - rcUnion.top, pCacheRender->GetDC(),
                rcUnion.left - m_rcItem.left, rcUnion.top - m_rcItem.top,
                rcUnion.right - rcUnion.left, rcUnion.bottom - rcUnion.top,
                m_nAlpha);
        ...
        int scaleOffset =
            m_boxShadow.HasShadow()
                ? (m_boxShadow.m_nBlurSize + abs(m_boxShadow.m_cpOffset.x))
                : 0;
        UiRect rcClip = {
            m_rcItem.left - scaleOffset,
            m_rcItem.top - scaleOffset,
            m_rcItem.right + scaleOffset,
            m_rcItem.bottom + scaleOffset,
        };
        AutoClip clip(pRender, rcClip, IsClip());
        AutoClip roundClip(pRender, rcClip, m_cxyBorderRound.cx,
                           m_cxyBorderRound.cy, bRoundClip);
...
void Control::PaintShadow(IRenderContext* pRender) {
    ...
    pRender->DrawBoxShadow(m_rcPaint, m_cxyBorderRound, m_boxShadow.m_cpOffset,
                           m_boxShadow.m_nBlurRadius, m_boxShadow.m_nBlurSize,
...
void Control::PaintBorder(IRenderContext* pRender) {
    ...
    if (dwBorderColor != 0) {
        if (m_rcBorderSize.left > 0 || m_rcBorderSize.top > 0 ||
            m_rcBorderSize.right > 0 || m_rcBorderSize.bottom > 0) {
            UiRect rcBorder;
            if (m_rcBorderSize.left > 0) {
                rcBorder = m_rcItem;
                rcBorder.right = rcBorder.left =
                    m_rcItem.left + m_rcBorderSize.left / 2;
            ...
            if (m_rcBorderSize.top > 0) {
                ...
                rcBorder.bottom = rcBorder.top =
                    m_rcItem.top + m_rcBorderSize.top / 2;
            ...
            if (m_rcBorderSize.right > 0) {
                ...
                rcBorder.left = rcBorder.right =
                    m_rcItem.right - (m_rcBorderSize.right + 1) / 2;
                if (m_rcBorderSize.right == 1) {
                    rcBorder.bottom -= 1;
                }
                pRender->DrawLine(rcBorder, m_rcBorderSize.right,
                                  dwBorderColor);
            }
            if (m_rcBorderSize.bottom > 0) {
                rcBorder = m_rcItem;
                rcBorder.top = rcBorder.bottom =
                    m_rcItem.bottom - (m_rcBorderSize.bottom + 1) / 2;
                if (m_rcBorderSize.bottom == 1) {
                    rcBorder.right -= 1;
                }
                pRender->DrawLine(rcBorder, m_rcBorderSize.bottom,
                                  dwBorderColor);
            }
            ...
            if (m_cxyBorderRound.cx > 0 || m_cxyBorderRound.cy > 0)
                pRender->DrawRoundRect(rcDraw, m_cxyBorderRound, m_nBorderSize,
                                       dwBorderColor);
            else
...
void Control::PaintLoading(IRenderContext* pRender) {
    ...
    Gdiplus::Bitmap* image = GdiHelper::CreateBitmapFromHBITMAP(
        m_loadingImage.imageCache->GetHBitmap(0));
    if (!image) {
        ASSERT(FALSE);
        return;
    }
    if (!m_strLoadingBkColor.empty()) {
        ...
        rcFill.left =
            m_rcItem.left + (m_rcItem.GetWidth() - image->GetWidth()) / 2;
        rcFill.right = rcFill.left + image->GetWidth();
        rcFill.top =
            m_rcItem.top + (m_rcItem.GetHeight() - image->GetHeight()) / 2;
    ...
    matrix.RotateAt(
        static_cast<float>(m_fCurrrentAngele),
        Gdiplus::PointF(image->GetWidth() / 2, image->GetHeight() / 2));
    ...
    graphics.DrawImage(
        &tempBitmap,
        Gdiplus::RectF(m_rcItem.left + (GetWidth() - image->GetWidth()) / 2,
                       m_rcItem.top + (GetHeight() - image->GetHeight()) / 2,
...
void Control::GifPlay() {
    ...
    if (!m_bkImage.IsPlaying()) {
        ...
        TimerManager::GetInstance()->AddCancelableTimer(
            m_gifWeakFlag.GetWeakFlag(), gifPlayCallback, lPause,
        ...
        if (!m_bkImage.ContinuePlay()) {
            ...
            if (lPrePause == 0 ||
                lPause == 0) {  // 0~{1mJ>~}GetCurrentInterval~{3v4m~}
                m_bkImage.SetPlaying(false);
                m_gifWeakFlag.Cancel();
                return;
            }
...
void Control::InvokeLoadImageCache() {
    ...
    if (!m_bkImage.imageCache ||
        m_bkImage.imageCache->sImageFullPath != imageFullPath) {
...
void Control::StartLoading(int fStartAngle) {
    ...
    SetEnabled(false);
    TimerManager::GetInstance()->AddCancelableTimer(
        m_loadingImageFlag.GetWeakFlag(), nbase::Bind(&Control::Loading, this),
