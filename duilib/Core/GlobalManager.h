...
public:
     ...
     * @return 无
     */
    static void ReloadLanguage(const std::wstring& languagePath,
     ...
     * @return 无
     */
    static void AddClass(const std::wstring& strClassName,
     ...
     * @return 无
     */
    static void AddTextColor(const std::wstring& strName,
     ...
     */
    static std::shared_ptr<ImageInfo> AddImageCached(
        const std::shared_ptr<ImageInfo>& shared_image);
     ...
     */
    static HFONT AddFont(const std::wstring& strFontId,
     ...
     * @return 返回字体的 HFONT 句柄
     */
    static HFONT GetFont(const std::wstring& strFontName,
                         int nSize,
                         bool bBold,
                         bool bUnderline,
                         bool bStrikeout,
     ...
     *     @retval false 不存在
     */
    static bool FindFont(const std::wstring& strFontName,
                         int nSize,
                         bool bBold,
                         bool bUnderline,
                         bool bStrikeout,
     ...
     */
    static bool OpenResZip(LPCTSTR resource_name,
     ...
     * @return 返回 true 打开成功，返回 false 为打开失败
     */
    static bool OpenResZip(const std::wstring& path,
                           const std::string& password);
     ...
     * @return 可用的资源路径
     */
    static std::wstring GetResPath(const std::wstring& res_path,
...
private:
    ...
    public:
        ...
        // 首先比较，长度小的更小
        // 再逆向比较，可以认为是逆向字典序
        bool operator()(const std::wstring& key1,
                        const std::wstring& key2) const;
    };
    typedef std::
        map<std::wstring, std::weak_ptr<ImageInfo>, ImageCacheKeyCompare>
            MapStringToImagePtr;

    static std::wstring
        m_pStrResourcePath;  //全局的资源路径，换肤的时候修改这个变量
