...
bool Layout::SetAttribute(const std::wstring& strName,
                          const std::wstring& strValue) {
    bool hasAttribute = true;
    if (strName == _T("padding")) {
        UiRect rcPadding;
        LPTSTR pstr = NULL;
        rcPadding.left = _tcstol(strValue.c_str(), &pstr, 10);
        ASSERT(pstr);
...
CSize Layout::AjustSizeByChild(const std::vector<Control*>& items,
                               CSize szAvailable) {
    ...
    for (auto it = items.begin(); it != items.end(); it++) {
        ...
        maxSize.cx = MAX(
            itemSize.cx + (*it)->GetMargin().left + (*it)->GetMargin().right,
            maxSize.cx);
        maxSize.cy = MAX(
            itemSize.cy + (*it)->GetMargin().top + (*it)->GetMargin().bottom,
            maxSize.cy);
    }
...
Box::Box(const Box& r)
    ...
    , m_items()
    , OnBubbledEvent() {
    m_pLayout.reset(new Layout(*r.m_pLayout)), m_pLayout->SetOwner(this);
}
...
UIAControlProvider* Box::GetUIAProvider() {
    ...
    if (m_pUIAProvider == nullptr) {
        m_pUIAProvider = static_cast<UIAControlProvider*>(
            new (std::nothrow) UIABoxProvider(this));
    }
...
void Box::SetAttribute(const std::wstring& strName,
                       const std::wstring& strValue) {
...
CSize Box::EstimateSize(CSize szAvailable) {
    ...
    if (GetFixedWidth() == DUI_LENGTH_AUTO ||
        ...
        szAvailable.cx -=
            m_pLayout->GetPadding().left + m_pLayout->GetPadding().right;
        szAvailable.cy -=
            m_pLayout->GetPadding().top + m_pLayout->GetPadding().bottom;
...
Control* Box::FindControl(FINDCONTROLPROC Proc,
                          LPVOID pData,
                          UINT uFlags,
                          CPoint scrollPos) {
...
Control* Box::FindSubControl(const std::wstring& pstrSubControlName) {
    ...
    pSubControl = static_cast<Control*>(
        GetWindow()->FindSubControlByName(this, pstrSubControlName));
    return pSubControl;
}
...
ScrollableBox::ScrollableBox(Layout* pLayout)
    ...
    m_rcScrollBarPadding.left = m_rcScrollBarPadding.top =
        m_rcScrollBarPadding.right = m_rcScrollBarPadding.bottom = 0;
}
...
UIAControlProvider* ScrollableBox::GetUIAProvider() {
    ...
    if (m_pUIAProvider == nullptr) {
        m_pUIAProvider = static_cast<UIAControlProvider*>(
            new (std::nothrow) UIAScrollableBoxProvider(this));
    }
    return m_pUIAProvider;
#else
    return nullptr;
...
void ScrollableBox::SetAttribute(const std::wstring& pstrName,
                                 const std::wstring& pstrValue) {
    if (pstrName == _T("vscrollbar")) {
        EnableScrollBar(pstrValue == _T("true"),
    ...
    else if (pstrName == _T("hscrollunit"))
        SetHorScrollUnitPixels(_ttoi(pstrValue.c_str()));
    else if (pstrName == _T("scrollbarfloat"))
        SetScrollBarFloat(pstrValue == _T("true"));
    else if (pstrName == _T("vscrollbarleft"))
        SetVScrollBarLeftPos(pstrValue == _T("true"));
...
void ScrollableBox::SetPos(UiRect rc) {
    bool bEndDown = false;
    if (IsHoldEnd() && IsVScrollBarValid() &&
        GetScrollRange().cy - GetScrollPos().cy == 0) {
        bEndDown = true;
    }
    SetPosInternally(rc);
...
CSize ScrollableBox::CalcRequiredSize(const UiRect& rc) {
    ...
    if (m_items.size() == 0) {
        ...
        UiRect childSize = rc;
        if (!m_bScrollBarFloat && m_pVerticalScrollBar &&
        ...
        if (!m_bScrollBarFloat && m_pHorizontalScrollBar &&
            m_pHorizontalScrollBar->IsValid()) {
...
bool ScrollableBox::MouseEnter(EventArgs& msg) {
    ...
    if (bRet && m_pVerticalScrollBar != NULL &&
        m_pVerticalScrollBar->IsValid() && m_pVerticalScrollBar->IsEnabled()) {
        if (m_pVerticalScrollBar->IsAutoHideScroll()) {
            m_pVerticalScrollBar->SetVisible(true);
        }
    }
    if (bRet && m_pHorizontalScrollBar != NULL &&
        m_pHorizontalScrollBar->IsValid() &&
        m_pHorizontalScrollBar->IsEnabled()) {
...
Control* ScrollableBox::FindControl(FINDCONTROLPROC Proc,
                                    LPVOID pData,
                                    UINT uFlags,
                                    CPoint scrollPos) {
    // Check if this guy is valid
    ...
    if ((uFlags & UIFIND_HITTEST) != 0) {
        ...
        if (!m_bMouseChildEnabled) {
            ...
            if (m_pVerticalScrollBar != NULL)
                pResult =
                    m_pVerticalScrollBar->FindControl(Proc, pData, uFlags);
            if (pResult == NULL && m_pHorizontalScrollBar != NULL)
                pResult =
                    m_pHorizontalScrollBar->FindControl(Proc, pData, uFlags);
            if (pResult == NULL)
                pResult = Control::FindControl(Proc, pData, uFlags);
            return pResult;
        }
...
void ScrollableBox::SetScrollPos(CSize szPos) {
    ...
    Invalidate();
    if (m_pWindow != NULL) {
        m_pWindow->SendNotify(this, kEventScrollChange, (cy == 0) ? 0 : 1,
                              (cx == 0) ? 0 : 1);
    }
}
...
void ScrollableBox::LineUp(int deltaValue, bool withAnimation) {
    ...
    if (!withAnimation) {
        ...
        if (m_scrollAnimation.IsPlaying()) {
            if (m_scrollAnimation.GetEndValue() >
                m_scrollAnimation.GetStartValue()) {
                m_scrollAnimation.SetEndValue(scrollPos.cy - cyLine);
            } else {
                m_scrollAnimation.SetEndValue(m_scrollAnimation.GetEndValue() -
                                              cyLine);
            }
        } else {
            m_scrollAnimation.SetEndValue(scrollPos.cy - cyLine);
        ...
        m_scrollAnimation.SetCallback(nbase::Bind(&ScrollableBox::SetScrollPosY,
...
void ScrollableBox::LineLeft(int detaValue) {
    ...
    m_scrollAnimation.SetSpeedDownRatio(0.5);
    m_scrollAnimation.SetTotalMillSeconds(DUI_NOSET_VALUE);
    m_scrollAnimation.SetCallback(nbase::Bind(&ScrollableBox::SetScrollPosX,
    this, std::placeholders::_1)); m_scrollAnimation.Start();*/
}
...
void ScrollableBox::PageUp() {
    ...
    int iOffset = m_rcItem.bottom - m_rcItem.top - m_pLayout->GetPadding().top -
...
void ScrollableBox::EndDown(bool arrange, bool withAnimation) {
    ...
    int renderOffsetY =
        GetScrollRange().cy - GetScrollPos().cy +
        (m_renderOffsetYAnimation.GetEndValue() - GetRenderOffset().y);
...
void ScrollableBox::PageLeft() {
    CSize sz = GetScrollPos();
    int iOffset = m_rcItem.right - m_rcItem.left -
                  m_pLayout->GetPadding().left - m_pLayout->GetPadding().right;
    // if( m_pVerticalScrollBar && m_pVerticalScrollBar->IsValid() ) iOffset -=
...
void ScrollableBox::EnableScrollBar(bool bEnableVertical,
    ...
    if (cyScroll <= 0 && !m_bScrollProcess) {
        ...
        if (m_bVScrollBarLeftPos) {
            UiRect rcVerScrollBarPos(
                rcScrollBarPos.left, rcScrollBarPos.top,
                rcScrollBarPos.left + m_pVerticalScrollBar->GetFixedWidth(),
            ...
            UiRect rcVerScrollBarPos(
                ...
                rcScrollBarPos.top, rcScrollBarPos.right,
                rcScrollBarPos.bottom);
            m_pVerticalScrollBar->SetPos(rcVerScrollBarPos);
        }
...
void ScrollableBox::ProcessHScrollBar(UiRect rc, int cxRequired) {
    ...
    if (cxScroll <= 0 && !m_bScrollProcess) {
        ...
        UiRect rcVerScrollBarPos(
            ...
            rcScrollBarPos.right, rcScrollBarPos.bottom);
...
void ScrollableBox::PlayRenderOffsetYAnimation(int nRenderY) {
    ...
    std::function<void(int)> playCallback = nbase::Bind(
        &ScrollableBox::SetRenderOffsetY, this, std::placeholders::_1);
