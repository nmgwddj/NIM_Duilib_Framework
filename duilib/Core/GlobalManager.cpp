...
std::wstring GetDpiImageFullPath(const std::wstring& strImageFullPath,
    ...
    std::list<std::wstring> strPathList =
        StringHelper::Split(strImageFullPath, L"\\");
    ...
    std::wstring strFileExtension =
        strPathFileName.substr(iPointPos, strPathFileName.length() - iPointPos);
    std::wstring strFile = strPathFileName.substr(0, iPointPos);
    strPathFileName = StringHelper::Printf(L"%s%s%d%s", strFile.c_str(), L"@",
                                           dpi, strFileExtension.c_str());
    ...
    return file_attr != INVALID_FILE_ATTRIBUTES ? strNewFilePath
                                                : strImageFullPath;
}
...
std::map<std::wstring, std::unique_ptr<WindowBuilder>>
    GlobalManager::m_builderMap;
...
std::wstring GlobalManager::m_strDefaultDisabledColor =
    L"textdefaultdisablecolor";
...
void GlobalManager::Startup(const std::wstring& strResourcePath,
    ...
    if (g_hzip) {
        HGLOBAL hGlobal =
            GetZipData(GetLanguagePath() + L"\\" + kLanguageFileName);
        if (hGlobal) {
            ui::MutiLanSupport::GetInstance()->LoadStringTable(hGlobal);
            GlobalFree(hGlobal);
        }
    } else {
        MutiLanSupport::GetInstance()->LoadStringTable(
            GetLanguagePath() + L"\\" + kLanguageFileName);
    }
...
void GlobalManager::LoadGlobalResource() {
    ...
    dialog_builder.Create(L"global.xml", CreateControlCallback(),
                          &paint_manager);
}
...
void GlobalManager::ReloadLanguage(const std::wstring& languagePath,
                                   bool invalidateAll) {
    if (GetLanguagePath() != languagePath) {
        SetLanguagePath(languagePath);

        MutiLanSupport::GetInstance()->LoadStringTable(languagePath + L"\\" +
                                                       kLanguageFileName);

        if (invalidateAll) {
            for (auto it = m_aPreMessages.begin(); it != m_aPreMessages.end();
                 it++) {
                (*it)->GetRoot()->Invalidate();
            }
        }
...
std::unique_ptr<ui::IPen> GlobalManager::CreatePen(DWORD color,
                                                   int width /*= 1*/) {
...
void GlobalManager::AddClass(const std::wstring& strClassName,
...
std::wstring GlobalManager::GetClassAttributes(
    const std::wstring& strClassName) {
...
void GlobalManager::AddTextColor(const std::wstring& strName,
...
std::shared_ptr<ImageInfo> GlobalManager::IsImageCached(
...
std::shared_ptr<ImageInfo> GlobalManager::AddImageCached(
    const std::shared_ptr<ImageInfo>& sharedImage) {
...
HFONT GlobalManager::AddFont(const std::wstring& strFontId,
...
HFONT GlobalManager::GetFont(const std::wstring& strFontName,
                             int nSize,
                             bool bBold,
                             bool bUnderline,
                             bool bStrikeout,
...
TFontInfo* GlobalManager::GetFontInfo(const std::wstring& strFontId,
                                      HDC hDcPaint) {
    TFontInfo* pFontInfo = GetTFontInfo(strFontId);
...
TFontInfo* GlobalManager::GetFontInfo(HFONT hFont, HDC hDcPaint) {
    for (auto it = m_mCustomFonts.begin(); it != m_mCustomFonts.end(); it++) {
        ...
        if (pFontInfo->hFont == hFont) {
            if (pFontInfo->tm.tmHeight == 0) {
                HFONT hOldFont =
                    (HFONT)::SelectObject(hDcPaint, pFontInfo->hFont);
                ::GetTextMetrics(hDcPaint, &pFontInfo->tm);
                ::SelectObject(hDcPaint, hOldFont);
            }
            return pFontInfo;
...
bool GlobalManager::FindFont(const std::wstring& strFontName,
                             int nSize,
                             bool bBold,
                             bool bUnderline,
                             bool bStrikeout,
...
Box* GlobalManager::CreateBox(const std::wstring& strXmlPath,
                              CreateControlCallback callback) {
...
Box* GlobalManager::CreateBoxWithCache(const std::wstring& strXmlPath,
                                       CreateControlCallback callback) {
    Box* box = nullptr;
...
void GlobalManager::FillBox(Box* pUserDefinedBox,
                            const std::wstring& strXmlPath,
                            CreateControlCallback callback) {
    WindowBuilder winBuilder;
    Box* box = winBuilder.Create(strXmlPath.c_str(), callback,
                                 pUserDefinedBox->GetWindow(), nullptr,
                                 pUserDefinedBox);
    ASSERT(box);
...
void GlobalManager::FillBoxWithCache(Box* pUserDefinedBox,
                                     const std::wstring& strXmlPath,
                                     CreateControlCallback callback) {
    Box* box = nullptr;
    auto it = m_builderMap.find(strXmlPath);
    if (it == m_builderMap.end()) {
        ...
        box = winBuilder->Create(strXmlPath.c_str(), callback,
                                 pUserDefinedBox->GetWindow(), nullptr,
                                 pUserDefinedBox);
        if (box) {
        ...
        box = it->second->Create(callback, pUserDefinedBox->GetWindow(),
...
bool GlobalManager::OpenResZip(LPCTSTR resource_name,
...
bool GlobalManager::OpenResZip(const std::wstring& path,
                               const std::string& password) {
    if (g_hzip) {
        CloseZip(g_hzip);
        g_hzip = NULL;
...
HGLOBAL GlobalManager::GetZipData(const std::wstring& path) {
    ...
    if (g_hzip && !file_path.empty()) {
        ...
        if (FindZipItem(g_hzip, file_path.c_str(), true, &i, &ze) == ZR_OK) {
            if (ze.index >= 0) {
                hGlobal =
                    GlobalAlloc(GMEM_MOVEABLE | GMEM_NODISCARD, ze.unc_size);
                if (hGlobal) {
                    TCHAR* pData = (TCHAR*)GlobalLock(hGlobal);
                    if (pData) {
                        ZRESULT res =
                            UnzipItem(g_hzip, ze.index, pData, ze.unc_size);
...
std::wstring GlobalManager::GetZipFilePath(const std::wstring& path) {
    ...
    for (unsigned int i = 0; i < file_path.size();) {
        ...
        if (start_node && wch == L'.') {
            if (i + 1 < file_path.size() &&
                file_path.at(i + 1) == L'/')  // "./"
            {
                file_path.erase(i, 2);
                continue;
            } else if (i + 2 < file_path.size() &&
                       file_path.at(i + 1) == L'.' &&
                       file_path.at(i + 2) == L'/')  // "../"
            {
                file_path.erase(i, 2);
...
std::wstring GlobalManager::GetResPath(const std::wstring& res_path,
                                       const std::wstring& window_res_path) {
    std::wstring imageFullPath = res_path;
    if (!::PathIsRelative(res_path.c_str()))
        return res_path;

    imageFullPath =
        GlobalManager::GetResourcePath() + window_res_path + res_path;
    imageFullPath = StringHelper::ReparsePath(imageFullPath);

    if (!GlobalManager::IsZipResExist(imageFullPath) &&
        !::PathFileExists(imageFullPath.c_str())) {
...
bool GlobalManager::IsZipResExist(const std::wstring& path) {
    ...
    if (g_hzip && !path.empty()) {
        ...
        int i = 0;
        bool find =
            FindZipItem(g_hzip, file_path.c_str(), true, &i, &ze) == ZR_OK;
        if (find)
...
bool GlobalManager::ImageCacheKeyCompare::operator()(
    ...
    // 逆向比较
    while (--pStr1End >= pStr1Begin && --pStr2End >= pStr2Begin &&
           *pStr1End == *pStr2End)
        ;
