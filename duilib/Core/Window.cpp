...
void Window::Close(UINT nRet) {
    ...
    if (m_pRoot && IsWindowsVistaOrGreater()) {
        ...
        TimerManager::GetInstance()->AddCancelableTimer(
            m_closeFlag.GetWeakFlag(), closeCallback, 300, 1);
    } else {
...
void Window::ShowWindow(bool bShow /*= true*/, bool bTakeFocus /*= false*/) {
    ...
    ::ShowWindow(m_hWnd, bShow
                             ? (bTakeFocus ? SW_SHOWNORMAL : SW_SHOWNOACTIVATE)
                             : SW_HIDE);
}
...
LRESULT CALLBACK Window::__WndProc(HWND hWnd,
    ...
    if (uMsg == WM_NCCREATE) {
        ...
        if (uMsg == WM_NCDESTROY && pThis != NULL) {
            LRESULT lRes = ::CallWindowProc(pThis->m_OldWndProc, hWnd, uMsg,
                                            wParam, lParam);
...
void Window::SetWindowResourcePath(const std::wstring& strPath) {
    ...
    if (m_strWindowResourcePath.empty())
        return;
    TCHAR cEnd =
        m_strWindowResourcePath.at(m_strWindowResourcePath.length() - 1);
    if (cEnd != _T('\\') && cEnd != _T('/'))
...
TFontInfo* Window::GetDefaultFontInfo() {
    if (m_defaultFontInfo.tm.tmHeight == 0) {
        HFONT hOldFont =
            (HFONT)::SelectObject(m_hDcPaint, m_defaultFontInfo.hFont);
...
void Window::AddTextColor(const std::wstring& strName,
...
void Window::SetTextId(const std::wstring& strTextId) {
    ::SetWindowText(
        m_hWnd,
        MutiLanSupport::GetInstance()->GetStringViaID(strTextId).c_str());
}
...
bool Window::RemoveControlFromPointFinder(IControlFromPointFinder* pFinder) {
    auto it = std::find_if(m_aIControlFromPointFinder.begin(),
                           m_aIControlFromPointFinder.end(),
                           [&](IControlFromPointFinder* item) {
                               return pFinder == item;
                           });
...
LRESULT Window::DoHandlMessage(UINT uMsg,
    ...
    switch (uMsg) {
        case WM_APP + 1: {
            for (auto it = m_aDelayedCleanup.begin();
                 it != m_aDelayedCleanup.end(); it++)
                delete *it;
        ...
        case WM_MOUSEHOVER: {
            ...
            if (m_pEventHover != NULL) {
                m_pEventHover->HandleMessageTemplate(kEventMouseHover, 0, 0, 0,
                                                     pt);
            }
            ...
            if (m_hwndTooltip != NULL && IsWindowVisible(m_hwndTooltip)) {
                ...
                toolTip.lpszText =
                    const_cast<LPTSTR>((LPCTSTR)toolTipText.c_str());
                ::SendMessage(m_hwndTooltip, TTM_GETTOOLINFO, 0,
                              (LPARAM)&toolTip);
                if (pHover == m_pEventHover &&
                    sToolTip == std::wstring(toolTipText.c_str()))
                    break;
            }
            ...
            if (m_hwndTooltip == NULL) {
                m_hwndTooltip = ::CreateWindowEx(
                    ...
                    CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, m_hWnd, NULL,
            ...
            if (!::IsWindowVisible(m_hwndTooltip)) {
                ...
                ::SendMessage(m_hwndTooltip, TTM_SETTOOLINFO, 0,
                              (LPARAM)&m_ToolTip);
                ::SendMessage(m_hwndTooltip, TTM_TRACKACTIVATE, TRUE,
                              (LPARAM)&m_ToolTip);
            }
        ...
        case WM_MOUSEMOVE: {
            ...
            // Do not move the focus to the new control when the mouse is
            // pressed
            if (!IsCaptured())
            ...
            if (m_pEventClick != NULL) {
                m_pEventClick->HandleMessageTemplate(kEventMouseMove, wParam,
                                                     lParam, 0, pt);
            } else if (m_pNewHover != NULL) {
                m_pNewHover->HandleMessageTemplate(kEventMouseMove, wParam,
                                                   lParam, 0, pt);
            }
        } break;
        case WM_LBUTTONDOWN: {
            ...
            pControl->HandleMessageTemplate(kEventMouseButtonDown, wParam,
                                            lParam, 0, pt);
        } break;
        case WM_RBUTTONDOWN: {
            if (m_pEventPointer != NULL)
        ...
        case WM_LBUTTONDBLCLK: {
            ...
            pControl->HandleMessageTemplate(kEventInternalDoubleClick, wParam,
                                            lParam, 0, pt);
        } break;
        case WM_LBUTTONUP: {
            ...
            m_pEventClick->HandleMessageTemplate(kEventMouseButtonUp, wParam,
                                                 lParam, 0, pt);
            m_pEventClick = NULL;
        } break;
        case WM_RBUTTONUP: {
            ...
            m_pEventClick->HandleMessageTemplate(kEventMouseRightButtonUp,
                                                 wParam, lParam, 0, pt);
        ...
        case WM_IME_STARTCOMPOSITION: {
            if (m_pFocus == NULL)
                break;
            m_pFocus->HandleMessageTemplate(kEventImeStartComposition, wParam,
                                            lParam, wParam);
        } break;
        case WM_IME_ENDCOMPOSITION: {
            if (m_pFocus == NULL)
                break;
            m_pFocus->HandleMessageTemplate(kEventImeEndComposition, wParam,
                                            lParam, wParam);
        } break;
        case WM_MOUSEWHEEL: {
            POINT pt = {GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)};
            ...
            pControl->HandleMessageTemplate(kEventMouseScrollWheel, zDelta,
                                            lParam);
        } break;
        case WM_TOUCH: {
            ...
            // 只关心第一个触摸位置
            if (nNumInputs >= 1 &&
                GetTouchInputInfoWrapper((HTOUCHINPUT)lParam, nNumInputs,
                ...
                if (pInputs[0].dwID != 0) {
                    ...
                    if (pInputs[0].dwFlags & TOUCHEVENTF_DOWN) {
                        ...
                        pControl->HandleMessageTemplate(kEventTouchDown, 0,
                                                        lParam, 0, pt);
                    } else if (pInputs[0].dwFlags & TOUCHEVENTF_MOVE) {
                        if (m_pEventClick != NULL)
                            break;

                        if (m_ptLastMousePos.x == pt.x &&
                        ...
                        m_pEventPointer->HandleMessageTemplate(kEventTouchMove,
                                                               0, 0, 0, pt);
                        ...
                        m_pEventPointer->HandleMessageTemplate(kEventTouchUp, 0,
                                                               lParam, 0, pt);
                        m_pEventPointer = NULL;
                    }
                }
        ...
        case WM_POINTERCAPTURECHANGED: {
            ...
            switch (uMsg) {
                case WM_POINTERDOWN: {
                    ...
                    pControl->HandleMessageTemplate(kEventPointDown, 0, lParam,
                                                    0, pt, pressure);

                    // 如果控件不支持处理WM_POINTERUPDATE消息，则不设置handled，程序会进入WM_BUTTON处理流程
                    if (m_pEventPointer &&
                        m_pEventPointer->IsReceivePointerMsg()) {
                        handled = true;
                    }
                } break;
                case WM_POINTERUPDATE:
                    ...
                    m_pEventPointer->HandleMessageTemplate(kEventPointMove, 0,
                                                           0, 0, pt, pressure);
                ...
                case WM_POINTERCAPTURECHANGED:
                    ...
                    if (uMsg != WM_POINTERLEAVE) {
                        // Refer to LBUTTONUP and MOUSELEAVE，LBUTTOUP
                        // ReleaseCapture while MOUSELEAVE DONOT ReleaseCapture
                        ReleaseCapture();
                    }
                    if (m_pEventPointer == NULL)
                        break;

                    m_pEventPointer->HandleMessageTemplate(
                        kEventPointUp, 0, lParam, 0, pt, pressure);
        ...
        case WM_KILLFOCUS: {
            ...
            pControl->HandleMessageTemplate(kEventMouseButtonUp, wParam, lParam,
                                            0, pt);
            m_pEventClick = NULL;
            m_pEventPointer = NULL;
        } break;
        ...
        case WM_KEYDOWN: {
            ...
            m_pFocus->HandleMessageTemplate(kEventKeyDown, wParam, lParam,
                                            wParam);
        } break;
        case WM_KEYUP: {
            if (m_pEventKey == NULL)
            ...
            m_pEventKey->HandleMessageTemplate(kEventKeyUp, wParam, lParam,
                                               wParam);
            m_pEventKey = NULL;
        } break;
        case WM_SETCURSOR: {
            ...
            if (pControl == NULL)
                break;
            pControl->HandleMessageTemplate(kEventSetCursor, wParam, lParam, 0,
                                            pt);
        }
            handled = true;
            return 0;
        case WM_NOTIFY: {
            ...
            if (lpNMHDR != NULL)
                return ::SendMessage(lpNMHDR->hwndFrom, OCM__BASE + uMsg,
                                     wParam, lParam);
            handled = true;
            return 0;
        } break;
        ...
        case WM_CTLCOLORSTATIC: {
            ...
            // WM_CTLCOLOREDIT message; instead, they send the WM_CTLCOLORSTATIC
            // message.
            if (lParam == 0)
                break;
            HWND hWndChild = (HWND)lParam;
...
#if defined(ENABLE_UIAUTOMATION)
        case WM_GETOBJECT: {
            if (static_cast<long>(lParam) ==
                    static_cast<long>(UiaRootObjectId) &&
                GlobalManager::IsAutomationEnabled()) {
                handled = TRUE;

                return UiaReturnRawElementProvider(m_hWnd, wParam, lParam,
                                                   GetUIAProvider());
            }
        } break;
#endif
...
bool Window::HandleMouseEnterLeave(const POINT& pt,
    ...
    if (m_pNewHover != m_pEventHover && m_pNewHover != NULL) {
        m_pNewHover->HandleMessageTemplate(kEventMouseEnter, wParam, lParam, 0,
                                           pt);
...
void Window::ReleaseEventClick(bool bClickOrPointer,
                               WPARAM wParam,
                               LPARAM lParam) {
    if (bClickOrPointer) {
        if (m_pEventClick) {
            m_pEventClick->HandleMessageTemplate(kEventMouseButtonUp, wParam,
                                                 lParam, 0, m_ptLastMousePos);
            m_pEventClick = NULL;
        }
    } else {
        if (m_pEventPointer) {
...
std::vector<Control*>* Window::FindSubControlsByClass(
    ...
    ASSERT(pParent);
    m_aFoundControls.clear();
    pParent->FindControl(__FindControlsFromClass, (LPVOID)&typeinfo,
                         UIFIND_ALL);
...
void Window::Paint() {
    ...
    if (m_bIsArranged && m_pRoot->IsArranged() &&
        ...
        ::MoveWindow(m_hWnd, rect.left, rect.top, needSize.cx, needSize.cy,
                     TRUE);
    }
    ...
    if (m_bIsLayeredWindow) {
        ...
        ::UpdateLayeredWindow(m_hWnd, NULL, &pt, &szWindow,
                              m_renderContext->GetDC(), &ptSrc, 0, &bf,
                              ULW_ALPHA);
    } else {
