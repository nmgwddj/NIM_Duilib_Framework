...
public:
     ...
     * @return 返回 true 为移除成功，false
     * 为移除失败（索引值太小或超出了子控件总数）
     */
    ...
    /**
     * @brief 设置容器响应鼠标操作
     * @param[in] bEnable 设置为 true 为响应鼠标操作，设置为 false
     * 为不响应，默认为 true
     * @return 无
     */
     ...
     * @return void 无
     */
    void AttachBubbledEvent(EventType eventType,
...
private:
     ...
     * @return void 无
     */
    void AttachXmlBubbledEvent(EventType eventType,
                               const EventCallback& callback) {
...
public:
     ...
     * @param[in] bScrollBarFloat true 表示悬浮在滚动条上，false
     * 表示不悬浮在控件上
     * @return 无
     */
