...
void PlaceHolder::ArrangeAncestor() {
    ...
    if (!m_pWindow || !m_pWindow->GetRoot()) {
        ...
        while (parent && (parent->GetFixedWidth() == DUI_LENGTH_AUTO ||
                          parent->GetFixedHeight() == DUI_LENGTH_AUTO)) {
...
CPoint PlaceHolder::GetScrollOffset() const {
    ...
    if (lbParent &&
        (lbParent->IsVScrollBarValid() || lbParent->IsHScrollBarValid()) &&
        IsFloat()) {
        return scrollPos;
    }
    ...
    while (parent) {
        while (parent &&
               (!dynamic_cast<ScrollableBox*>(parent) ||
                (!dynamic_cast<ScrollableBox*>(parent)->IsVScrollBarValid() &&
                 !dynamic_cast<ScrollableBox*>(parent)->IsHScrollBarValid()))) {
            parent = parent->GetParent();
        }
