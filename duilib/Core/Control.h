...
public:
     ...
     * @return 无
     */
    void SetStateColor(ControlStateType stateType,
     ...
     * @return 无
     */
    void SetStateImage(ControlStateType stateType,
     ...
     * @return 无
     */
    void SetForeStateImage(ControlStateType stateType,
                           const std::wstring& strImage);
    ...
    /**
     * @brief 设置边框颜色
     * @param[in] strBorderColor 设置边框的颜色字符串值，该值必须在 global.xml
     * 中存在
     * @return 无
     */
    ...
    /**
     * @brief 设置鼠标悬浮到控件显示的提示文本在语言文件中对应的文字（UTF8
     * 格式）
     * @param[in] strTextId 在语言文件中对应的提示文字 ID
     * @return 无
     */
    ...
    /**
     * @brief 控件是否响应右键菜单消息
     * @return 返回结果表示了是否响应右键菜单消息，true 响应右键菜单消息，false
     * 不响应右键菜单消息
     */
     ...
     * @return 待补充
     */
    virtual void SetInternVisible(
        bool bVisible =
            true);  // 仅供内部调用，有些ui拥有窗口句柄，需要重写此函数
    ...
    /**
     * @brief 检查控件是否响应键盘事件
     * @return 返回控件是否响应键盘事件，返回 true 响应键盘事件，false
     * 不响应键盘事件
     */
    ...
    /**
     * @brief 检查控件是否具有焦点
     * @return 返回控件是否具有检点，为 true 时是当前具有焦点，为 false
     * 时控件没有焦点
     */
     ...
     * @return 返回鼠标焦点是否在控件上，true 鼠标焦点在控件上，false
     * 鼠标焦点不在控件上
     */
    ...
    /**
     * @brief 设置是否将鼠标焦点到控件上
     * @param[in] bMouseFocused 为 true 时设置鼠标焦点到控件上，为 false
     * 时让控件失去鼠标焦点
     * @return 无
     */
    virtual void SetMouseFocused(bool bMouseFocused) {
        m_bMouseFocused = bMouseFocused;
    }

    /**
     * @brief 判断控件当前是否是激活状态
     * @return 返回控件状态，true 控件当前是激活状态，可见并可用，false
     * 控件当前非激活状态，可能不可见或被禁用
     */
     ...
     * @return 返回控件的指针
     */
    virtual Control* FindControl(FINDCONTROLPROC Proc,
                                 LPVOID pData,
                                 UINT uFlags,
                                 CPoint scrollPos = CPoint());

    /// 位置相关
    /**
     * @brief 获取控件位置
     * @param[in] bContainShadow 是否包含阴影，默认为 true 则包含阴影范围，否则
     * false 为不包含
     * @return 返回控件的位置信息
     */
     ...
     * @param[in] bNeedDpiScale 是否让外边距根据 DPI 适配，默认为 true，false
     * 不适配 DPI
     * @return 无
     */
     ...
     * @return szAvailable 控件实际大小（如果设置了图片并设置 width 或 height
     * 任意一项为 auto，将根据图片来计算最终大小）
     */
     ...
     * @param[in] point 具体坐标
     * @return 返回是否在范围内，true 在滚动条当前滚动位置范围内，false
     * 不在滚动条当前滚动位置范围内
     */
     ...
     * @return 无
     */
    void HandleMessageTemplate(EventType eventType,
                               WPARAM wParam = 0,
                               LPARAM lParam = 0,
                               TCHAR tChar = 0,
                               CPoint mousePos = CPoint(),
                               FLOAT pressure = 0.0f);
     ...
     * @return 无
     */
    virtual void SetReceivePointerMsg(bool bRecv) {
        m_bReceivePointerMsg = bRecv;
    };
    ...
    void SetNeedButtonUpWhenKillFocus(bool bNeed) {
        m_bNeedButtonUpWhenKillFocus = bNeed;
    };
    ...
    bool IsNeedButtonUpWhenKillFocus() const {
        return m_bNeedButtonUpWhenKillFocus;
    };
     ...
     * @return 待补充
     */
    bool OnApplyAttributeList(const std::wstring& strReceiver,
    ...
    /**
     * @brief 根据图片路径缓存图片信息
     * @param[in，out] duiImage
     * 传入时标注图片的路径信息，如果成功则会缓存图片并记录到该参数的成员中
     * @return 无
     */
     ...
     */
    bool DrawImage(IRenderContext* pRender,
                   Image& duiImage,
    ...
    /**
     * @brief 播放 GIF
     * @param[in] 播放完成停止在哪一帧，可设置第一帧、当前帧和最后一帧。请参考
     * GifStopType 枚举
     * @return 无
     */
    void StartGifPlayForUI(GifStopType frame = kGifStopFirst,
                           int playcount = -1);

    /**
     * @brief 停止播放 GIF
     * @param[in] transfer 是否将停止事件通知给订阅者，参考 AttachGifPlayStop
     * 方法
     * @param[frame] frame
     * 播放结束停止在哪一帧，可设置第一帧、当前帧和最后一帧。请参考 GifStopType
     * 枚举
     * @return 无
     */
    void StopGifPlayForUI(bool transfer = false,
                          GifStopType frame = kGifStopCurrent);
    ...
    void AttachGifPlayStop(const EventCallback& callback) {
        OnGifEvent[m_nVirtualEventGifStop] += callback;
    };
     ...
     * @brief 关闭loading状态
     * @param[in] frame
     * 播放完成停止在哪一帧，可设置第一帧、当前帧和最后一帧。请参考 GifStopType
     * 枚举
     * @return 无
     */
     ...
     * @return 无
     */
    void AttachLastEvent(const EventCallback& callback) {
        OnEvent[kEventLast] += callback;
    }
     ...
     * @return 无
     */
    void AttachVisibleChange(const EventCallback& callback) {
        OnEvent[kEventVisibleChange] += callback;
    }
