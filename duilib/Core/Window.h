...
#define UIFLAG_TABSTOP \
    0x00000001  // 标识控件是否在收到 TAB 切换焦点时允许设置焦点
...
public:
     ...
     * @return 无
     */
    void AddClass(const std::wstring& strClassName,
     ...
     * @return 无
     */
    void AddTextColor(const std::wstring& strName,
    ...
    /**
     * @brief 获取透明通道修补范围的的九宫格描述，对应 XML 中 alphafixcorner
     * 属性
     * @return 返回修补范围
     */
    ...
    /**
     * @brief 获取窗口的初始高度占屏幕高度的百分比，对应 XML 中 heightpercent
     * 属性
     * @return 返回指定百分比数据
     */
     ...
     * @return 无
     */
    void SetPos(const UiRect& rc,
                bool bNeedDpiScale,
                UINT uFlags,
                HWND hWndInsertAfter = NULL,
                bool bContainShadow = false);
     ...
     * @param[in] pFilter 一个继承了 IUIMessageFilter 的对象实例，需要实现
     * MessageHandler 方法
     * @return 始终返回 true
     */
     ...
     * @param[in] pFinder 继承了 IControlFromPointFinder 对象指针，需实现
     * FindControlFromPoint 方法
     * @return 始终返回 true
     */
     ...
     * @param[in] pTranslateAccelerator 继承了 ITranslateAccelerator
     * 对象指针，需实现 TranslateAccelerator 方法
     * @return 始终返回 true
     */
     ...
     */
    bool RemoveTranslateAccelerator(
     ...
     * @param[in] wParam 消息附加参数
     * @param[in] lParam 消息附加参数
     * @param[out] handled 消息是否已经处理，返回 true
     * 表明已经成功处理消息，否则将消息继续传递给窗口过程
     * @return 返回消息处理结果
     */
    LRESULT DoHandlMessage(UINT uMsg,
                           WPARAM wParam,
                           LPARAM lParam,
                           bool& handled);
     ...
     */
    inline bool HandleMouseEnterLeave(const POINT& pt,
                                      WPARAM wParam,
                                      LPARAM lParam);
     ...
     * @return 返回控件指针
     */
    Control* FindSubControlByName(Control* pParent,
     ...
     * @return 控件指针
     */
    Control* FindSubControlByClass(Control* pParent,
     ...
     */
    std::vector<Control*>* FindSubControlsByClass(Control* pParent,
                                                  const type_info& typeinfo);
     ...
     * @return 始终返回 true
     */
    bool SendNotify(Control* pControl,
                    EventType msg,
                    WPARAM wParam = 0,
                    LPARAM lParam = 0);
...
private:
    static Control* CALLBACK __FindControlFromNameHash(Control* pThis,
                                                       LPVOID pData);
    static Control* CALLBACK __FindControlFromCount(Control* pThis,
                                                    LPVOID pData);
    static Control* CALLBACK __FindControlFromPoint(Control* pThis,
                                                    LPVOID pData);
    static Control* CALLBACK __FindControlFromTab(Control* pThis, LPVOID pData);
    // static Control* CALLBACK __FindControlFromShortcut(Control* pThis, LPVOID
    // pData);
    static Control* CALLBACK __FindControlFromUpdate(Control* pThis,
                                                     LPVOID pData);
    static Control* CALLBACK __FindControlFromName(Control* pThis,
                                                   LPVOID pData);
    static Control* CALLBACK __FindControlFromClass(Control* pThis,
                                                    LPVOID pData);
    static Control* CALLBACK __FindControlsFromClass(Control* pThis,
                                                     LPVOID pData);
...
protected:
    ...
    std::wstring
        m_strWindowResourcePath;  //每个窗口的资源路径,等于GetSkinFolder()
    ...
    bool m_bCloseing =
        false;  // add by djj 20200428 调用Close时会延迟Post WM_CLOSE,
                // 这个期间需要有一个标识此种'待关闭状态'
};
