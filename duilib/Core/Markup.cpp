...
CMarkupNode CMarkupNode::GetChild(LPCTSTR pstrName) {
    ...
    ULONG iPos = m_pOwner->m_pElements[m_iPos].iChild;
    while (iPos != 0) {
        if (_tcscmp(m_pOwner->m_pstrXML + m_pOwner->m_pElements[iPos].iStart,
                    pstrName) == 0) {
            return CMarkupNode(m_pOwner, iPos);
        }
...
LPCTSTR CMarkupNode::GetAttributeValue(LPCTSTR pstrName) {
    ...
    for (int i = 0; i < m_nAttributes; i++) {
        if (_tcscmp(m_pOwner->m_pstrXML + m_aAttributes[i].iName, pstrName) ==
            0)
...
bool CMarkupNode::GetAttributeValue(int iIndex,
                                    LPTSTR pstrValue,
                                    SIZE_T cchMax) {
    if (m_pOwner == NULL)
        return false;
    ...
    if (iIndex < 0 || iIndex >= m_nAttributes)
        return false;
    _tcsncpy(pstrValue, m_pOwner->m_pstrXML + m_aAttributes[iIndex].iValue,
             cchMax);
    return true;
}
...
bool CMarkupNode::GetAttributeValue(LPCTSTR pstrName,
    ...
    for (int i = 0; i < m_nAttributes; i++) {
        if (_tcscmp(m_pOwner->m_pstrXML + m_aAttributes[i].iName, pstrName) ==
            0) {
            _tcsncpy(pstrValue, m_pOwner->m_pstrXML + m_aAttributes[i].iValue,
                     cchMax);
            return true;
        }
    }
...
bool CMarkup::LoadFromMem(BYTE* pByte, DWORD dwSize, int encoding) {
    ...
    if (encoding == XMLFILE_ENCODING_UTF8) {
        ...
        ::MultiByteToWideChar(CP_UTF8, 0, (LPCSTR)pByte, dwSize, m_pstrXML,
                              nWide);
        ...
        m_pstrXML = static_cast<LPTSTR>(malloc((nWide + 1) * sizeof(TCHAR)));
        ::MultiByteToWideChar(CP_ACP, 0, (LPCSTR)pByte, dwSize, m_pstrXML,
                              nWide);
        m_pstrXML[nWide] = _T('\0');
    } else {
    ...
    if (encoding == XMLFILE_ENCODING_UTF8) {
        ...
        DWORD wide = ::WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)w_str, nWide,
                                           NULL, 0, NULL, NULL);

        m_pstrXML = static_cast<LPTSTR>(malloc((wide + 1) * sizeof(TCHAR)));
        ::WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)w_str, nWide, m_pstrXML, wide,
                              NULL, NULL);
        m_pstrXML[wide] = _T('\0');
        ...
        if (dwSize >= 2 && ((pByte[0] == 0xFE && pByte[1] == 0xFF) ||
            ...
            DWORD nWide = ::WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)pByte,
                                                dwSize, NULL, 0, NULL, NULL);
            m_pstrXML =
                static_cast<LPTSTR>(malloc((nWide + 1) * sizeof(TCHAR)));
            ::WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)pByte, dwSize, m_pstrXML,
                                  nWide, NULL, NULL);
            m_pstrXML[nWide] = _T('\0');
...
bool CMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding) {
    ...
    HANDLE hFile =
        ::CreateFile(sFile.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL,
...
bool CMarkup::_Failed(LPCTSTR pstrError, LPCTSTR pstrLocation) {
    ...
    if (pstrLocation != NULL)
        TRACE(pstrLocation);
    _tcsncpy(m_szErrorMsg, pstrError,
             (sizeof(m_szErrorMsg) / sizeof(m_szErrorMsg[0])) - 1);
    _tcsncpy(m_szErrorXML, pstrLocation != NULL ? pstrLocation : _T(""),
