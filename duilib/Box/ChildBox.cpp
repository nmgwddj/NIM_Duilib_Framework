...
void ChildBox::Init() {
    if (!m_strXMLFile.empty()) {
        ...
        if (pBaseWindow != nullptr)
            callback = std::bind(&WindowImplBase::CreateControl, pBaseWindow,
                                 std::placeholders::_1);

        Box* pChildWindow = static_cast<Box*>(
            GlobalManager::CreateBoxWithCache(m_strXMLFile.c_str(), callback));
        if (pChildWindow) {
...
void ChildBox::SetAttribute(const ::std::wstring& strName,
