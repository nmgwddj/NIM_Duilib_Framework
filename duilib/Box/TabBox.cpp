    ...
    for (std::size_t it = 0; it < m_items.size(); it++) {
        if ((int)it == iIndex) {
            ...
            if (!IsFadeSwitch()) {
                ...
                auto player = m_items[it]->GetAnimationManager().SetFadeInOutX(
                    true, true);
            ...
            if ((int)it != iOldSel) {
                ...
                if (!IsFadeSwitch()) {
                    ...
                    auto player =
                        m_items[it]->GetAnimationManager().SetFadeInOutX(true,
                                                                         true);
                    player->SetStartValue(startValue);
                    player->SetEndValue(endValue);
                    player->SetSpeedUpfactorA(0.015);
                    CompleteCallback compelteCallback =
                        nbase::Bind(&TabBox::HideTabItem, this, it);
