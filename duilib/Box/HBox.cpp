...
CSize HLayout::ArrangeChild(const std::vector<Control*>& items, UiRect rc) {
    ...
    for (auto it = items.begin(); it != items.end(); it++) {
        ...
        if (sz.cx == DUI_LENGTH_STRETCH) {
            ...
            cxFixed += sz.cx + pControl->GetMargin().left +
                       pControl->GetMargin().right;
        }
    ...
    for (auto it = items.begin(); it != items.end(); it++) {
        ...
        if (verAlignType == kVerAlignTop) {
            ...
            childTop = iPosTop + (iPosBottom - iPosTop + rcMargin.top -
                                  rcMargin.bottom - sz.cy) /
                                     2;
            childBottm = childTop + sz.cy;
        }

        UiRect rcChildPos(iPosX + rcMargin.left, childTop,
                          iPosX + rcMargin.left + sz.cx, childBottm);
...
CSize HLayout::AjustSizeByChild(const std::vector<Control*>& items,
                                CSize szAvailable) {
    CSize totalSize;
    CSize itemSize;
    ...
    for (int it = 0; it < count; it++) {
        ...
        if (pChildControl->GetMaxWidth() >= 0 &&
        ...
        totalSize.cx += itemSize.cx + pChildControl->GetMargin().left +
                        pChildControl->GetMargin().right;
        totalSize.cy = MAX(itemSize.cy + pChildControl->GetMargin().top +
                               pChildControl->GetMargin().bottom,
                           totalSize.cy);
    }
