...
CSize VLayout::ArrangeChild(const std::vector<Control*>& items, UiRect rc) {
    ...
    for (auto it = items.begin(); it != items.end(); it++) {
        ...
        if (sz.cy == DUI_LENGTH_STRETCH) {
            ...
            cyFixed += sz.cy + pControl->GetMargin().top +
                       pControl->GetMargin().bottom;
        }
    ...
    for (auto it = items.begin(); it != items.end(); it++) {
        ...
        if (horAlignType == kHorAlignLeft) {
            ...
            childLeft = iPosLeft + (iPosRight - iPosLeft + rcMargin.left -
                                    rcMargin.right - sz.cx) /
                                       2;
            childRight = childLeft + sz.cx;
        }

        UiRect rcCtrl(childLeft, iPosY + rcMargin.top, childRight,
                      iPosY + rcMargin.top + sz.cy);
...
CSize VLayout::AjustSizeByChild(const std::vector<Control*>& items,
                                CSize szAvailable) {
    CSize totalSize;
    CSize itemSize;
    ...
    for (int it = 0; it < count; it++) {
        ...
        totalSize.cx = MAX(itemSize.cx + pChildControl->GetMargin().left +
                               pChildControl->GetMargin().right,
                           totalSize.cx);
        totalSize.cy += itemSize.cy + pChildControl->GetMargin().top +
