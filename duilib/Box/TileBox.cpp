    ...
    for (auto it = items.begin(); it != items.end(); it++) {
        ...
        if ((iCount % m_nColumns) == 0) {
            ...
            for (auto it2 = it; it2 != items.end(); it2++) {
                ...
                CSize szAvailable(
                    rcTile.right - rcTile.left - rcMargin.left - rcMargin.right,
                    9999);
                ...
                cyHeight =
                    MAX(cyHeight, szTile.cy + rcMargin.top + rcMargin.bottom);
        ...
        CSize szAvailable(newRcTile.right - newRcTile.left,
                          newRcTile.bottom - newRcTile.top);
        ...
        if (szTile.cy > pControl->GetMaxHeight())
            szTile.cy = pControl->GetMaxHeight();
        UiRect rcPos(
            (newRcTile.left + newRcTile.right - szTile.cx) / 2,
            (newRcTile.top + newRcTile.bottom - szTile.cy) / 2,
            (newRcTile.left + newRcTile.right - szTile.cx) / 2 + szTile.cx,
            (newRcTile.top + newRcTile.bottom - szTile.cy) / 2 + szTile.cy);
...
CSize TileLayout::AjustSizeByChild(const std::vector<Control*>& items,
                                   CSize szAvailable) {
    ...
    if (visibleCount > 0) {
        ...
        size.cy += pControl->GetFixedHeight() * rows + m_rcPadding.top +
                   m_rcPadding.bottom + childMarginTotal;
    }

    return size;
}

bool TileLayout::SetAttribute(const std::wstring& strName,
                              const std::wstring& strValue) {
    bool hasAttribute = true;
    if (strName == _T("itemsize")) {
        CSize szItem;
        LPTSTR pstr = NULL;
        szItem.cx = _tcstol(strValue.c_str(), &pstr, 10);
        ASSERT(pstr);
