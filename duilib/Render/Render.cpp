...
static inline void DrawFunction(HDC hDC,
                                bool bTransparent,
                                UiRect rcDest,
                                HDC hdcSrc,
                                UiRect rcSrc,
    ...
    if (bTransparent || bAlphaChannel || uFade < 255 ||
        (rcSrc.GetWidth() == rcDest.GetWidth() &&
         rcSrc.GetHeight() == rcDest.GetHeight())) {
        BLENDFUNCTION ftn = {AC_SRC_OVER, 0, uFade, AC_SRC_ALPHA};
        ::AlphaBlend(hDC, rcDest.left, rcDest.top, rcDest.GetWidth(),
                     rcDest.GetHeight(), hdcSrc, rcSrc.left, rcSrc.top,
                     rcSrc.GetWidth(), rcSrc.GetHeight(), ftn);
    } else {
        ::StretchBlt(hDC, rcDest.left, rcDest.top, rcDest.GetWidth(),
                     rcDest.GetHeight(), hdcSrc, rcSrc.left, rcSrc.top,
                     rcSrc.GetWidth(), rcSrc.GetHeight(), SRCCOPY);
    }
}
...
std::unique_ptr<ui::IRenderContext> RenderContext_GdiPlus::Clone() {
    std::unique_ptr<ui::IRenderContext> pClone =
        std::make_unique<ui::RenderContext_GdiPlus>();
...
void RenderContext_GdiPlus::RestoreAlpha(const UiRect& rcDirty,
                                         const UiRect& rcShadowPadding,
                                         int alpha) {
    m_bitmap.RestoreAlpha(rcDirty, rcShadowPadding, alpha);
}

void RenderContext_GdiPlus::RestoreAlpha(
    const UiRect& rcDirty,
    const UiRect& rcShadowPadding /*= UiRect()*/) {
...
void RenderContext_GdiPlus::SetRoundClip(const UiRect& rc,
                                         int width,
                                         int height) {
...
HRESULT RenderContext_GdiPlus::BitBlt(int x,
                                      int y,
                                      int cx,
                                      int cy,
                                      HDC hdcSrc,
...
bool RenderContext_GdiPlus::StretchBlt(int xDest,
                                       int yDest,
                                       int widthDest,
                                       int heightDest,
                                       HDC hdcSrc,
    ...
    bool ret =
        (TRUE == ::StretchBlt(m_hDC, xDest, yDest, widthDest, heightDest,
                              hdcSrc, xSrc, yScr, widthSrc, heightSrc, rop));
    ::SetStretchBltMode(m_hDC, stretchBltMode);
    return ret;
}

bool RenderContext_GdiPlus::AlphaBlend(int xDest,
                                       int yDest,
                                       int widthDest,
                                       int heightDest,
                                       HDC hdcSrc,
    ...
    return (TRUE == ::AlphaBlend(m_hDC, xDest, yDest, widthDest, heightDest,
                                 hdcSrc, xSrc, yScr, widthSrc, heightSrc, bf));
}

void RenderContext_GdiPlus::DrawImage(const UiRect& rcPaint,
                                      HBITMAP hBitmap,
                                      bool bAlphaChannel,
    ...
    ASSERT(::GetObjectType(m_hDC) == OBJ_DC ||
           ::GetObjectType(m_hDC) == OBJ_MEMDC);
    ...
    if (::IntersectRect(&rcTemp, &rcPaint, &rcDest)) {
        if (!xtiled && !ytiled) {
            DrawFunction(m_hDC, m_bTransparent, rcDest, hCloneDC, rcSource,
                         bAlphaChannel, uFade);
        } else if (xtiled && ytiled) {
            LONG lWidth = rcImageSource.right - rcImageSource.left -
                          rcCorners.left - rcCorners.right;
            LONG lHeight = rcImageSource.bottom - rcImageSource.top -
            ...
            for (int j = 0; j < iTimesY; ++j) {
                ...
                for (int i = 0; i < iTimesX; ++i) {
                    ...
                    DrawFunction(m_hDC, m_bTransparent, rcDest, hCloneDC,
                                 rcSource, bAlphaChannel, uFade);
                }
            }
        } else if (xtiled) {  // supp
            LONG lWidth = rcImageSource.right - rcImageSource.left -
            ...
            if (fullytiled)
                iTimes =
                    (rcDest.bottom - rcDest.top + lHeight + nTiledMargin - 1) /
                    (lHeight + nTiledMargin);
            else
                iTimes = (rcDest.bottom - rcDest.top + nTiledMargin) /
                         (lHeight + nTiledMargin);
            ...
            for (int i = 0; i < iTimes; ++i) {
                ...
                DrawFunction(m_hDC, m_bTransparent, rcDestTemp, hCloneDC,
                             rcSource, bAlphaChannel, uFade);
            }
        }
    }
...
void RenderContext_GdiPlus::DrawColor(const UiRect& rc,
                                      DWORD dwColor,
                                      BYTE uFade) {
...
void RenderContext_GdiPlus::DrawLine(const UiRect& rc,
                                     int nSize,
                                     DWORD dwPenColor) {
    Gdiplus::Graphics graphics(m_hDC);
    Gdiplus::Pen pen(Gdiplus::Color(dwPenColor), (Gdiplus::REAL)nSize);
    graphics.DrawLine(&pen, Gdiplus::Point(rc.left, rc.top),
                      Gdiplus::Point(rc.right, rc.bottom));
}

void RenderContext_GdiPlus::DrawLine(const IPen* pen,
                                     int x1,
                                     int y1,
                                     int x2,
                                     int y2) {
...
void RenderContext_GdiPlus::DrawBezier(const IPen* pen,
                                       int x1,
                                       int y1,
                                       int x2,
                                       int y2,
    ...
    Gdiplus::Graphics graphics(m_hDC);
    graphics.SetSmoothingMode(Gdiplus::SmoothingModeAntiAlias);
    graphics.DrawBezier(((Pen_GdiPlus*)pen)->GetPen(), x1, y1, x2, y2, x3, y3,
                        x4, y4);
}

void RenderContext_GdiPlus::DrawRect(const UiRect& rc,
                                     int nSize,
                                     DWORD dwPenColor) {
    Gdiplus::Graphics graphics(m_hDC);
    Gdiplus::Pen pen(Gdiplus::Color(dwPenColor), (Gdiplus::REAL)nSize);
    graphics.DrawRectangle(&pen, rc.left, rc.top, rc.GetWidth(),
                           rc.GetHeight());
}

void RenderContext_GdiPlus::DrawRoundRect(const UiRect& rc,
    ...
    pPath.AddLine(rc.left + roundSize.cx, rc.top, rc.right - roundSize.cx,
                  rc.top);
    pPath.AddArc(rc.right - roundSize.cx, rc.top, roundSize.cx, roundSize.cy,
                 270, 90);
    pPath.AddLine(rc.right, rc.top + roundSize.cy, rc.right,
                  rc.bottom - roundSize.cy);
    pPath.AddArc(rc.right - roundSize.cx, rc.bottom - roundSize.cy,
                 roundSize.cx, roundSize.cy, 0, 90);
    pPath.AddLine(rc.right - roundSize.cx, rc.bottom, rc.left + roundSize.cx,
                  rc.bottom);
    pPath.AddArc(rc.left, rc.bottom - roundSize.cy, roundSize.cx, roundSize.cy,
                 90, 90);
    pPath.AddLine(rc.left, rc.bottom - roundSize.cy, rc.left,
                  rc.top + roundSize.cy);
    pPath.CloseFigure();
...
void RenderContext_GdiPlus::DrawText(const UiRect& rc,
    ...
    if (count == 0) {
        ...
        ::SetTextColor(m_hDC,
                       RGB(GetBValue(dwTextColor), GetGValue(dwTextColor),
                           GetRValue(dwTextColor)));
        HFONT hOldFont =
            (HFONT)::SelectObject(m_hDC, GlobalManager::GetFont(strFontId));
    ...
    Gdiplus::RectF rcPaint((Gdiplus::REAL)rc.left, (Gdiplus::REAL)rc.top,
    ...
    Gdiplus::SolidBrush tBrush(Gdiplus::Color(uFade, GetBValue(dwTextColor),
    ...
    if (bFillPath) {
        ...
        path.AddString(strText.c_str(), (int)strText.length(), &font_family,
                       font.GetStyle(), font.GetSize(), rcPaint, &stringFormat);
        graphics.FillPath(&tBrush, &path);
        return;
    }
    graphics.DrawString(strText.c_str(), (int)strText.length(), &font, rcPaint,
                        &stringFormat, &tBrush);
}

void RenderContext_GdiPlus::DrawEllipse(const UiRect& rc,
                                        int nSize,
                                        DWORD dwColor) {
    Gdiplus::Graphics graphics(m_hDC);
    graphics.SetSmoothingMode(Gdiplus::SmoothingModeAntiAlias);
...
void RenderContext_GdiPlus::FillEllipse(const UiRect& rc, DWORD dwColor) {
    ...
    graphics.FillEllipse(&brush, rc.left, rc.top, rc.GetWidth(),
                         rc.GetHeight());
}
...
void RenderContext_GdiPlus::DrawPath(const IPath* path, const IPen* pen) {
    Gdiplus::Graphics graphics(m_hDC);
    graphics.SetSmoothingMode(Gdiplus::SmoothingModeAntiAlias);
    graphics.DrawPath(((Pen_GdiPlus*)pen)->GetPen(),
                      ((Path_Gdiplus*)path)->GetPath());
}

void RenderContext_GdiPlus::FillPath(const IPath* path, const IBrush* brush) {
    Gdiplus::Graphics graphics(m_hDC);
    graphics.SetSmoothingMode(Gdiplus::SmoothingModeAntiAlias);
    graphics.FillPath(((Brush_Gdiplus*)brush)->GetBrush(),
                      ((Path_Gdiplus*)path)->GetPath());
}
...
#define USE_COLOR_MATRIX 0
    ...
    Gdiplus::RectF srcRc(0.0f, 0.0f, destRc.GetWidth() / scale,
                         destRc.GetHeight() / scale);
    Gdiplus::RectF excludeRc(abs(nBlurSize) / scale, abs(nBlurSize) / scale,
    ...
    auto add_roundcorner_path = [](Gdiplus::GraphicsPath& path,
                                   const Gdiplus::RectF& rect,
                                   const CSize& roundSize) {
        path.AddArc(rect.GetLeft(), rect.GetTop(), (Gdiplus::REAL)roundSize.cx,
                    (Gdiplus::REAL)roundSize.cy, 180, 90);
        path.AddLine(
            rect.GetLeft() + (Gdiplus::REAL)roundSize.cx, rect.GetTop(),
            rect.GetRight() - (Gdiplus::REAL)roundSize.cx, rect.GetTop());
        path.AddArc(rect.GetRight() - (Gdiplus::REAL)roundSize.cx,
                    rect.GetTop(), (Gdiplus::REAL)roundSize.cx,
                    (Gdiplus::REAL)roundSize.cy, 270, 90);
        path.AddLine(
            rect.GetRight(), rect.GetTop() + (Gdiplus::REAL)roundSize.cy,
            rect.GetRight(), rect.GetBottom() - (Gdiplus::REAL)roundSize.cy);
        path.AddArc(rect.GetRight() - (Gdiplus::REAL)roundSize.cx,
                    rect.GetBottom() - (Gdiplus::REAL)roundSize.cy,
                    (Gdiplus::REAL)roundSize.cx, (Gdiplus::REAL)roundSize.cy, 0,
                    90);
        path.AddLine(
            rect.GetRight() - (Gdiplus::REAL)roundSize.cx, rect.GetBottom(),
            rect.GetLeft() + (Gdiplus::REAL)roundSize.cx, rect.GetBottom());
        path.AddArc(
            rect.GetLeft(), rect.GetBottom() - (Gdiplus::REAL)roundSize.cy,
            (Gdiplus::REAL)roundSize.cx, (Gdiplus::REAL)roundSize.cy, 90, 90);
        path.AddLine(
            rect.GetLeft(), rect.GetBottom() - (Gdiplus::REAL)roundSize.cy,
            rect.GetLeft(), rect.GetTop() + (Gdiplus::REAL)roundSize.cy);
        path.CloseFigure();
    };
    ...
    Gdiplus::Color colors[] = {Gdiplus::Color::Transparent,
                               Gdiplus::Color(dwColor),
                               Gdiplus::Color(dwColor)};
    ...
    RECT rcBlurEffect{nBlurSize, nBlurSize, srcRc.Width - 2 * nBlurSize,
                      srcRc.Height - 2 * nBlurSize};
    ...
    // transformation. The element in the fifth row and fifth column of a 5×5
    // homogeneous matrix must be 1, and all of the other elements in the fifth
    // column must be 0. Color matrices are used to transform color vectors. The
    // first four components of a color vector hold the red, green, blue, and
    // alpha components(in that order) of a color. The fifth component of a color
    // vector is always 1.
    Gdiplus::ColorMatrix colorMatrix = {
        0.99215, 0, 0, 0, 0, 0,       0.24313, 0, 0, 0, 0, 0, 0.24705,
        0,       0, 0, 0, 0, 0.53923, 0,       0, 0, 0, 0, 1};
    ...
    graphics.DrawImage(&tempBitmap,
                       Gdiplus::RectF(destRc.left, destRc.top,
                                      destRc.GetWidth(), destRc.GetHeight()),
                       0, 0, srcRc.Width, srcRc.Height, Gdiplus::UnitPixel);
}

ui::UiRect RenderContext_GdiPlus::MeasureText(const std::wstring& strText,
                                              const std::wstring& strFontId,
                                              UINT uStyle,
    ...
    if (count == 0) {
        ...
        SIZE size = {0};
        GetTextExtentPoint32(m_hDC, strText.c_str(), _tcslen(strText.c_str()),
                             &size);
        ::SelectObject(m_hDC, hOldFont);
    ...
    if (width == DUI_NOSET_VALUE) {
        graphics.MeasureString(strText.c_str(), (int)strText.length(), &font,
                               Gdiplus::PointF(), &stringFormat, &bounds);
    } else {
        Gdiplus::REAL height = 0;
        if ((uStyle & DT_SINGLELINE) != 0) {
            Gdiplus::RectF rcEmpty((Gdiplus::REAL)0, (Gdiplus::REAL)0,
                                   (Gdiplus::REAL)0, (Gdiplus::REAL)0);
            graphics.MeasureString(L"测试", 2, &font, rcEmpty, &stringFormat,
                                   &bounds);
            height = bounds.Height;
        }
        Gdiplus::RectF rcText((Gdiplus::REAL)0, (Gdiplus::REAL)0,
                              (Gdiplus::REAL)width, height);
        graphics.MeasureString(strText.c_str(), (int)strText.length(), &font,
                               rcText, &stringFormat, &bounds);
    }

    UiRect rc(int(bounds.GetLeft()), int(bounds.GetTop()),
              int(bounds.GetRight() + 1), int(bounds.GetBottom() + 1));
    return rc;
}
