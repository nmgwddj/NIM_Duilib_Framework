...
HBITMAP GdiBitmap::CreateDIBBitmap(HDC hdc,
                                   int width,
                                   int height,
                                   bool flipBItmap,
                                   LPVOID* pBits) {
...
void GdiBitmap::ClearAlpha(const UiRect& rcDirty, int alpha) {
    ...
    if (nRight > nLeft) {
        for (int i = nTop; i < nBottom; ++i) {
            ::memset(pBmpBits + i * m_nWidth + nLeft, alpha,
                     (nRight - nLeft) * 4);
        }
    }
}

void GdiBitmap::RestoreAlpha(const UiRect& rcDirty,
    ...
    for (int i = nTop; i < nBottom; i++) {
        for (int j = nLeft; j < nRight; j++) {
            ...
            if (((j >= rcShadowPadding.left &&
                  j < m_nWidth - rcShadowPadding.right) ||
                 (i >= rcShadowPadding.top &&
                  i < m_nHeight - rcShadowPadding.bottom))) {
