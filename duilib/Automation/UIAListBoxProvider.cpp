...
IFACEMETHODIMP UIAListBoxProvider::QueryInterface(REFIID riid,
                                                  void** ppInterface) {
    *ppInterface = nullptr;
...
IFACEMETHODIMP UIAListBoxProvider::GetPatternProvider(PATTERNID iid,
                                                      IUnknown** retVal) {
...
IFACEMETHODIMP UIAListBoxProvider::GetPropertyValue(PROPERTYID propertyId,
                                                    VARIANT* pRetVal) {
    pRetVal->vt = VT_EMPTY;
    UIA_CHECK_ELEMENT(m_pControl);
