...
IFACEMETHODIMP UIAComboBoxProvider::QueryInterface(REFIID riid,
                                                   void** ppInterface) {
    *ppInterface = nullptr;
...
IFACEMETHODIMP UIAComboBoxProvider::GetPatternProvider(PATTERNID iid,
                                                       IUnknown** retVal) {
...
IFACEMETHODIMP UIAComboBoxProvider::GetPropertyValue(PROPERTYID propertyId,
                                                     VARIANT* pRetVal) {
    pRetVal->vt = VT_EMPTY;
    UIA_CHECK_ELEMENT(m_pControl);
...
IFACEMETHODIMP UIAComboBoxProvider::get_ExpandCollapseState(
    ExpandCollapseState* pRetVal) {
    *pRetVal = ExpandCollapseState_LeafNode;
    UIA_CHECK_ELEMENT(m_pControl);

    auto pComboBox = dynamic_cast<Combo*>(m_pControl);
    if (pComboBox && pComboBox->IsActivatable()) {
        *pRetVal = pComboBox->IsActivated() ? ExpandCollapseState_Expanded
