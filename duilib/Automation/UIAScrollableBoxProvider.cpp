...
IFACEMETHODIMP UIAScrollableBoxProvider::QueryInterface(REFIID riid,
                                                        void** ppInterface) {
    *ppInterface = nullptr;
...
IFACEMETHODIMP UIAScrollableBoxProvider::GetPatternProvider(PATTERNID iid,
                                                            IUnknown** retVal) {
...
IFACEMETHODIMP UIAScrollableBoxProvider::GetPropertyValue(PROPERTYID propertyId,
                                                          VARIANT* pRetVal) {
    pRetVal->vt = VT_EMPTY;
    UIA_CHECK_ELEMENT(m_pControl);
...
IFACEMETHODIMP UIAScrollableBoxProvider::Scroll(ScrollAmount horizontalAmount,
    ...
    if (pScrollableBox) {
        ...
        if (verticalAmount == ScrollAmount_LargeDecrement ||
            verticalAmount == ScrollAmount_SmallDecrement)
            pScrollableBox->LineUp(cyLine);
        else if (verticalAmount == ScrollAmount_LargeIncrement ||
                 verticalAmount == ScrollAmount_SmallIncrement)
            pScrollableBox->LineDown(cyLine);

        // decrement to left default
        if (horizontalAmount == ScrollAmount_LargeDecrement ||
            horizontalAmount == ScrollAmount_SmallDecrement)
            pScrollableBox->LineLeft(cxLine);
        else if (horizontalAmount == ScrollAmount_LargeIncrement ||
                 horizontalAmount == ScrollAmount_SmallIncrement)
...
IFACEMETHODIMP UIAScrollableBoxProvider::SetScrollPercent(
...
IFACEMETHODIMP UIAScrollableBoxProvider::get_HorizontalScrollPercent(
    double* pRetVal) {
    *pRetVal = 0.0f;
...
IFACEMETHODIMP UIAScrollableBoxProvider::get_VerticalScrollPercent(
    double* pRetVal) {
    *pRetVal = 0.0f;
...
IFACEMETHODIMP UIAScrollableBoxProvider::get_HorizontalViewSize(
    double* pRetVal) {
    *pRetVal = 0.0f;
...
IFACEMETHODIMP UIAScrollableBoxProvider::get_HorizontallyScrollable(
    BOOL* pRetVal) {
    *pRetVal = FALSE;
...
IFACEMETHODIMP UIAScrollableBoxProvider::get_VerticallyScrollable(
    BOOL* pRetVal) {
    *pRetVal = FALSE;
