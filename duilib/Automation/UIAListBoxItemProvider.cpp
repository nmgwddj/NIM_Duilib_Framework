...
IFACEMETHODIMP UIAListBoxItemProvider::QueryInterface(REFIID riid,
                                                      void** ppInterface) {
    *ppInterface = nullptr;
...
IFACEMETHODIMP UIAListBoxItemProvider::GetPatternProvider(PATTERNID iid,
                                                          IUnknown** retVal) {
...
IFACEMETHODIMP UIAListBoxItemProvider::GetPropertyValue(PROPERTYID propertyId,
                                                        VARIANT* pRetVal) {
    pRetVal->vt = VT_EMPTY;
    UIA_CHECK_ELEMENT(m_pControl);
...
IFACEMETHODIMP UIAListBoxItemProvider::get_SelectionContainer(
    ...
    *pRetVal = nullptr;
    ...
    if (pListBox) {
        *pRetVal =
            static_cast<IRawElementProviderSimple*>(pListBox->GetUIAProvider());
