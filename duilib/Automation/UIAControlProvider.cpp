...
UIAControlProvider::UIAControlProvider(Control* pControl)
    : m_refCount(1) {
    m_pControl = pControl;
}
...
IFACEMETHODIMP UIAControlProvider::QueryInterface(REFIID riid,
                                                  void** ppInterface) {
    *ppInterface = nullptr;
...
//
IFACEMETHODIMP UIAControlProvider::get_ProviderOptions(
    ...
    // instructs UIA that this provider works in terms of client coordinates
    // rather than screen coordinates. Constant may not be available yet in the
    // SDK, depending on when the SDK was constructed.
...
IFACEMETHODIMP UIAControlProvider::GetPatternProvider(PATTERNID patternId,
                                                      IUnknown** pRetVal) {
    UIA_CHECK_ELEMENT(m_pControl);
...
IFACEMETHODIMP UIAControlProvider::GetPropertyValue(PROPERTYID propertyId,
                                                    VARIANT* pRetVal) {
    pRetVal->vt = VT_EMPTY;
    UIA_CHECK_ELEMENT(m_pControl);
    ...
    switch (propertyId) {
        ...
        case UIA_LocalizedControlTypePropertyId:  // only lower string
            pRetVal->bstrVal = SysAllocString(
                StringHelper::MakeLowerString(m_pControl->GetType()).c_str());
        ...
        case UIA_NamePropertyId:
            if (!m_pControl->GetName().empty()) {
                pRetVal->bstrVal =
                    SysAllocString(m_pControl->GetName().c_str());
                if (pRetVal->bstrVal != nullptr) {
                    pRetVal->vt = VT_BSTR;
                }
            }
        ...
        case UIA_HelpTextPropertyId:
            if (!m_pControl->GetToolTipText().empty()) {
                pRetVal->bstrVal =
                    SysAllocString(m_pControl->GetToolTipText().c_str());
                if (pRetVal->bstrVal != nullptr) {
                    pRetVal->vt = VT_BSTR;
                }
            }
        ...
        case UIA_IsEnabledPropertyId:
            pRetVal->boolVal =
                m_pControl->IsEnabled() ? VARIANT_TRUE : VARIANT_FALSE;
            pRetVal->vt = VT_BOOL;
            break;
        case UIA_IsKeyboardFocusablePropertyId:
            pRetVal->boolVal =
                m_pControl->IsKeyboardEnabled() ? VARIANT_TRUE : VARIANT_FALSE;
            pRetVal->vt = VT_BOOL;
            break;
        case UIA_HasKeyboardFocusPropertyId:
            pRetVal->boolVal =
                m_pControl->IsFocused() && !m_pControl->IsMouseFocused()
                    ? VARIANT_TRUE
                    : VARIANT_FALSE;
            pRetVal->vt = VT_BOOL;
            break;
        ...
        case UIA_ItemStatusPropertyId:
            pRetVal->vt =
                VT_EMPTY;  // should be busy or connected with VT_BSTR type
            break;
            // view types
        ...
        case UIA_IsOffscreenPropertyId:
            pRetVal->boolVal =
                m_pControl->IsVisible() ? VARIANT_FALSE : VARIANT_TRUE;
            pRetVal->vt = VT_BOOL;
            break;
        case 30162:  // UIA_FillTypePropertyId: // uch as none, color, gradient,
...
// Return nullptr.
//
IFACEMETHODIMP UIAControlProvider::get_HostRawElementProvider(
...
//
IFACEMETHODIMP UIAControlProvider::Navigate(
    ...
    switch (direction) {
        case NavigateDirection_Parent: {
            if (pParent)
                *pRetVal = static_cast<IRawElementProviderFragment*>(
                    pParent->GetUIAProvider());
            else {
                Window* pRootWindow = m_pControl->GetWindow();
                if (pRootWindow) {
                    *pRetVal = static_cast<IRawElementProviderFragment*>(
                        pRootWindow->GetUIAProvider());
                }
            }
        } break;
        ...
        case NavigateDirection_NextSibling: {
            if (pParent) {
                ...
                if (nextSiblingControl)
                    *pRetVal = static_cast<IRawElementProviderFragment*>(
                        nextSiblingControl->GetUIAProvider());
            }
        } break;
        ...
        case NavigateDirection_PreviousSibling: {
            if (pParent) {
                ...
                if (preSiblingControl)
                    *pRetVal = static_cast<IRawElementProviderFragment*>(
                        preSiblingControl->GetUIAProvider());
            }
        } break;
        ...
        case NavigateDirection_FirstChild: {
            if (pBox && pBox->GetItemAt(0))
                *pRetVal = static_cast<IRawElementProviderFragment*>(
                    pBox->GetItemAt(0)->GetUIAProvider());
        } break;

        case NavigateDirection_LastChild:
            if (pBox && pBox->GetItemAt(pBox->GetCount() - 1))
                *pRetVal = static_cast<IRawElementProviderFragment*>(
                    pBox->GetItemAt(pBox->GetCount() - 1)->GetUIAProvider());
            break;
...
// Gets the runtime identifier. This is an array consisting of
// UiaAppendRuntimeId, which makes the ID unique among instances of the control,
// and the Automation Id.
//
...
//
IFACEMETHODIMP UIAControlProvider::GetEmbeddedFragmentRoots(
    SAFEARRAY** pRetVal) {
    UIA_CHECK_ELEMENT(m_pControl);
...
//
IFACEMETHODIMP UIAControlProvider::get_FragmentRoot(
    ...
    *pRetVal = nullptr;
    ...
    Window* pRootWindow = m_pControl->GetWindow();
    if (pRootWindow) {
        *pRetVal = static_cast<IRawElementProviderFragmentRoot*>(
            pRootWindow->GetUIAProvider());
        (*pRetVal)->AddRef();
    } else
        return E_FAIL;
