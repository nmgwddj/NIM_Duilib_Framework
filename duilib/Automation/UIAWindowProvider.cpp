...
IFACEMETHODIMP UIAWindowProvider::QueryInterface(REFIID riid,
                                                 void** ppInterface) {
...
//
IFACEMETHODIMP UIAWindowProvider::get_ProviderOptions(
...
//
IFACEMETHODIMP UIAWindowProvider::GetPatternProvider(PATTERNID patternId,
                                                     IUnknown** pRetVal) {
    UIA_CHECK_ELEMENT(m_pWnd);
...
//
IFACEMETHODIMP UIAWindowProvider::GetPropertyValue(PROPERTYID propertyId,
                                                   VARIANT* pRetVal) {
    UIA_CHECK_ELEMENT(m_pWnd);
...
//
IFACEMETHODIMP UIAWindowProvider::get_HostRawElementProvider(
...
//
IFACEMETHODIMP UIAWindowProvider::Navigate(
    NavigateDirection direction,
    IRawElementProviderFragment** pRetVal) {
    UIA_CHECK_ELEMENT(m_pWnd);

    IRawElementProviderFragment* pFrag = nullptr;
    if (direction == NavigateDirection_FirstChild && nullptr != m_pWnd &&
        nullptr != m_pWnd->GetRoot()) {
...
// UI Automation gets this value from the host window provider, so supply
// nullptr here.
//
...
//
IFACEMETHODIMP UIAWindowProvider::GetEmbeddedFragmentRoots(
    SAFEARRAY** pRetVal) {
    UIA_CHECK_ELEMENT(m_pWnd);
...
//
IFACEMETHODIMP UIAWindowProvider::get_FragmentRoot(
...
// Retrieves the IRawElementProviderFragment interface for the item at the
// specified point (in client coordinates). UI Spy uses this to determine what
// element is under the cursor when Ctrl is pressed.
//
IFACEMETHODIMP UIAWindowProvider::ElementProviderFromPoint(
    double x,
    double y,
    IRawElementProviderFragment** pRetVal) {
    UIA_CHECK_ELEMENT(m_pWnd);
...
//
IFACEMETHODIMP UIAWindowProvider::GetFocus(
    IRawElementProviderFragment** pRetVal) {
    UIA_CHECK_ELEMENT(m_pWnd);
...
IFACEMETHODIMP UIAWindowProvider::WaitForInputIdle(int milliseconds,
                                                   BOOL* pRetVal) {
    UIA_CHECK_ELEMENT(m_pWnd);
...
IFACEMETHODIMP UIAWindowProvider::get_WindowInteractionState(
...
IFACEMETHODIMP UIAWindowProvider::get_WindowVisualState(
    ...
    if (::IsIconic(m_pWnd->GetHWND()))
        ...
        *pRetVal = WindowVisualState::WindowVisualState_Normal;
