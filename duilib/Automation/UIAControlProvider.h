...
public:
    ...
    virtual IFACEMETHODIMP GetPropertyValue(_In_ PROPERTYID idProp,
                                            _Out_ VARIANT* retVal);
    virtual IFACEMETHODIMP get_HostRawElementProvider(
        _Outptr_result_maybenull_ IRawElementProviderSimple** retVal);

    // IRawElementProviderFragment methods
    virtual IFACEMETHODIMP Navigate(
        _In_ NavigateDirection direction,
        _Outptr_result_maybenull_ IRawElementProviderFragment** retVal);
    virtual IFACEMETHODIMP GetRuntimeId(
        _Outptr_result_maybenull_ SAFEARRAY** retVal);
    virtual IFACEMETHODIMP get_BoundingRectangle(_Out_ UiaRect* retVal);
    virtual IFACEMETHODIMP GetEmbeddedFragmentRoots(
        _Outptr_result_maybenull_ SAFEARRAY** retVal);
    virtual IFACEMETHODIMP SetFocus();
    virtual IFACEMETHODIMP get_FragmentRoot(
        _Outptr_result_maybenull_ IRawElementProviderFragmentRoot** retVal);
    ...
    virtual IFACEMETHODIMP get_ToggleState(
    ...
    virtual IFACEMETHODIMP FindItemByProperty(
    ...
    virtual IFACEMETHODIMP Scroll(enum ScrollAmount horizontalAmount,
                                  enum ScrollAmount verticalAmount) {
        UIA_CHECK_ELEMENT_RETURN(m_pControl);
    }
    virtual IFACEMETHODIMP SetScrollPercent(double horizontalPercent,
                                            double verticalPercent) {
        UIA_CHECK_ELEMENT_RETURN(m_pControl);
    }
    virtual IFACEMETHODIMP get_HorizontalScrollPercent(
        __RPC__out double* pRetVal) {
        *pRetVal = 0.0f;
        UIA_CHECK_ELEMENT_RETURN(m_pControl);
    }
    virtual IFACEMETHODIMP get_VerticalScrollPercent(
        __RPC__out double* pRetVal) {
        *pRetVal = 0.0f;
        UIA_CHECK_ELEMENT_RETURN(m_pControl);
    }
    ...
    virtual IFACEMETHODIMP get_HorizontallyScrollable(
    ...
    virtual IFACEMETHODIMP get_SelectionContainer(
...
public:
    ...
    void ResetControl(Control* pControl) { m_pControl = pControl; }
