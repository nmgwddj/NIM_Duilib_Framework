...
IFACEMETHODIMP UIABoxProvider::GetPatternProvider(PATTERNID iid,
                                                  IUnknown** retVal) {
...
IFACEMETHODIMP UIABoxProvider::GetPropertyValue(PROPERTYID propertyId,
                                                VARIANT* pRetVal) {
    pRetVal->vt = VT_EMPTY;
    UIA_CHECK_ELEMENT(m_pControl);
...
//
IFACEMETHODIMP UIABoxProvider::FindItemByProperty(
    ...
    *pFound = nullptr;
    ...
    // When the propertyId parameter is 0, the provider should return the next
    // item after pStartAfter. If the pStartAfter parameter is NULL and
    // propertyId is 0, the provider should return the first item in the
    // container.
    if (propertyId == 0) {
        if (pStartAfter == nullptr) {
            ...
            auto pStarterProvider =
                dynamic_cast<UIAControlProvider*>(pStartAfter);
            if (pStarterProvider && pStarterProvider->GetControl()) {
                auto pStarterControl = pStarterProvider->GetControl();
                auto pNextControl =
                    pBox->GetItemAt(pBox->GetItemIndex(pStarterControl) + 1);
                if (pNextControl)
                    *pFound = static_cast<IRawElementProviderSimple*>(
                        pNextControl->GetUIAProvider());
            }
        }
    }
    // While not required, Microsoft highly recommends that FindItemByProperty
    // support the Name, AutomationId, and IsSelected properties.
    else {
        switch (propertyId) {
            ...
            case UIA_SelectionItemIsSelectedPropertyId: {
                ...
                if (QueryInterface(__uuidof(ISelectionProvider),
                                   (void**)&pSelectionProvider) == S_OK &&
                    pSelectionProvider) {
                    SAFEARRAY* pSelectedItemArray = nullptr;
                    if (pSelectionProvider->GetSelection(&pSelectedItemArray) ==
                            S_OK &&
                        pSelectedItemArray &&
                        SafeArrayGetElemsize(pSelectedItemArray)) {
                        long index = 0;
                        SafeArrayGetElement(pSelectedItemArray, &index,
                                            *pFound);
                    }
