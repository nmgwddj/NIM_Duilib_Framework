...
IFACEMETHODIMP UIATreeNodeProvider::QueryInterface(REFIID riid,
                                                   void** ppInterface) {
    *ppInterface = nullptr;
...
IFACEMETHODIMP UIATreeNodeProvider::GetPatternProvider(PATTERNID iid,
                                                       IUnknown** retVal) {
...
IFACEMETHODIMP UIATreeNodeProvider::GetPropertyValue(PROPERTYID propertyId,
                                                     VARIANT* pRetVal) {
    pRetVal->vt = VT_EMPTY;
    UIA_CHECK_ELEMENT(m_pControl);
...
IFACEMETHODIMP UIATreeNodeProvider::get_ExpandCollapseState(
    ...
    *pRetVal = ExpandCollapseState_LeafNode;
    ...
    if (pNode && pNode->GetChildNodeCount()) {
        *pRetVal = pNode->IsExpand() ? ExpandCollapseState_Expanded
