...
//		(PS:如果开发者不使用负责Cef功能的开发，可以切换到release模式的cef
//dll文件，这样即使在deubg下也不会报错，修改AddCefDllToPath代码可以切换到release目录)
bool CefManager::Initialize(const std::wstring& app_data_dir,
                            CefSettings& settings,
                            bool is_enable_offset_render /*= true*/) {
#if !defined(SUPPORT_CEF)
    return true;
#endif
    ...
    if (is_enable_offset_render_) {
        HWND hwnd = CreateWindow(L"Static", L"", WS_POPUP, 0, 0, 0, 0, NULL,
                                 NULL, NULL, NULL);
...
void CefManager::PostQuitMessage(int nExitCode) {
    ...
    if (browser_count_ == 0) {
        ...
        nbase::ThreadManager::PostDelayedTask(
            kThreadUI, cb, nbase::TimeDelta::FromMilliseconds(500));
    }
}
...
client::DropTargetHandle CefManager::GetDropTarget(HWND hwnd) {
    ...
    if (it == map_drag_target_reference_.end()) {
        ...
        // 如果不存在就新增一个
        client::DropTargetHandle handle(new client::DropTargetWin(hwnd),
                                        deleter);
...
void CefManager::GetCefSetting(const std::wstring& app_data_dir,
                               CefSettings& settings) {
