 ...
 * @brief
 *当multi_threaded_message_loop为false时，需要我们在自己的消息循环中主动调用Cef的消息循环接口
...
public:
    ...
    /**
     * 把cef
     * dll文件的位置添加到程序的"path"环境变量中,这样可以把dll文件放到bin以外的目录，并且不需要手动频繁切换dll文件
     * @return void	无返回值
     */
     ...
     */
    bool Initialize(const std::wstring& app_data_dir,
...
private:
    ...
    std::map<HWND, std::weak_ptr<client::DropTargetWin>>
        map_drag_target_reference_;  // 各个DropTarget的弱引用，一个窗口对应一个DropTarget，这个DropTarget可以给多个BorwserHandler使用
};
}  // namespace nim_comp