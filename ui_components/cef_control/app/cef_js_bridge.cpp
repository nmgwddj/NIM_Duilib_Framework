...
bool CefJSBridge::CallCppFunction(const CefString& function_name,
    ...
    if (it == render_callback_.cend()) {
        ...
        CefRefPtr<CefProcessMessage> message =
            CefProcessMessage::Create(kCallCppFunctionMessage);
        ...
        render_callback_.emplace(js_callback_id_++,
...
void CefJSBridge::RemoveCallbackFuncWithFrame(CefRefPtr<CefFrame> frame) {
    if (!render_callback_.empty()) {
        for (auto it = render_callback_.begin();
             it != render_callback_.end();) {
...
bool CefJSBridge::ExecuteJSCallbackFunc(int js_callback_id,
                                        bool has_error,
                                        const CefString& json_result) {
    ...
    if (it != render_callback_.cend()) {
        ...
        if (context.get() && callback.get()) {
            ...
            CefRefPtr<CefV8Value> json_parse =
                context->GetGlobal()->GetValue("JSON")->GetValue("parse");
            CefRefPtr<CefV8Value> json_object =
                json_parse->ExecuteFunction(NULL, json_parse_args);
...
bool CefJSBridge::RegisterJSFunc(const CefString& function_name,
    ...
    if (replace) {
        render_registered_function_.emplace(
            std::make_pair(function_name, frame->GetIdentifier()), function);
        return true;
    } else {
        auto it = render_registered_function_.find(
            std::make_pair(function_name, frame->GetIdentifier()));
...
void CefJSBridge::UnRegisterJSFunc(const CefString& function_name,
                                   CefRefPtr<CefFrame> frame) {
    render_registered_function_.erase(
        std::make_pair(function_name, frame->GetIdentifier()));
}

void CefJSBridge::UnRegisterJSFuncWithFrame(CefRefPtr<CefFrame> frame) {
    // 由于本类中每一个 render 和 browser 进程都独享一份实例，而不是单例模式
    // 所以这里获取的 browser 都是全局唯一的，可以根据这个 browser 获取所有
    // frame 和 context
    auto browser = frame->GetBrowser();

    if (!render_registered_function_.empty()) {
        for (auto it = render_registered_function_.begin();
             it != render_registered_function_.end();) {
...
bool CefJSBridge::ExecuteJSFunc(const CefString& function_name,
    ...
    if (it != render_registered_function_.cend()) {
        ...
        if (context.get() && function.get()) {
            ...
            CefRefPtr<CefV8Value> json_object =
                context->GetGlobal()->GetValue("JSON");
            CefRefPtr<CefV8Value> json_parse = json_object->GetValue("parse");
            CefRefPtr<CefV8Value> json_stringify =
                json_object->GetValue("stringify");
            ...
            // 执行回调函数
            CefRefPtr<CefV8Value> retval =
                function->ExecuteFunction(NULL, arguments);
            if (retval.get() && retval->IsObject()) {
                ...
                CefRefPtr<CefV8Value> json_string =
                    json_stringify->ExecuteFunction(NULL, json_stringify_args);
                CefString str = json_string->GetStringValue();

                CefRefPtr<CefProcessMessage> message =
                    CefProcessMessage::Create(kExecuteCppCallbackMessage);
...
bool CefJSBridge::CallJSFunction(const CefString& js_function_name,
                                 const CefString& params,
                                 CefRefPtr<CefFrame> frame,
    ...
    if (it == browser_callback_.cend()) {
        ...
        CefRefPtr<CefProcessMessage> message =
            CefProcessMessage::Create(kCallJsFunctionMessage);
        CefRefPtr<CefListValue> args = message->GetArgumentList();
...
bool CefJSBridge::ExecuteCppCallbackFunc(int cpp_callback_id,
...
bool CefJSBridge::RegisterCppFunc(const CefString& function_name,
                                  CppFunction function,
                                  CefRefPtr<CefBrowser> browser,
                                  bool replace /*= false*/) {
    if (replace) {
        browser_registered_function_.emplace(
            std::make_pair(function_name,
                           browser ? browser->GetIdentifier() : -1),
            function);  // = ;
        return true;
    } else {
        auto it = browser_registered_function_.find(std::make_pair(
            function_name, browser ? browser->GetIdentifier() : -1));
...
void CefJSBridge::UnRegisterCppFunc(const CefString& function_name,
                                    CefRefPtr<CefBrowser> browser) {
    browser_registered_function_.erase(
        std::make_pair(function_name, browser ? browser->GetIdentifier() : -1));
}

bool CefJSBridge::ExecuteCppFunc(const CefString& function_name,
                                 const CefString& params,
                                 int js_callback_id,
                                 CefRefPtr<CefBrowser> browser) {
    CefRefPtr<CefProcessMessage> message =
    ...
    auto it = browser_registered_function_.find(
        std::make_pair(function_name, browser->GetIdentifier()));
    if (it != browser_registered_function_.cend()) {
        auto function = it->second;
        nbase::ThreadManager::PostTask(kThreadUI, [=]() {
            function(params,
                     [=](bool has_error, const std::string& json_result) {
                         // 测试代码，需要封装到管理器中
