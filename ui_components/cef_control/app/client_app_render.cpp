...
void ClientApp::OnWebKitInitialized() {
    /**
     * JavaScript 扩展代码，这里定义一个 NimCefWebFunction 对象提供 call
     * 方法来让 Web 端触发 native 的 CefV8Handler 处理代码 param[in]
     * functionName	要调用的 C++ 方法名称 param[in] params
     * 调用该方法传递的参数，在前端指定的是一个 Object，但转到 Native
     * 的时候转为了字符串 param[in] callback		执行该方法后的回调函数
     * 前端调用示例
...
bool ClientApp::OnProcessMessageReceived(CefRefPtr<CefBrowser> browser,
    ...
    if (message_name == kExecuteJsCallbackMessage) {
        ...
        render_js_bridge_->ExecuteJSCallbackFunc(callback_id, has_error,
                                                 json_string);
        ...
        // frame_id 小于 0 则可能是 browser 进程的 browser
        // 是无效的，所以这里为了避免出现错误就获取一个顶层 frame 执行代码
        render_js_bridge_->ExecuteJSFunc(function_name, json_string,
                                         frame_id < 0
                                             ? browser->GetMainFrame()
                                             : browser->GetFrame(frame_id),
                                         cpp_callback_id);
    }
