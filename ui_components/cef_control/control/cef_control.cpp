...
void CefControl::OnPaint(CefRefPtr<CefBrowser> browser,
    ...
    } else if (type == PET_POPUP && dc_cef_.IsValid() &&
               rect_popup_.width > 0 && rect_popup_.height > 0) {
        // 单独保存popup窗口的位图
        if (dc_cef_popup_.GetWidth() != width ||
            dc_cef_popup_.GetHeight() != height)
...
void CefControl::OnPopupSize(CefRefPtr<CefBrowser> browser,
                             const CefRect& rect) {
