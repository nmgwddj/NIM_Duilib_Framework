...
void CefNativeControl::Init() {
    if (browser_handler_.get() == nullptr) {
        ...
        SetWindowLong(m_pWindow->GetHWND(), GWL_STYLE,
                      style | WS_CLIPSIBLINGS | WS_CLIPCHILDREN);
        ASSERT((GetWindowExStyle(m_pWindow->GetHWND()) & WS_EX_LAYERED) == 0 &&
               L"无法在分层窗口内使用本控件");
...
void CefNativeControl::SetPos(UiRect rc) {
    ...
    if (hwnd) {
        SetWindowPos(hwnd, HWND_TOP, rc.left, rc.top, rc.GetWidth(),
                     rc.GetHeight(), SWP_NOZORDER);
    }
}
...
void CefNativeControl::SetWindow(ui::Window* pManager,
    ...
    SetWindowLong(pManager->GetHWND(), GWL_STYLE,
                  style | WS_CLIPSIBLINGS | WS_CLIPCHILDREN);
...
bool CefNativeControl::AttachDevTools(Control* /*view*/) {
    ...
    if (browser == nullptr) {
        ...
        browser->GetHost()->ShowDevTools(
            windowInfo, new nim_comp::BrowserHandler, settings, CefPoint());
