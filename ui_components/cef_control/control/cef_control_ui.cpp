...
void CefControl::ReCreateBrowser() {
    if (browser_handler_->GetBrowser() == nullptr) {
        ...
        CefBrowserHost::CreateBrowser(window_info, browser_handler_, L"",
...
void CefControl::Paint(IRenderContext* pRender, const UiRect& rcPaint) {
    ...
    if (dc_cef_.IsValid() && browser_handler_.get() &&
        browser_handler_->GetBrowser().get()) {
        // 绘制cef PET_VIEW类型的位图
        BitBlt(pRender->GetDC(), m_rcItem.left, m_rcItem.top,
               m_rcItem.GetWidth(), m_rcItem.GetHeight(), dc_cef_.GetDC(), 0, 0,
               SRCCOPY);
        ...
        if (!rect_popup_.IsEmpty() && dc_cef_popup_.IsValid()) {
            ...
            BitBlt(pRender->GetDC(), m_rcItem.left + paint_x,
                   m_rcItem.top + paint_y, rect_popup_.width,
                   rect_popup_.height, dc_cef_popup_.GetDC(), paint_buffer_x,
                   paint_buffer_y, SRCCOPY);
        }
    }
}
...
LRESULT CefControl::MessageHandler(UINT uMsg,
                                   WPARAM wParam,
                                   LPARAM lParam,
                                   BOOL& bHandled) {
    ...
    bHandled = false;
    if (browser_handler_.get() == nullptr ||
        browser_handler_->GetBrowser().get() == nullptr)
        return 0;
...
bool CefControl::AttachDevTools(Control* control) {
    ...
    if (browser == nullptr || view_browser == nullptr) {
        ...
        browser->GetHost()->ShowDevTools(windowInfo,
                                         view_browser->GetHost()->GetClient(),
                                         settings, CefPoint());
...
//////////////////////////////////////////////////////////////////////////////////
LRESULT CefControl::SendButtonDownEvent(UINT uMsg,
                                        WPARAM wParam,
                                        LPARAM lParam,
                                        BOOL& bHandled) {
...
LRESULT CefControl::SendButtonDoubleDownEvent(UINT uMsg,
                                              WPARAM wParam,
                                              LPARAM lParam,
                                              BOOL& bHandled) {
    CefRefPtr<CefBrowserHost> host = browser_handler_->GetBrowserHost();
...
LRESULT CefControl::SendButtonUpEvent(UINT uMsg,
                                      WPARAM wParam,
                                      LPARAM lParam,
                                      BOOL& bHandled) {
    CefRefPtr<CefBrowserHost> host = browser_handler_->GetBrowserHost();
...
LRESULT CefControl::SendMouseMoveEvent(UINT uMsg,
                                       WPARAM wParam,
                                       LPARAM lParam,
                                       BOOL& bHandled) {
    CefRefPtr<CefBrowserHost> host = browser_handler_->GetBrowserHost();
...
LRESULT CefControl::SendMouseWheelEvent(UINT uMsg,
                                        WPARAM wParam,
                                        LPARAM lParam,
                                        BOOL& bHandled) {
    CefRefPtr<CefBrowserHost> host = browser_handler_->GetBrowserHost();
...
LRESULT CefControl::SendMouseLeaveEvent(UINT uMsg,
                                        WPARAM wParam,
                                        LPARAM lParam,
                                        BOOL& bHandled) {
    CefRefPtr<CefBrowserHost> host = browser_handler_->GetBrowserHost();
...
LRESULT CefControl::SendKeyEvent(UINT uMsg,
                                 WPARAM wParam,
                                 LPARAM lParam,
                                 BOOL& bHandled) {
    CefRefPtr<CefBrowserHost> host = browser_handler_->GetBrowserHost();
...
LRESULT CefControl::SendCaptureLostEvent(UINT uMsg,
                                         WPARAM wParam,
                                         LPARAM lParam,
                                         BOOL& bHandled) {
    CefRefPtr<CefBrowserHost> host = browser_handler_->GetBrowserHost();
