...
void CefControlBase::LoadString(const CefString& stringW,
                                const CefString& url) {
    if (browser_handler_.get() && browser_handler_->GetBrowser().get()) {
        CefRefPtr<CefFrame> frame =
            browser_handler_->GetBrowser()->GetMainFrame();
...
CefString CefControlBase::GetMainURL(const CefString& url) {
    ...
    int end_pos =
        temp.find("#") == std::string::npos ? temp.length() : temp.find("#");
    temp = temp.substr(0, end_pos);
    return CefString(temp.c_str());
}

bool CefControlBase::RegisterCppFunc(const std::wstring& function_name,
                                     nim_comp::CppFunction function,
                                     bool global_function /* = false*/) {
    if (browser_handler_.get() && browser_handler_->GetBrowser().get() &&
        js_bridge_.get()) {
        return js_bridge_->RegisterCppFunc(
            nbase::UTF16ToUTF8(function_name).c_str(), function,
            global_function ? nullptr : browser_handler_->GetBrowser());
    }
...
void CefControlBase::UnRegisterCppFunc(const std::wstring& function_name) {
    if (browser_handler_.get() && browser_handler_->GetBrowser().get() &&
        js_bridge_.get()) {
        js_bridge_->UnRegisterCppFunc(nbase::UTF16ToUTF8(function_name).c_str(),
                                      browser_handler_->GetBrowser());
    }
}

bool CefControlBase::CallJSFunction(const std::wstring& js_function_name,
                                    const std::wstring& params,
                                    nim_comp::CallJsFunctionCallback callback,
                                    const std::wstring& frame_name /*= L""*/) {
    if (browser_handler_.get() && browser_handler_->GetBrowser().get() &&
        js_bridge_.get()) {
        CefRefPtr<CefFrame> frame =
            frame_name == L""
        ...
        if (!js_bridge_->CallJSFunction(
                nbase::UTF16ToUTF8(js_function_name).c_str(),
...
bool CefControlBase::CallJSFunction(const std::wstring& js_function_name,
    ...
    if (browser_handler_.get() && browser_handler_->GetBrowser().get() &&
        js_bridge_.get()) {
        CefRefPtr<CefFrame> frame =
            browser_handler_->GetBrowser()->GetFrame(frame_id);
