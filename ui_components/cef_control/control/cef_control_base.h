...
class CefControlBase : public Control,
                       public nim_comp::BrowserHandler::HandlerDelegate {
public:
    CefControlBase(void);
...
public:
     ...
     */
    bool RegisterCppFunc(const std::wstring& function_name,
     ...
     */
    bool CallJSFunction(const std::wstring& js_function_name,
...
public:
     ...
     * @param[in] callback 一个回调函数，参考 OnDevToolAttachedStateChangeEvent
     * 声明
     * @return 无
     */
    void AttachDevToolAttachedStateChange(
        const OnDevToolAttachedStateChangeEvent& callback) {
...
private:
    ...
    virtual bool OnExecuteCppFunc(const CefString& function_name,
                                  const CefString& params,
                                  int js_callback_id,
                                  CefRefPtr<CefBrowser> browser) OVERRIDE;

    virtual bool OnExecuteCppCallbackFunc(
        int cpp_callback_id,
        const CefString& json_string) OVERRIDE;
...
protected:
    ...
    int js_callback_thread_id_ =
        -1;  // 保存接收到 JS 调用 CPP 函数的代码所属线程，以后触发 JS
             // 回调时把回调转到那个线程
};
}  // namespace nim_comp