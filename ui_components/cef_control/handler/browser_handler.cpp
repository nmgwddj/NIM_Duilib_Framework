...
void BrowserHandler::CloseAllBrowser() {
    ...
    public:
        CloseAllBrowserTask(
            const std::vector<CefRefPtr<CefBrowser>>& browser_list) {
...
bool BrowserHandler::OnProcessMessageReceived(
    ...
    if (message_name == kFocusedNodeChangedMessage) {
        ...
        if (handle_delegate_)
            handle_delegate_->OnExecuteCppFunc(fun_name, param, js_callback_id,
                                               browser);
...
void BrowserHandler::OnAfterCreated(CefRefPtr<CefBrowser> browser) {
    ...
    nbase::ThreadManager::PostTask(
        kThreadUI, ToWeakCallback([this, browser]() {
    ...
    // 所以在cef ui线程调用RegisterDragDrop，让后面一系列操作都在cef
    // ui线程里同步执行，则没问题
    //
...
void BrowserHandler::OnBeforeClose(CefRefPtr<CefBrowser> browser) {
    ...
    nbase::ThreadManager::PostTask(
        kThreadUI, ToWeakCallback([this, browser]() {
            ...
            auto it = std::find_if(browser_list_.begin(), browser_list_.end(),
                                   [&](const CefRefPtr<CefBrowser>& item) {
            ...
            if (it != browser_list_.end()) {
                ...
                if (closed_browser->IsSame(browser_)) {
                    browser_ = browser_list_.size() > 0
                                   ? *browser_list_.rbegin()
                                   : nullptr;
                    if (browser_ != nullptr) {
...
void BrowserHandler::OnPopupShow(CefRefPtr<CefBrowser> browser, bool show) {
    if (handle_delegate_)
        nbase::ThreadManager::PostTask(
            kThreadUI, nbase::Bind(&HandlerDelegate::OnPopupShow,
                                   handle_delegate_, browser, show));
}
...
void BrowserHandler::OnPopupSize(CefRefPtr<CefBrowser> browser,
    ...
    if (handle_delegate_)
        nbase::ThreadManager::PostTask(
            kThreadUI, nbase::Bind(&HandlerDelegate::OnPopupSize,
                                   handle_delegate_, browser, rect));
}
...
void BrowserHandler::OnPaint(CefRefPtr<CefBrowser> browser,
    ...
    if (handle_delegate_) {
        ...
        nbase::ThreadManager::PostTask(
            kThreadUI,
            nbase::Bind(&HandlerDelegate::OnPaint, handle_delegate_, browser,
                        type, dirtyRects, &paint_buffer_, width, height));
    }
}
...
CefBrowserHost::DragOperationsMask BrowserHandler::OnDragEnter(
    CefRefPtr<CefDragData> drag_data,
    CefMouseEvent ev,
...
CefBrowserHost::DragOperationsMask BrowserHandler::OnDragOver(
    CefMouseEvent ev,
    CefBrowserHost::DragOperationsMask effect) {
    REQUIRE_UI_THREAD();
    if (browser_) {
...
CefBrowserHost::DragOperationsMask BrowserHandler::OnDrop(
    CefMouseEvent ev,
    CefBrowserHost::DragOperationsMask effect) {
    if (browser_) {
        POINT pt = {ev.x, ev.y};
...
bool BrowserHandler::OnContextMenuCommand(
    ...
    if (handle_delegate_)
        return handle_delegate_->OnContextMenuCommand(browser, frame, params,
                                                      command_id, event_flags);
    else
        return false;
}

void BrowserHandler::OnContextMenuDismissed(CefRefPtr<CefBrowser> browser,
                                            CefRefPtr<CefFrame> frame) {}
...
void BrowserHandler::OnAddressChange(CefRefPtr<CefBrowser> browser,
    ...
    if (handle_delegate_)
        nbase::ThreadManager::PostTask(
            kThreadUI, nbase::Bind(&HandlerDelegate::OnAddressChange,
                                   handle_delegate_, browser, frame, url));
}
...
void BrowserHandler::OnTitleChange(CefRefPtr<CefBrowser> browser,
    ...
    if (handle_delegate_)
        nbase::ThreadManager::PostTask(
            kThreadUI, nbase::Bind(&HandlerDelegate::OnTitleChange,
                                   handle_delegate_, browser, title));
}
...
void BrowserHandler::OnLoadingStateChange(CefRefPtr<CefBrowser> browser,
    ...
    if (handle_delegate_)
        nbase::ThreadManager::PostTask(
            kThreadUI, nbase::Bind(&HandlerDelegate::OnLoadingStateChange,
                                   handle_delegate_, browser, isLoading,
...
void BrowserHandler::OnLoadStart(CefRefPtr<CefBrowser> browser,
    ...
    if (handle_delegate_)
        nbase::ThreadManager::PostTask(
            kThreadUI, nbase::Bind(&HandlerDelegate::OnLoadStart,
                                   handle_delegate_, browser, frame));
}
...
void BrowserHandler::OnLoadEnd(CefRefPtr<CefBrowser> browser,
    ...
    if (handle_delegate_)
        nbase::ThreadManager::PostTask(
            kThreadUI,
            nbase::Bind(&HandlerDelegate::OnLoadEnd, handle_delegate_, browser,
                        frame, httpStatusCode));
}
...
void BrowserHandler::OnLoadError(CefRefPtr<CefBrowser> browser,
    ...
    if (handle_delegate_)
        nbase::ThreadManager::PostTask(
            kThreadUI,
            nbase::Bind(&HandlerDelegate::OnLoadError, handle_delegate_,
...
bool BrowserHandler::OnBeforeBrowse(CefRefPtr<CefBrowser> browser,
                                    ...
                                    bool is_redirect) {
    if (handle_delegate_)
        return handle_delegate_->OnBeforeBrowse(browser, frame, request,
                                                is_redirect);

    return false;
}

void BrowserHandler::OnProtocolExecution(CefRefPtr<CefBrowser> browser,
                                         const CefString& url,
...
CefRequestHandler::ReturnValue BrowserHandler::OnBeforeResourceLoad(
    ...
    if (handle_delegate_)
        return handle_delegate_->OnBeforeResourceLoad(browser, frame, request,
                                                      callback);
...
void BrowserHandler::OnRenderProcessTerminated(CefRefPtr<CefBrowser> browser,
    ...
    if (handle_delegate_)
        nbase::ThreadManager::PostTask(
            kThreadUI, nbase::Bind(&HandlerDelegate::OnRenderProcessTerminated,
                                   handle_delegate_, browser, status));
}
...
void BrowserHandler::OnBeforeDownload(
    ...
    if (handle_delegate_)
        handle_delegate_->OnBeforeDownload(browser, download_item,
...
bool BrowserHandler::OnFileDialog(CefRefPtr<CefBrowser> browser,
    ...
    if (handle_delegate_)
        return handle_delegate_->OnFileDialog(browser, mode, title,
                                              default_file_path, accept_filters,
