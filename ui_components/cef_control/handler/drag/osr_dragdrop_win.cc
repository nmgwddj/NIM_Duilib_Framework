    ...
    if (tag_start == std::string::npos) {
        ...
        if (start_fragment_start != std::string::npos) {
            *fragment_start = static_cast<size_t>(
                atoi(cf_html.c_str() + start_fragment_start +
...
bool DragDataToDataObject(CefRefPtr<CefDragData> drag_data,
    ...
    if (bufferSize) {
        CefRefPtr<BytesWriteHandler> handler =
            new BytesWriteHandler(bufferSize);
            ...
            if (data_object->GetData(&rgelt[i], &medium) == S_OK) {
                ...
                if (format == CF_HDROP) {
                    ...
                    for (unsigned int x = 0; x < num_files; ++x) {
                        ...
                        if (!DragQueryFileW(hdrop, x, filename,
                                            kMaxFilenameLen))
                            continue;
                        WCHAR* name = wcsrchr(filename, '\\');
                        drag_data->AddFile(filename,
...
HRESULT DropTargetWin::DragEnter(IDataObject* data_object,
    ...
    CefBrowserHost::DragOperationsMask mask =
        DropEffectToDragOperation(*effect);
...
HRESULT DragEnumFormatEtc::Clone(IEnumFORMATETC** ppEnumFormatEtc) {
    ...
    if (hResult == S_OK) {
        ...
        reinterpret_cast<DragEnumFormatEtc*>(*ppEnumFormatEtc)->m_nIndex =
            m_nIndex;
    }
