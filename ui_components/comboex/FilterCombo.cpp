...
LRESULT CFilterComboWnd::HandleMessage(UINT uMsg,
                                       WPARAM wParam,
                                       LPARAM lParam) {
    if (uMsg == WM_CREATE) {
...
bool ListElementMatch::StringMatch(const std::string& utf8str) {
    ...
    if (utf8text.find(searchkey) != std::string::npos ||
        utf8date.find(searchkey) != std::string::npos) {
        return true;
    }
    return false;
}
...
FilterCombo::FilterCombo()
    ...
    m_pLayout->AttachSelect(
        nbase::Bind(&FilterCombo::OnSelectItem, this, std::placeholders::_1));

    m_pRichEdit = new RichEdit;
    m_pRichEdit->AttachTextChange(nbase::Bind(
        &FilterCombo::OnRichEditTextChanged, this, std::placeholders::_1));
    m_pRichEdit->AttachButtonDown(nbase::Bind(
        &FilterCombo::OnRichEditButtonDown, this, std::placeholders::_1));
...
bool FilterCombo::Add(Control* pControl) {
    ListElementMatch* pListElementMatch =
        dynamic_cast<ListElementMatch*>(pControl);
...
void FilterCombo::SetAttribute(const std::wstring& strName,
...
bool FilterCombo::OnRichEditTextChanged(EventArgs* args) {
    if (m_pLayout) {
        ...
        if (m_pComboWnd) {
            ...
            MONITORINFO oMonitor = {};
            oMonitor.cbSize = sizeof(oMonitor);
            ::GetMonitorInfo(::MonitorFromWindow(m_pComboWnd->GetHWND(),
