...
void CShadowComboWnd::Init(ShadowCombo* pOwner) {
    ...
    int iItemHeight =
        m_iOldSel > -1 ? pOwner->GetItemAt(m_iOldSel)->GetFixedHeight() : 0;
    ...
    if (listBox) {
        padding = listBox->GetLayout()->GetPadding().top +
                  listBox->GetLayout()->GetPadding().bottom;
    }
    ...
    Create(pOwner->GetWindow()->GetHWND(), NULL, WS_POPUP, WS_EX_TOOLWINDOW,
           true, rc);
...
LRESULT CShadowComboWnd::HandleMessage(UINT uMsg,
                                       WPARAM wParam,
                                       LPARAM lParam) {
    if (uMsg == WM_CREATE) {
        ...
        this->SetWindowResourcePath(
            m_pOwner->GetWindow()->GetWindowResourcePath());
        ...
        if (m_hWnd != (HWND)wParam) {
            ...
            m_pOwner->GetListBox()->PlaceHolder::SetWindow(nullptr, nullptr,
                                                           false);
        }
    }
...
ShadowCombo::ShadowCombo()
    ...
    , m_bPopupTop(false)
    , m_sShadowImage(
          L"file = '../public/bk/bk_combo_shadow.png' corner = '5,4,5,6'")
    , m_cArrow(nullptr)
    , m_bInit(false) {
    ...
    m_pLayout->AttachSelect(
        nbase::Bind(&ShadowCombo::OnSelectItem, this, std::placeholders::_1));
    ...
    m_cArrow->SetStateImage(kControlStateNormal,
                            L"../public/combo/arrow_normal.svg");
...
void ShadowCombo::DoInit() {
    ...
    if (!m_bInit) {
        ...
        AttachResize(ToWeakCallback([this](ui::EventArgs* args) {
            ...
            ui::CSize ArrrowSize = m_cArrow->EstimateSize(
                CSize(m_rcItem.GetWidth(), m_rcItem.GetHeight()));
            rect.top =
                m_rcItem.top + (m_rcItem.GetHeight() - ArrrowSize.cy) / 2;
...
UIAControlProvider* ShadowCombo::GetUIAProvider() {
    ...
    if (m_pUIAProvider == nullptr) {
        m_pUIAProvider = static_cast<UIAControlProvider*>(
            new (std::nothrow) UIAComboBoxProvider(this));
    }
...
void ShadowCombo::SetAttribute(const std::wstring& strName,
    ...
    if (strName == _T("dropbox"))
        ...
        CSize szDropBoxSize;
        LPTSTR pstr = NULL;
        szDropBoxSize.cx = _tcstol(strValue.c_str(), &pstr, 10);
        ASSERT(pstr);
        szDropBoxSize.cy = _tcstol(pstr + 1, &pstr, 10);
        ASSERT(pstr);
        SetDropBoxSize(szDropBoxSize);
    } else if (strName == _T("popuptop"))
        ...
        LPTSTR pstr = NULL;
        rcShadowCorner.left = _tcstol(strValue.c_str(), &pstr, 10);
        ASSERT(pstr);
        rcShadowCorner.top = _tcstol(pstr + 1, &pstr, 10);
        ASSERT(pstr);
        rcShadowCorner.right = _tcstol(pstr + 1, &pstr, 10);
        ASSERT(pstr);
        rcShadowCorner.bottom = _tcstol(pstr + 1, &pstr, 10);
        ASSERT(pstr);
...
void ShadowCombo::PaintText(IRenderContext* pRender) {
    ...
    if (m_iCurSel >= 0) {
        Control* pControl =
            static_cast<Control*>((m_pLayout->GetItemAt(m_iCurSel)));
        ListContainerElement* pElement =
            dynamic_cast<ListContainerElement*>(pControl);
        assert(pElement);
        if (!pElement)
            return;
        ...
        if (pElement) {
            ...
            if (rcPadding.left == 0 && rcPadding.top == 0 &&
                rcPadding.right == 0 && rcPadding.bottom == 0)
            ...
            dwTextColor = this->GetWindowColor(
                pElement->GetStateTextColor(kControlStateNormal));
...
std::wstring ShadowCombo::GetText() const {
    if (m_iCurSel < 0)
        return _T("");
    ListContainerElement* pControl =
        static_cast<ListContainerElement*>(m_pLayout->GetItemAt(m_iCurSel));
...
bool ShadowCombo::SelectItemInternal(int iIndex) {
    ...
    if (m_pUIAProvider != nullptr && UiaClientsAreListening()) {
        ...
        ListContainerElement* pControl =
            static_cast<ListContainerElement*>(m_pLayout->GetItemAt(m_iCurSel));
        vtOld.bstrVal =
            SysAllocString(pControl ? pControl->GetText().c_str() : L"");
