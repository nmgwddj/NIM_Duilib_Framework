...
CheckCombo::CheckCombo()
    ...
    m_pList->AttachButtonDown(
        std::bind(&CheckCombo::OnListButtonDown, this, std::placeholders::_1));
...
bool CheckCombo::Add(Control* pControl) {
    ...
    if (pCheckBox) {
        pCheckBox->AttachSelect(
            std::bind(&CheckCombo::OnSelectItem, this, std::placeholders::_1));
        pCheckBox->AttachUnSelect(std::bind(&CheckCombo::OnUnSelectItem, this,
                                            std::placeholders::_1));
    } else if (pCheckBoxBox = dynamic_cast<CheckBoxBox*>(pControl)) {
        pCheckBoxBox->AttachSelect(
            std::bind(&CheckCombo::OnSelectItem, this, std::placeholders::_1));
        pCheckBoxBox->AttachUnSelect(std::bind(&CheckCombo::OnUnSelectItem,
...
void CheckCombo::Activate() {
    ...
    if (m_pCheckComboWnd != NULL)
        m_pCheckComboWnd->SendNotify(this, kEventClick);
    Invalidate();
}

void CheckCombo::SetAttribute(const std::wstring& strName,
...
bool CheckCombo::SelectItem(int iIndex) {
    /*if (iIndex < 0 || iIndex >= m_pDropList->GetCount() || m_iCurSel ==
    iIndex) return false;
...
bool CheckCombo::OnUnSelectItem(EventArgs* args) {
    ...
    assert(std::find(m_vecDate.cbegin(), m_vecDate.cend(), date) !=
           m_vecDate.cend());
    if (std::find(m_vecDate.cbegin(), m_vecDate.cend(), date) !=
        m_vecDate.cend()) {
