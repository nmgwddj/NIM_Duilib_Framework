...
// 示例_T("Text Files(*.txt)\0*.txt\0网页文件\0*.htm;*.html\0All
// Files(*.*)\0*.*\0\0")
...
void CFileDialogEx::AyncShowOpenFileDlg(
    FileDialogCallback2 file_dialog_callback2) {
    file_dialog_callback2_ = file_dialog_callback2;
    file_dialog_type_ = FDT_OpenFile;
    AsyncDoModal(this);
}

void CFileDialogEx::AyncShowSaveFileDlg(
    FileDialogCallback2 file_dialog_callback2) {
    file_dialog_callback2_ = file_dialog_callback2;
...
void CFileDialogEx::AsyncShowSelectFolderDlg(
    FileDialogCallback2 file_dialog_callback2) {
    file_dialog_callback2_ = file_dialog_callback2;
...
void CFileDialogEx::SyncShowModal() {
    if (file_dialog_type_ == FDT_OpenFile) {
        ...
        if ((m_stOFN.Flags & OFN_ALLOWMULTISELECT) > 0) {
            ...
            if (file_name.size() == 0) {
                StdClosure closure =
                    nbase::Bind(file_dialog_callback2_, ret, file_directory);
                ...
                while (file_name.size() > 0) {
                    ...
                    if (nbase::FilePathIsExist(file_path, false)) {
                        StdClosure closure =
                            nbase::Bind(file_dialog_callback2_, ret, file_path);
                        nbase::ThreadManager::PostTask(kThreadUI, closure);
                    }
            ...
            StdClosure closure =
                nbase::Bind(file_dialog_callback2_, ret, GetPathName());
            nbase::ThreadManager::PostTask(kThreadUI, closure);
        }
        ...
        if (::SHGetPathFromIDList(idl, m_stOFN.lpstrFile)) {
            StdClosure closure =
                nbase::Bind(file_dialog_callback2_, true, GetPathName());
            nbase::ThreadManager::PostTask(kThreadUI, closure);
        } else {
            StdClosure closure =
                nbase::Bind(file_dialog_callback2_, false, GetPathName());
            nbase::ThreadManager::PostTask(kThreadUI, closure);
        }
