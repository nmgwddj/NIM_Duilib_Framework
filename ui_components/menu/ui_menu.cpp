...
void CMenuWnd::Init(STRINGorID xml,
                    LPCTSTR pSkinType,
                    POINT point,
                    PopupPosType popupPosType,
                    bool no_focus,
    ...
    Create(m_hParent, L"NIM_DUILIB_MENU_WINDOW", WS_POPUP,
           WS_EX_TOOLWINDOW | WS_EX_TOPMOST, true, UiRect());
...
void CMenuWnd::DetouchOwner() {
    if (m_pOwner) {
        ...
        for (int i = 0; i < m_pOwner->GetSubMenuItemCount(); i++) {
            CMenuElementUI* pItem =
                static_cast<CMenuElementUI*>(m_pOwner->GetSubMenuItemAt(i));
            if (pItem) {
    ...
    switch (uMsg) {
        case WM_KILLFOCUS: {
            ...
            ContextMenuObserver::Iterator<BOOL, ContextMenuParam> iterator(
                GetMenuObserver());
            ReceiverImplBase<BOOL, ContextMenuParam>* pReceiver =
                iterator.next();
        ...
        case WM_KEYDOWN:
            if (wParam == VK_ESCAPE || wParam == VK_LEFT)
                ...
                if (m_pLayout) {
                    ...
                    CMenuElementUI* pItem = dynamic_cast<CMenuElementUI*>(
                        m_pLayout->GetItemAt(index));
                    if (pItem) {
...
void CMenuWnd::ResizeMenu() {
    ...
    CSize szAvailable = {rcWork.right - rcWork.left,
    ...
    szInit.cy -= rcCorner.top + rcCorner.bottom;  //这里去掉阴影窗口，即用户的视觉有效面积
                                                  //szInit<=szAvailable

    CPoint point =
        m_BasedPoint;  //这里有个bug，由于坐标点与包含在窗口内，会直接出发mouseenter导致出来子菜单，偏移1个像素
    ...
    SetWindowPos(m_hWnd, HWND_TOPMOST, point.x - rcCorner.left,
                 point.y - rcCorner.top, szAvailable.cx, szAvailable.cy,
...
void CMenuWnd::ResizeSubMenu() {
    ...
    for (int it = 0; it < m_pLayout->GetCount(); it++) {
        //取子菜单项中的最大值作为菜单项
        CMenuElementUI* pItem =
            dynamic_cast<CMenuElementUI*>(m_pLayout->GetItemAt(it));
        if (pItem) {
    ...
    while (pReceiver != NULL) {
        CMenuWnd* pContextMenu = dynamic_cast<CMenuWnd*>(pReceiver);
        if (pContextMenu != NULL) {
            GetWindowRect(pContextMenu->GetHWND(),
                          &rcPreWindow);  //需要减掉阴影
    ...
    SetWindowPos(m_hWnd, HWND_TOPMOST, rc.left - rcCorner.left,
                 rc.top - rcCorner.top, rc.right - rc.left, rc.bottom - rc.top,
                 SWP_SHOWWINDOW);
...
void CMenuWnd::Show() {
    ...
    SetPos(rc, false, SWP_SHOWWINDOW | (no_focus_ ? SWP_NOACTIVATE : 0),
           HWND_TOPMOST, false);
    if (!no_focus_)
        SetForegroundWindow(m_hWnd);
}
...
void CMenuWnd::InitWindow() {
    if (m_pOwner) {
        ...
        for (int i = 0; i < m_pOwner->GetSubMenuItemCount(); i++) {
            CMenuElementUI* subMenuItem =
                dynamic_cast<CMenuElementUI*>(m_pOwner->GetSubMenuItemAt(i));
            subMenuItem->SetInternVisible(true);  // add by djj 20200506
            if (subMenuItem && subMenuItem->IsVisible()) {
                m_pLayout->Add(
                    subMenuItem);  //内部会调用subMenuItem->SetOwner(m_pLayout);
                                   //会调用SetWindows，改变了归属窗口、父控件。
            }
        }
    } else {
...
bool CMenuElementUI::Add(Control* pControl) {
    ...
    if (std::find(m_child_menus.cbegin(), m_child_menus.cend(), pMenuItem) !=
        m_child_menus.cend()) {
        assert(0);
        return false;
    }
...
bool CMenuElementUI::AddSubMenuItemAt(CMenuElementUI* pMenuItem,
                                      std::size_t iIndex) {
    if (pMenuItem == NULL)
        return false;
...
bool CMenuElementUI::MouseEnter(EventArgs& msg) {
    ...
    if (ret && !weakFlag.expired()) {
        ...
        if (!CheckSubMenuItem()) {
            ...
            // 有些老版本attachselect会触发 这里得把之前选中的置为未选中
...
void CMenuElementUI::PaintChild(IRenderContext* pRender,
...
void CMenuElementUI::CreateMenuWnd() {
    ...
    m_pSubWindow->Init(_T("submenu.xml"), _T(""), CPoint(),
                       CMenuWnd::RIGHT_BOTTOM, false, this);
}
