...
public:
    enum PopupPosType  //鼠标点击的point属于菜单的哪个位置    1.-----.2 1左上
                       //2右上
    {  //      |     |
       //这里假定用户是喜欢智能的                            3.-----.4 3左下
       //4右下
        RIGHT_BOTTOM = eMenuAlignment_Right | eMenuAlignment_Bottom |
                       eMenuAlignment_Intelligent,
        RIGHT_TOP = eMenuAlignment_Right | eMenuAlignment_Top |
                    eMenuAlignment_Intelligent,
        LEFT_BOTTOM = eMenuAlignment_Left | eMenuAlignment_Bottom |
                      eMenuAlignment_Intelligent,
        LEFT_TOP = eMenuAlignment_Intelligent | eMenuAlignment_Top |
                   eMenuAlignment_Intelligent,
        //这里是normal，非智能的
    ...
    void Init(STRINGorID xml,
              LPCTSTR pSkinType,
              POINT point,
              PopupPosType popupPosType = LEFT_TOP,
              bool no_focus = false,
              CMenuElementUI* pOwner = NULL);
    void Show();
    // 重新调整菜单的大小
    void ResizeMenu();
    // 重新调整子菜单的大小
    ...
    virtual std::wstring GetSkinFolder() override { return L"menu"; }
    virtual std::wstring GetSkinFile() override { return m_xml.m_lpstr; }
...
public:
    ...
    virtual bool AddSubMenuItemAt(CMenuElementUI* pMenuItem,
                                  std::size_t iIndex);
