...
public:
    ...
    // 监听关闭按钮点击事件
    void AttachCloseClicked(const ui::EventCallback& callback) {
        OnClose += callback;
    }
...
public:
    ...
    void AttachCancelClicked(const ui::EventCallback& callback) {
        OnCancel += callback;
    }
    void AttachOkClicked(const ui::EventCallback& callback) {
        OnOk += callback;
    }
...
public:
    std::wstring GetType() const override { return L"PopoverRoot"; }
};
...
public:
    ...
    // 更新弹窗内容
    virtual void Update(PopoverHeader* header,
                        PopoverBody* body,
                        PopoverFooter* footer);
    ...
    // 监听结果回调
    void BindResultCallback(const PopoverCallback& callback) {
        m_cbResult += callback;
    }

    // 监听关闭回调
    void BindCloseCallback(const PopoverCallback& callback) {
        m_cbClose += callback;
    }
...
protected:
    ...
    // 初始化展示和关闭触发器
    bool HasTrigger(int nType, PopoverTriggerType nTrigger) {
        return nType & nTrigger;
    }
    virtual void InitializeShowTriggers(int nOldType,
                                        int nNewType,
                                        int nTimeouts = 0);
    virtual void InitializeDisappearTriggers(int nOldType,
                                             int nNewType,
                                             int nTimeouts = 0);
    ...
    // 触发关闭弹窗
    virtual void TriggerResult(const PopoverResult& result,
...
public:
    PopoverHolderLayer(std::wstring type,
                       ui::Layout* pLayout = new ui::Layout())
        : ui::Box(pLayout)
        , m_strType(type) {
...
public:
    std::wstring GetType() const override { return m_strType; }
...
public:
    NotificationHolderLayer(ui::Control* pAnchor)
        : PopoverHolderLayer(PopoverHolderLayer::kHolderTypeNotification,
                             new ui::VLayout())
        , m_pAnchor(pAnchor)
        , m_bInited(false) {}
    ...
    void DoInit() override {
        ...
        m_pAnchor->AttachResize(
            nbase::Bind(&NotificationHolderLayer::OnAnchorResize, this,
                        std::placeholders::_1));
...
public:
     ...
     * @param[in] nButtons 按钮类型组合，kButtonOk | kButtonCancel
     * 表示显示确认和取消按钮
     ...
     * @param[in] strContent 内容，不需要则为空
     * @param[in] nPlacement 弹出方向，默认为锚点控件外和锚点控件上
     * @param[in] nButtons 按钮类型组合，kButtonOk | kButtonClose
     * 表示显示确认和关闭按钮
     * @param[in] nShowType 自动显示的触发方式，kTriggerNone表示不自动触发
     * @param[in] nDisappearType 自动关闭的触发方式，kTriggerNone表示不自动触发
     * @param[in] bRemoveOnClose 关闭后是否删除
