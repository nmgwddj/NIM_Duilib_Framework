...
ui::HorAlignType PopoverArrow::GetHorAlignType() {
    ...
    if (m_nPlacement & kPlaceTopLeft || m_nPlacement & kPlaceRightTop ||
        m_nPlacement & kPlaceRight || m_nPlacement & kPlaceRightBottom ||
        m_nPlacement & kPlaceBottomLeft)
        return ui::kHorAlignLeft;
    else if (m_nPlacement & kPlaceTop || m_nPlacement & kPlaceBottom)
...
ui::VerAlignType PopoverArrow::GetVerAlignType() {
    // coz arrow is always point to anchor
    if (m_nPlacement & kPlaceTopLeft || m_nPlacement & kPlaceTop ||
        m_nPlacement & kPlaceTopRight || m_nPlacement & kPlaceLeftBottom ||
        m_nPlacement & kPlaceRightBottom)
        return ui::kVerAlignBottom;
    else if (m_nPlacement & kPlaceLeft || m_nPlacement & kPlaceRight)
...
ui::UiRect PopoverArrow::GetMarginByPlacement() {
    ...
    // coz arrow is always point to anchor
    if (m_nPlacement & kPlaceLeftTop || m_nPlacement & kPlaceLeft ||
    ...
    if (m_nPlacement & kPlaceRightTop || m_nPlacement & kPlaceRight ||
        m_nPlacement & kPlaceRightBottom) {
        auto marginLeft = m_rcArea.right - GetFixedWidth() + 1;
        margin = {marginLeft > 0 ? marginLeft : 0, kMargin, 0, kMargin};
    }

    if (m_nPlacement & kPlaceBottomLeft || m_nPlacement & kPlaceBottom ||
...
void PopoverArrow::CalcTrianglePoints() {
    ...
    if (m_nPlacement & kPlaceLeftTop || m_nPlacement & kPlaceLeft ||
        ...
        m_ptTriangle[2] = {rcPos.right,
                           (rcPos.bottom - rcPos.top) / 2 + rcPos.top};
    }
    ...
    if (m_nPlacement & kPlaceTopLeft || m_nPlacement & kPlaceTop ||
        ...
        m_ptTriangle[2] = {(rcPos.right - rcPos.left) / 2 + rcPos.left,
                           rcPos.bottom};
    }
...
// PopoverHeader
PopoverHeader::PopoverHeader(const std::wstring& strTitle,
                             bool bShowClose,
...
ui::CSize PopoverHeader::EstimateSize(ui::CSize szAvailable) {
    ...
    if (GetFixedWidth() == DUI_LENGTH_AUTO ||
        ...
        if (m_pRichEditTitle) {
            ...
            editSize = m_pRichEditTitle->EstimateText(
                {editMaxSize.cx - editMargin.left - editMargin.right,
                 editMaxSize.cy});

            if (m_bUseMaxSize) {
                m_pRichEditTitle->SetFixedWidth(
                    editMaxSize.cx - editMargin.left - editMargin.right, true,
                    false);
...
// PopoverBody
PopoverBody::PopoverBody(const std::wstring& content,
                         const std::wstring& colorId)
    : m_bInited(false)
    , m_bUseMaxSize(false)
...
ui::CSize PopoverBody::EstimateSize(ui::CSize szAvailable) {
    ...
    if (GetFixedWidth() == DUI_LENGTH_AUTO ||
        ...
        if (m_pRichEditContent) {
            ...
            editSize = m_pRichEditContent->EstimateText(
                {maxSize.cx - editMargin.left - editMargin.right, maxSize.cy});
...
ui::CSize PopoverFooter::EstimateSize(ui::CSize szAvailable) {
    ...
    if (GetFixedWidth() == DUI_LENGTH_AUTO ||
        ...
        fixedSize.cx = okSize.cx + cancelSize.cx + paddingRect.left +
                       paddingRect.right + (m_items.size() - 1) * childMargin;

        fixedSize.cy = max(okSize.cy, cancelSize.cy) + paddingRect.top +
                       paddingRect.bottom;
        if (m_bUseMaxSize && fixedSize.cx < maxSize.cx)
            fixedSize.cx = maxSize.cx;
...
void PopoverFooter::DoInit() {
    ...
    if (m_strCancel.length()) {
        ...
        m_pButtonCancel->AttachClick(
            ToWeakCallback([this](ui::EventArgs* args) {
...
ui::CSize PopoverRoot::EstimateSize(ui::CSize szAvailable) {
    ...
    if (GetFixedWidth() == DUI_LENGTH_AUTO ||
        ...
        for (auto it = m_items.begin(); it != m_items.end(); it++) {
            ...
            if ((*it)->GetFixedWidth() == DUI_LENGTH_AUTO ||
                (*it)->GetFixedHeight() == DUI_LENGTH_AUTO) {
...
ui::CSize Popover::EstimateSize(ui::CSize szAvailable) {
    ...
    if (GetFixedWidth() == DUI_LENGTH_AUTO ||
        ...
        maxSize.cx -= paddingRect.left + paddingRect.right;
        ui::CSize sizeRoot =
            m_pLayout->AjustSizeByChild({m_pPopoverRoot}, maxSize);
...
void Popover::DoInit() {
    ...
    InitializeDisappearTriggers(kTriggerNone, m_nDisappearType,
                                m_nDisappearTimeouts);
...
void Popover::Update(PopoverHeader* header,
                     PopoverBody* body,
                     PopoverFooter* footer) {
    ...
    if (header) {
        ...
        m_pPopoverHeader->AttachCloseClicked(
            ToWeakCallback([this](ui::EventArgs* args) {
    ...
    if (footer) {
        ...
        m_pPopoverFooter = footer;
        m_pPopoverFooter->AttachCancelClicked(
            ToWeakCallback([this](ui::EventArgs* args) {
                nbase::ThreadManager::PostTask(
                    kThreadUI, ToWeakCallback([this]() {
                        TriggerResult({kResultCancel});
                    }));
                return true;
            }));
        m_pPopoverFooter->AttachOkClicked(
            ToWeakCallback([this](ui::EventArgs* args) {
                nbase::ThreadManager::PostTask(kThreadUI,
                                               ToWeakCallback([this]() {
                                                   TriggerResult({kResultOk});
                                               }));
                return true;
            }));
...
void Popover::UpdateNotification(const std::wstring& strTitle,
...
ui::HorAlignType Popover::GetHorAlignType() {
    if (m_nPlacement & kPlaceLeft || m_nPlacement & kPlaceLeftTop ||
        m_nPlacement & kPlaceLeftBottom || m_nPlacement & kPlaceTopLeft ||
        m_nPlacement & kPlaceBottomLeft)
        return ui::kHorAlignLeft;

    if (m_nPlacement & kPlaceRightTop || m_nPlacement & kPlaceRight ||
        m_nPlacement & kPlaceRightBottom || m_nPlacement & kPlaceTopRight ||
        m_nPlacement & kPlaceBottomRight)
        return ui::kHorAlignRight;
...
ui::VerAlignType Popover::GetVerAlignType() {
    if (m_nPlacement & kPlaceTopLeft || m_nPlacement & kPlaceTop ||
        m_nPlacement & kPlaceTopRight || m_nPlacement & kPlaceRightTop ||
        m_nPlacement & kPlaceLeftTop)
        return ui::kVerAlignTop;

    if (m_nPlacement & kPlaceBottomLeft || m_nPlacement & kPlaceBottom ||
        m_nPlacement & kPlaceBottomRight || m_nPlacement & kPlaceLeftBottom ||
        m_nPlacement & kPlaceRightBottom)
        return ui::kVerAlignBottom;
...
ui::UiRect Popover::ReCalcByAnchorPos(ui::UiRect rc) {
    ...
    if (bIsInner) {
        ...
        } else if (m_nPlacement & kPlaceTopLeft || m_nPlacement & kPlaceTop ||
                   m_nPlacement & kPlaceTopRight) {
        ...
        if (m_nPlacement & kPlaceLeftTop || m_nPlacement & kPlaceRightTop)
            rc.top = anchorPos.top;
        else if (m_nPlacement & kPlaceLeft || m_nPlacement & kPlaceRight)
        ...
        if (bHasArrow) {
            ...
            if (m_nPlacement & kPlaceLeftTop || m_nPlacement & kPlaceRightTop) {
                ...
                verOffset -= size.cy / 2;
                       ...
                       m_nPlacement & kPlaceRightBottom) {
                int offset =
                    (PopoverArrow::kMargin + PopoverArrow::kHeight / 2);
            ...
            if (m_nPlacement & kPlaceTopLeft ||
                ...
                int offset = (PopoverArrow::kMargin + PopoverArrow::kWidth / 2);
                horOffset -=
                    size.cx - ui::DpiManager::GetInstance()->ScaleInt(offset);
            }
        }
    }
...
void Popover::InitializePosition() {
    if (m_pAnchor) {
        m_pAnchor->AttachResize(
            nbase::Bind(&Popover::OnAnchorResize, this, std::placeholders::_1));
        m_pAnchor->AttachVisibleChange(nbase::Bind(
            &Popover::OnAnchorVisibleChange, this, std::placeholders::_1));
        m_pAnchor->AttachLastEvent(nbase::Bind(&Popover::OnAnchorLastEvent,
...
void Popover::InitializeShowTriggers(int nOldType,
                                     int nNewType,
                                     int nTimeouts) {
    if (m_pAnchor) {
        if (!HasTrigger(nOldType, kTriggerSetFocus) &&
            HasTrigger(nNewType, kTriggerSetFocus))
            m_pAnchor->AttachSetFocus(nbase::Bind(&Popover::OnAnchorSetFocus,
                                                  this, std::placeholders::_1));
        if (!HasTrigger(nOldType, kTriggerClick) &&
            HasTrigger(nNewType, kTriggerClick))
            m_pAnchor->AttachButtonDown(nbase::Bind(
                &Popover::OnAnchorMouseClick, this, std::placeholders::_1));
        if (!HasTrigger(nOldType, kTriggerEnter) &&
            HasTrigger(nNewType, kTriggerEnter))
            m_pAnchor->AttachMouseEnter(nbase::Bind(
                &Popover::OnAnchorMouseEnter, this, std::placeholders::_1));
        if (!HasTrigger(nOldType, kTriggerHover) &&
            HasTrigger(nNewType, kTriggerHover))
            m_pAnchor->AttachMouseHover(nbase::Bind(
                &Popover::OnAnchorMouseHover, this, std::placeholders::_1));
    }

    if (!HasTrigger(nOldType, kTriggerImmediately) &&
        HasTrigger(nNewType, kTriggerImmediately)) {
        SetVisible(true);
    }
...
void Popover::InitializeDisappearTriggers(int nOldType,
                                          int nNewType,
                                          int nTimeouts) {
    if (m_pAnchor) {
        if (!HasTrigger(nOldType, kTriggerKillFocus) &&
            HasTrigger(nNewType, kTriggerKillFocus))
            m_pAnchor->AttachKillFocus(nbase::Bind(
                &Popover::OnAnchorKillFocus, this, std::placeholders::_1));
        if (!HasTrigger(nOldType, kTriggerLeave) &&
            HasTrigger(nNewType, kTriggerLeave))
            m_pAnchor->AttachMouseLeave(nbase::Bind(
                &Popover::OnAnchorMouseLeave, this, std::placeholders::_1));
    }
...
void Popover::OnShowTriggerTimeout(int index) {
    if (index != m_nShowTriggerIndex) {
        int index = m_nShowTriggerIndex;
        nbase::ThreadManager::PostDelayedTask(
            kThreadUI, nbase::Bind(&Popover::OnShowTriggerTimeout, this, index),
            nbase::TimeDelta::FromMilliseconds(m_nShowTimeouts));
        return;
    }
...
// PopoverHolderLayer
const std::wstring PopoverHolderLayer::kHolderTypeNotification =
...
void PopoverLayer::DoInit() {
    ...
    m_pPopoverLayer =
        new PopoverHolderLayer(PopoverHolderLayer::kHolderTypePopover);
    Add(m_pPopoverLayer);

    m_pAlertLayer =
        new PopoverHolderLayer(PopoverHolderLayer::kHolderTypeAlert);
    Add(m_pAlertLayer);
...
void PopoverLayer::OnMouseEventButtonDown(POINT pt) {
    auto trigger_loop = [this](PopoverHolderLayer* holder, POINT pt) {
        for (int index = 0; index < holder->GetCount(); index++) {
            ...
            auto control =
                m_pWindow->FindSubControlByPoint(m_pWindow->GetRoot(), pt);

            auto pos = popover->GetPos();
            if (!::PtInRect(&pos, pt) &&
                (!(popover->m_nShowType & kTriggerClick) ||
                 popover->GetAnchor() != control) &&
                popover->m_nDisappearType & kTriggerClickLayer) {
                nbase::ThreadManager::PostTask(
                    kThreadUI, ToWeakCallback([this, popover]() {
...
Popover* PopoverLayer::CreatePopover(ui::Control* pAnchor,
    ...
    if (!body && strContent.length()) {
        body = new PopoverBody(strContent,
                               header ? L"" : L"popover_content_color1");
    }
    ...
    if (!footer && (nButtons & kButtonOk | nButtons & kButtonCancel)) {
        ...
        footer = new PopoverFooter(
            nButtons & kButtonOk
                ? (strOk.length() ? strOk
                                  : pMutiLan->GetStringViaID(L"STRING_OK"))
                : L"",
            nButtons & kButtonCancel
                ? (strCancel.length()
                       ? strCancel
                       : pMutiLan->GetStringViaID(L"STRING_CANCEL"))
                : L"");
    ...
    auto popover = new Popover(pAnchor, nPlacement, nShowType, nDisappearType,
                               bRemoveOnClose, nShowTimeouts,
                               nDisappearTimeouts, header, body, footer, arrow);
...
Popover* PopoverLayer::CreateAlert(ui::Control* pAnchor,
    ...
    auto popover = CreatePopover(pAnchor, strTitle, strContent, strOk,
                                 strCancel, nPlacement, nButtons, nShowType,
                                 nDisappearType, false, bRemoveOnClose, 0, 0,
                                 pCustomHeader, pCustomBody, footer, nullptr);
...
Popover* PopoverLayer::CreateNotification(ui::Control* pAnchor,
    ...
    ASSERT(strTitle.length() || strContent.length() || pCustomHeader ||
           pCustomBody);

    // Create header
    if (!header && (strTitle.length() || nButtons & kButtonClose)) {
        header = static_cast<PopoverHeader*>(new NotificationHeader(
            strTitle, nButtons & kButtonClose, nIconType));
    }
    ...
    auto popover = new Notification(
        pAnchor, nPlacement, nShowType, nDisappearType, bRemoveOnClose,
        nShowTimeouts, nDisappearTimeouts, header, body);
...
Popover* PopoverLayer::CreateToolltip(ui::Control* pAnchor,
    ...
    auto popover = CreatePopover(pAnchor, L"", strContent, L"", L"", nPlacement,
                                 kButtonNone, nShowType, nDisappearType, false,
                                 false, 0, 0, nullptr, body, nullptr, nullptr);
