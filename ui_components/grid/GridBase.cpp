...
void GridSelRange::Clear() {
    bool bFlash = (m_vecRange.size() > 0 || m_mapSelRow.size() > 0 ||
                   m_mapSelCol.size() > 0);
...
void GridSelRange::ClearContent() {
    ...
    if (m_mapSelCol.size() > 0) {
        for (int row = fixed_row_count; row < row_count; row++) {
            GridRow* pRow = m_pBody->m_vecRow[row];
            for (auto it = m_mapSelCol.cbegin(); it != m_mapSelCol.cend();
                 it++) {
...
void GridSelRange::SetSelAll() {
    if (m_bSelAll) {
        ...
        if (fixed_row_count == row_count || fixed_col_count == col_count)
            return;
        UiRect rc(fixed_col_count, fixed_row_count, col_count - 1,
                  row_count - 1);
...
void GridSelRange::SetSelItem(int row_index,
                              int col_index,
                              bool ctrl,
                              bool shift) {
    ...
    if (ctrl) {
        ...
        if (m_mapSelRow.find(row_index) !=
            m_mapSelRow.end())  // row_index为选中状态
        {
            assert(bSelected);
            bInSelRow = true;
        ...
        if (m_mapSelCol.find(col_index) !=
            m_mapSelCol.end())  // col_index为选中状态
        {
            assert(bSelected);
            bInSelCol = true;
        ...
        if (rc.left > col_index || rc.right < col_index || rc.top > row_index ||
            rc.bottom < row_index)  //不位于rc中
        {
            // assert(!bSelected);
        ...
        Clear();
        m_vecRange.push_back(
            UiRect(col_index, row_index, col_index, row_index));
        m_pBody->_SelItem(row_index, col_index, true);
        m_pBody->Invalidate();
    }
    if (m_bSelAll)
...
void GridSelRange::SetSelItemRange(UiRect rc, bool ctrl, bool shift) {
    ...
    if (!ctrl) {
        ...
        assert(m_vecRange.size() >
               0);  // ButtonDown时应该调用了SetSelItem, 给m_vecRange赋值
        if (m_vecRange.size() == 0) {
            m_vecRange.push_back(rc);
            m_pBody->_SelRange(rc, true);
            m_pBody->Invalidate();
...
void GridSelRange::SetSelRowRange(int row_index,
                                  int row_index_end,
                                  bool ctrl,
                                  bool shift) {
    assert(m_vecRange.size() < 2);
...
void GridSelRange::SetSelColRange(int col_index,
                                  int col_index_end,
                                  bool ctrl,
                                  bool shift) {
    assert(m_vecRange.size() < 2);
...
void GridSelRange::CtrlCorX(bool cut) {
    ...
    if (::OpenClipboard(NULL)) {
        ...
        HGLOBAL hClip = ::GlobalAlloc(
            GMEM_MOVEABLE, (strContent.length() + 1) * sizeof(wchar_t));
        if (NULL == hClip) {
            ::CloseClipboard();
            return;
        }
        ...
        if (cut) {
            for (int row_index = rc.top; row_index <= rc.bottom; row_index++) {
                GridRow* pRow = m_pBody->m_vecRow[row_index];
                for (int col_index = rc.left; col_index <= rc.right;
                     col_index++) {
                    pItem = pRow->at(col_index);
                    assert(pItem);
                    if (pItem && pItem->IsSelected())
