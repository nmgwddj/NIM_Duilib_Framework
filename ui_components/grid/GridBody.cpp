...
void GridBody::_BeginEditGridItem(GridItem* item) {
    ...
    for (size_t j = 0; j < col_index; j++)
        posx += m_hLayout[j];
    if (item->type == GIT_String || item->type == GIT_Int ||
        item->type == GIT_Double) {
    ...
    if (m_pReEditGridItem) {
        ...
        if (m_pReEditGridItem->text != m_pReEdit->GetText()) {
            ...
            m_pWindow->SendNotify(this, kEventTextChange,
                                  m_pReEditGridItem->row_index,
                                  m_pReEditGridItem->col_index);
        }
...
bool GridBody::_GetGridItemByMouse(CPoint pt, CPoint& position, bool fixed) {
    ...
    UiRect rcFixedRow(
        {fixed_col_width, 0, m_pGrid->GetWidth(), fixed_row_height + 1});
    UiRect rcFixedCol(
        {0, fixed_row_height, fixed_col_width + 1, m_pGrid->GetHeight()});
    if (rcFixed.IsPointIn(pt) || rcFixedRow.IsPointIn(pt) ||
        rcFixedCol.IsPointIn(pt))  // in position of fixed row or fixed col
    {
        bool bFind = false;
        if (fixed) {
            int posx = 0, posy = 0;
    ...
    assert(pt.x - fixed_col_width > 0 && pt.y - fixed_row_height > 0 &&
           pt.x < m_pGrid->GetWidth() && pt.y < m_pGrid->GetHeight());
    if (pt.x - fixed_col_width > 0 && pt.y - fixed_row_height > 0 &&
        pt.x < m_pGrid->GetWidth() && pt.y < m_pGrid->GetHeight()) {
...
UiRect GridBody::_GetGridItemPos(int row_index, int col_index) {
    ...
    return UiRect(posx, posy, posx + m_hLayout[col_index],
                  posy + m_vLayout[row_index]);
}
...
GridBody::GridBody(Grid* pGrid)
    ...
    m_pComboEdit->SetVisible_(false);
    m_pComboEdit->AttachSelect(nbase::Bind(&GridBody::OnComboEditSelected, this,
                                           std::placeholders::_1));
    Add(m_pComboEdit);
...
void GridBody::SetColCount(int count) {
    ...
    if (count > col_count) {
        ...
        for (size_t i = 0; i < row_count; i++) {
            ...
            delay_delete_items.insert(delay_delete_items.end(),
                                      m_vecRow[i]->items.begin() + count,
                                      m_vecRow[i]->items.end());
            if (i == 0) {
                for (size_t j = 0; j < delay_delete_items.size(); j++) {
                    GridHeaderItem* pHeaderItem =
                        dynamic_cast<GridHeaderItem*>(delay_delete_items[j]);
            ...
            m_vecRow[i]->items.erase(m_vecRow[i]->items.begin() + count,
                                     m_vecRow[i]->items.end());
        }
        ...
        std::thread thread_delete([delay_delete_items]() {
            int index = 0;
            for (auto it = delay_delete_items.cbegin();
                 it < delay_delete_items.cend(); it++, index++) {
...
void GridBody::SetRowCount(int count) {
    ...
    if (count > row_count) {
        ...
        delay_delete_rows.insert(delay_delete_rows.begin(),
                                 m_vecRow.begin() + count, m_vecRow.end());
...
bool GridBody::SetGridItemText(std::wstring text,
                               int row_index,
                               int col_index) {
    bool ret = false;
    GridItem* item = GetGridItem(row_index, col_index);
...
GridItem* GridBody::GetGridItem(int row_index, int col_index) {
    assert(GetHeader()->size() == m_hLayout.size() &&
           m_vecRow.size() == m_vLayout.size());
    GridItem* item = nullptr;
    if (row_index >= 0 && row_index < m_vecRow.size() && col_index >= 0 &&
        col_index < m_vecRow[row_index]->size()) {
    ...
    if (col_index > 0 && col_index < GetHeader()->size()) {
        ...
        for (auto it = m_vecRow.begin(); it != m_vecRow.end(); it++) {
            ...
            if (it == m_vecRow.begin()) {
                GridHeaderItem* pHeaderItem =
                    dynamic_cast<GridHeaderItem*>(delay_delete_items[0]);
                if (pHeaderItem && pHeaderItem->control_) {
                    Remove(pHeaderItem->control_);
                }
                assert(GetCount() == GRIDBODY_CHILD_COUNT);
        ...
        if (book->load(file.c_str())) {
            ...
            for (int s = 0; s < sheet_count; s++) {
                ...
                if (pSheet) {
                    ...
                    int additional_header_row =
                        (touch_header ? 0 : 1);  //计算额外的一行
                    ...
                    for (int row = 0; row < row_num; ++row) {
                        for (int col = 0; col < col_num; ++col) {
                            ...
                            if (t == libxl::CellType::CELLTYPE_EMPTY ||
                                t == libxl::CellType::CELLTYPE_BLANK) {
                                ...
                                /*enum NumFormat {
                                NUMFORMAT_GENERAL, NUMFORMAT_NUMBER,
                                NUMFORMAT_NUMBER_D2, NUMFORMAT_NUMBER_SEP,
                                ...
                                NUMFORMAT_ACCOUNT, NUMFORMAT_ACCOUNTCUR,
                                NUMFORMAT_ACCOUNT_D2, NUMFORMAT_ACCOUNT_D2_CUR,
                                NUMFORMAT_CUSTOM_MMSS, NUMFORMAT_CUSTOM_H0MMSS,
                                ...
                                TCHAR buf[128] = {};
                                if (num_format == libxl::NUMFORMAT_GENERAL ||
                                    num_format == libxl::NUMFORMAT_NUMBER ||
                                    num_format == libxl::NUMFORMAT_NUMBER_SEP) {
                                    str = dealNumberD2(num);
                                } else if (num_format ==
                                               libxl::NUMFORMAT_NUMBER_D2 ||
                                           num_format ==
                                               libxl::NUMFORMAT_NUMBER_SEP_D2) {
                                    str = dealNumberD2(num);
                                } else if (num_format ==
                                               libxl::NUMFORMAT_PERCENT ||
                                           num_format ==
                                               libxl::NUMFORMAT_PERCENT_D2) {
                                    swprintf_s(buf, 128, L"%.2f", num * 100);
                                    str = buf;
                                } else if (num_format ==
                                               libxl::NUMFORMAT_DATE ||
                                           num_format ==
                                               libxl::
                                                   NUMFORMAT_CUSTOM_D_MON_YY ||
                                           num_format ==
                                               libxl::NUMFORMAT_CUSTOM_D_MON ||
                                           num_format ==
                                               libxl::NUMFORMAT_CUSTOM_MON_YY) {
                                    int y = 0, m = 0, d = 0;
                                    if (book->dateUnpack(num, &y, &m, &d)) {
                                        wchar_t buf[32] = {};
                                        swprintf_s(buf, L"%04d-%02d-%02d", y, m,
                                                   d);
                                        str = buf;
                                    }
                                } else {
                                    if (date_costom_format == -1) {
                                        const wchar_t* p1 =
                                            book->customNumFormat(num_format);
                                        if (p1 && wcslen(p1) > 0 &&
                                            (wcscmp(p1, L"yyyy\\-mm\\-dd") ==
                                                 0 ||
                                             wcscmp(p1, L"yyyy-mm-dd") == 0 ||
                                             wcscmp(p1, L"yyyy/m/d;@") == 0 ||
                                             wcscmp(p1, L"yyyy/mm/dd;@") == 0 ||
                                             wcscmp(p1, L"yyyy/m/d") == 0 ||
                                             wcscmp(p1, L"yyyy/mm/dd") == 0)) {
                            ...
                            SetGridItemText(str, row + additional_header_row,
                                            col + 1);
                        }
                    }
                }
...
bool GridBody::AutoFixColWidth(int col_index, int min_width, int max_width) {
    ...
    for (int i = 0; i < row_count; i++) {
        ...
        if (!str.empty()) {
            UiRect rcMessure =
                pRender->MeasureText(str, m_strGridFont, m_uTextStyle, 0);
...
void GridBody::HandleMessage(EventArgs& event) {
    if (!IsMouseEnabled() && event.Type > kEventMouseBegin &&
        ...
        if (position.x < m_nFixedCol && position.y < m_nFixedRow) {
            if (position.y == 0) {
                for (size_t i = 0; i < m_nFixedCol; i++) {
                    posx += m_hLayout[i];
                    if (posx - pt.x >= 0 &&
                        posx - pt.x < DRAG_HEADER_OFF_SIZE) {
                        m_nDragColIndex = i;
        ...
        } else if (position.y < m_nFixedRow)  //点击第一行 选择列
        {
            if (position.y == 0) {
                ...
                for (size_t i = m_nFixedCol; i < m_hLayout.size(); i++) {
                    posx += m_hLayout[i];
                    if (posx - pt.x - szOff.cx >= 0 &&
                        posx - pt.x - szOff.cx < DRAG_HEADER_OFF_SIZE) {
                        m_nDragColIndex = i;
                        m_ptDragColumnStart = pt;
                        m_ptDragColumnMoving = pt;
...
bool GridBody::ButtonUp(EventArgs& msg) {
    if (m_nDragColIndex != -1) {
        ...
        pt.Offset(-m_rcItem.left, -m_rcItem.top);
        int width =
            GetColumnWidth(m_nDragColIndex) + pt.x - m_ptDragColumnStart.x;
...
bool GridBody::OnMouseDoubleClick(EventArgs& msg) {
    ...
    if (bFind) {
        ...
        UiRect rcHeader(
            {fixed_col_width, 0, m_pGrid->GetWidth(), m_vLayout[0]});
        int posx = 0;
...
bool GridBody::OnMouseMove(EventArgs& msg) {
    ...
    if ((msg.wParam & MK_LBUTTON) > 0)  //鼠标左键被按下
    {
        ...
        {
            if (pt.x >= m_pGrid->GetWidth() ||
                pt.y >= m_pGrid->GetHeight())  //超出Grid控件位置限制
                return true;
            CPoint position;
            bool ctrl = (msg.wParam & MK_CONTROL);
            bool shift = (msg.wParam & MK_SHIFT);
            ...
            if (bFind && (position != m_ptDragSelStart || m_bDrageSelChanged)) {
                ...
                if (m_ptDragSelStart.x < m_nFixedCol &&
                    m_ptDragSelStart.x == 0)  // sel row drag
                {
                    m_selRange.SetSelRowRange(top, bottom, ctrl, shift);
                } else if (m_ptDragSelStart.y < m_nFixedRow &&
                           m_ptDragSelStart.y == 0)  // sel col drag
                {
                    m_selRange.SetSelColRange(left, right, ctrl, shift);
                } else {
                    m_selRange.SetSelItemRange({left, top, right, bottom}, ctrl,
                                               shift);
                }
            }
        }
...
bool GridBody::OnKeyDown(EventArgs& msg) {
    bool ctrl =
        (::GetKeyState(VK_CONTROL) & 0x80) > 0;  //判断CTRL键是否是被按下的状态
    bool shift =
        (::GetKeyState(VK_SHIFT) & 0x80) > 0;  //判断Shift键是否是被按下的状态
...
bool GridBody::OnComboEditSelected(EventArgs* args) {
    if (m_pComboEditGridItem && m_pComboEdit && m_pComboEdit->IsVisible()) {
        if (m_pComboEditGridItem->text != m_pComboEdit->GetText()) {
            ...
            m_pWindow->SendNotify(this, kEventTextChange,
                                  m_pComboEditGridItem->row_index,
                                  m_pComboEditGridItem->col_index);
        }
    }
    return true;
}
...
void GridBody::PaintChild(IRenderContext* pRender, const UiRect& rcPaint) {
    ...
    for (auto it = m_items.begin(); it != m_items.end(); it++) {
        ...
        else */
        {
            ...
            if (rcNewPaint.left > rcNewPaint.right)
                rcNewPaint.left = rcNewPaint.right;
            if (rcNewPaint.top > rcNewPaint.bottom)
                rcNewPaint.top = rcNewPaint.bottom;
...
void GridBody::PaintBorder(IRenderContext* pRender) {
    ...
    if (m_pGrid->m_bPaintGridLine && m_hLayout.size() > 0 &&
        m_vLayout.size() > 0) {
        CSize szOff = m_pGrid->GetScrollPos();
        int posx = 0, posy = 0;
        ...
        CPoint ptOldOrg =
            pRender->OffsetWindowOrg({-m_rcItem.left, -m_rcItem.top});
        {
            // draw fixed HLine
            rcLineH.left = 0;
            rcLineH.right = GetFixedWidth() - szOff.cx > grid_width
                                ? grid_width
                                : GetFixedWidth() - szOff.cx;
            assert(m_nFixedRow <= m_vLayout.size());
            for (size_t i = 0; i < m_nFixedRow; i++) {
                posy += m_vLayout[i];
            ...
            for (size_t i = m_nFixedRow; i < m_vLayout.size(); i++) {
                ...
                if (posy - szOff.cy > grid_height)  //超出grid高度
                    break;
                if (posy - szOff.cy > fixed_row_height &&
                    posy - szOff.cy < grid_height) {
            ...
            // draw fixed VLine
            rcLineV.top = 0;
            rcLineV.bottom = GetFixedHeight() - szOff.cy > grid_height
                                 ? grid_height
                                 : GetFixedHeight() - szOff.cy;
            assert(m_nFixedCol <= m_hLayout.size());
            for (size_t i = 0; i < m_nFixedCol; i++) {
                posx += m_hLayout[i];
...
void GridBody::PaintBody(IRenderContext* pRender) {
    ...
    if (fixed_row_height > 0) {
        ...
        rcPaint.right = rcPaint.left + (GetFixedWidth() - szOff.cx > grid_width
                                            ? grid_width
                                            : GetFixedWidth() - szOff.cx);
    ...
    if (fixed_col_width > 0) {
        ...
        rcPaint.bottom =
            rcPaint.top + (GetFixedHeight() - szOff.cy > grid_height
                               ? grid_height
                               : GetFixedHeight() - szOff.cy);
        pRender->DrawColor(rcPaint, m_strFixedBkColor, 255);
    }
    ...
    for (int i = 0; i < m_nFixedRow; i++) {
        ...
        for (size_t j = 0; j < m_nFixedCol; j++) {
            ...
            pRender->DrawText(rc, grid_row->at(j)->text, dwDefColor,
                              m_strGridFont, m_uTextStyle, 255, false);
            posx += m_hLayout[j];
        }
        posy += m_vLayout[i];
    }
    ...
    // draw fixed row text
    {
        ...
        for (int i = 0; i < m_nFixedRow; i++) {
            ...
            for (size_t j = m_nFixedCol; j < col_count; j++) {
                ...
                if (!str.empty() &&
                    posx + m_hLayout[j] - szOff.cx >
                        fixed_col_width)  //单元格右边线没有超过fixed_col_width
                {
                    UiRect rc = {posx, posy, posx + m_hLayout[j],
                                 posy + m_vLayout[i]};
                    rc.Offset({m_rcItem.left - szOff.cx, m_rcItem.top});
                    pRender->DrawText(rc, str, dwDefColor, m_strGridFont,
                                      m_uTextStyle, 255, false);
                }
                posx += m_hLayout[j];
                if (posx - szOff.cx > grid_width)  //超出grid宽度
                    break;
    ...
    // draw fixed col text
    {
        ...
        for (int i = 0; i < m_nFixedCol; i++) {
            ...
            for (size_t j = m_nFixedRow; j < row_count; j++) {
                ...
                if (!str.empty() &&
                    posy + m_vLayout[j] - szOff.cy >
                        fixed_row_height)  //单元格下边线没有超过fixed_row_height
                {
                    UiRect rc = {posx, posy, posx + m_hLayout[i],
                                 posy + m_vLayout[j]};
                ...
                fixed_row_height)  //单元格下边线没有超过fixed_row_height
            {
                ...
                for (size_t j = m_nFixedCol; j < col_count; j++) {
                    ...
                    UiRect rc = {posx, posy, posx + m_hLayout[j],
                                 posy + m_vLayout[i]};
                    rc.Offset(
                        {m_rcItem.left - szOff.cx, m_rcItem.top - szOff.cy});
                    rc.Deflate({1, 1, 2, 2});
                    //绘制单元格背景色
                    if (pItem->IsSelected()) {
                            ...
                            fixed_col_width)  //单元格右边线没有超过fixed_col_width
                    {
                        if (pItem->text_color.empty() && pItem->text_style == 0)
                            pRender->DrawText(rc, str, dwDefColor,
                                              m_strGridFont, m_uTextStyle, 255,
                                              false);
                        else {
                            DWORD dwColor = (pItem->text_color.empty()
                                                 ? dwDefColor
                                                 : GlobalManager::GetTextColor(
                                                       pItem->text_color));
                            UINT text_style =
                                (pItem->text_style == 0 ? m_uTextStyle
                                                        : pItem->text_style);
                            pRender->DrawText(rc, str, dwColor, m_strGridFont,
                                              text_style, 255, false);
                        }
                    }
                    posx += m_hLayout[j];
                    if (posx - szOff.cx > grid_width)  //超出grid宽度
