...
public:
     ...
     */
    bool RegisterWindow(const std::wstring wnd_class_name,
     ...
     * @return void 无返回值
     */
    void UnRegisterWindow(const std::wstring& wnd_class_name,
                          const std::wstring& wnd_id,
                          WindowEx* wnd);
     ...
     */
    WindowEx* GetWindow(const std::wstring& wnd_class_name,
                        const std::wstring& wnd_id);
    ...
    static WindowType* SingletonShow(const std::wstring& window_id,
        ...
        WindowType* window =
            (WindowType*)(WindowsManager::GetInstance()->GetWindow(
                WindowType::kClassName, window_id));
        if (!window) {
            window = new WindowType(params...);
            window->Create(NULL, WindowType::kClassName,
