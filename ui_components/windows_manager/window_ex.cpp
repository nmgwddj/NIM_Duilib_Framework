...
HWND WindowEx::Create(HWND hwndParent,
                      LPCTSTR pstrName,
                      DWORD dwStyle,
                      DWORD dwExStyle,
...
LRESULT WindowEx::OnDestroy(UINT uMsg,
                            WPARAM wParam,
                            LPARAM lParam,
                            BOOL& bHandled) {
    UnRegisterWnd();
...
bool WindowEx::RegisterWnd() {
    ...
    if (!WindowsManager::GetInstance()->RegisterWindow(wnd_class_name, wnd_id,
                                                       this)) {
        return false;
    }
    return true;
...
void WindowEx::UnRegisterWnd() {
    std::wstring wnd_class_name = GetWindowClassName();
    std::wstring wnd_id = GetWindowId();
    WindowsManager::GetInstance()->UnRegisterWindow(wnd_class_name, wnd_id,
                                                    this);
}
...
POINT GetPopupWindowPos(WindowEx* window) {
    ...
    ::GetMonitorInfo(
        ::MonitorFromWindow(window->GetHWND(), MONITOR_DEFAULTTONEAREST),
        &oMonitor);
