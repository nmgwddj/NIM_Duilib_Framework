...
bool WindowsManager::RegisterWindow(const std::wstring wnd_class_name,
    ...
    if (it != windows_map_.end()) {
        std::map<std::wstring, WindowEx*>::iterator it2 =
            it->second.find(wnd_id);
...
void WindowsManager::UnRegisterWindow(const std::wstring& wnd_class_name,
                                      const std::wstring& wnd_id,
                                      WindowEx* wnd) {
    WindowsMap::iterator it = windows_map_.find(wnd_class_name);
    if (it != windows_map_.end()) {
...
WindowEx* WindowsManager::GetWindow(const std::wstring& wnd_class_name,
                                    const std::wstring& wnd_id) {
    WindowsMap::iterator it = windows_map_.find(wnd_class_name);
    if (it != windows_map_.end()) {
        std::map<std::wstring, WindowEx*>::iterator it2 =
