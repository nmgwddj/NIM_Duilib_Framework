...
HWND ShadowWnd::Create(Window* window) {
    window_ = window;
    return Window::Create(NULL, L"ShadowWindow",
                          WS_OVERLAPPEDWINDOW & ~WS_MAXIMIZEBOX,
                          WS_EX_TRANSPARENT | WS_EX_TOOLWINDOW, true);
}

LRESULT ShadowWnd::MessageHandler(UINT uMsg,
                                  WPARAM wParam,
                                  LPARAM lParam,
                                  BOOL& bHandled) {
    ...
    switch (uMsg) {
        ...
        case WM_NCACTIVATE: {
            if (::IsWindowVisible(window_->GetHWND())) {
                ...
                SetPos(rc, false, SWP_SHOWWINDOW | SWP_NOACTIVATE,
                       window_->GetHWND());
            }
            break;
        }
...
HWND ShadowWndBase::Create(HWND hwndParent,
                           LPCTSTR pstrName,
                           DWORD dwStyle,
                           DWORD dwExStyle,
