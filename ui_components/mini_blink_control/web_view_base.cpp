...
void OnTitleChangedCallback(wkeWebView webView,
                            void* param,
...
void OnUrlChangedCallback(wkeWebView webView,
                          void* param,
                          const wkeString url) {
    nim_comp::WebViewBase* web_view = (nim_comp::WebViewBase*)param;
...
bool OnNavigationCallback(wkeWebView webView,
                          void* param,
...
wkeWebView OnCreateViewCallback(wkeWebView webView,
                                void* param,
                                wkeNavigationType navigationType,
                                const wkeString url,
...
void OnLoadingFinishCallback(wkeWebView webView,
                             void* param,
                             const wkeString url,
...
bool OnLoadUrlBeginCallback(wkeWebView webView,
                            void* param,
                            const utf8* url,
                            wkeNetJob job) {
    nim_comp::WebViewBase* web_view = (nim_comp::WebViewBase*)param;
...
void OnLoadUrlEndCallback(wkeWebView webView,
                          void* param,
                          const utf8* url,
                          wkeNetJob job,
                          void* buf,
...
void OnLoadUrlFailCallback(wkeWebView webView,
                           void* param,
                           const utf8* url,
                           wkeNetJob job) {
    nim_comp::WebViewBase* web_view = (nim_comp::WebViewBase*)param;
...
bool NetOnResponseCallback(wkeWebView webView,
                           void* param,
                           const utf8* url,
                           wkeNetJob job) {
    nim_comp::WebViewBase* web_view = (nim_comp::WebViewBase*)param;
...
void WebViewBase::RemoveObserver(WebViewObserver* observer) {
    auto item =
        std::find(m_observer_list.begin(), m_observer_list.end(), observer);
...
wkeWebView WebViewBase::CreateWebWindow(wkeWindowType type,
                                        HWND parent,
                                        int x,
                                        int y,
                                        int width,
...
bool WebViewBase::fireMouseEvent(unsigned int message,
                                 int x,
                                 int y,
                                 unsigned int flags) {
    return m_wke_web_view
               ? wkeFireMouseEvent(m_wke_web_view, message, x, y, flags)
               : false;
}

bool WebViewBase::fireContextMenuEvent(int x, int y, unsigned int flags) {
    return m_wke_web_view ? wkeFireContextMenuEvent(m_wke_web_view, x, y, flags)
                          : false;
}

bool WebViewBase::fireMouseWheelEvent(int x,
                                      int y,
                                      int delta,
                                      unsigned int flags) {
    return m_wke_web_view
               ? wkeFireMouseWheelEvent(m_wke_web_view, x, y, delta, flags)
               : false;
}

bool WebViewBase::fireKeyUpEvent(unsigned int virtualKeyCode,
                                 unsigned int flags,
                                 bool systemKey) {
    return m_wke_web_view ? wkeFireKeyUpEvent(m_wke_web_view, virtualKeyCode,
                                              flags, systemKey)
                          : false;
}

bool WebViewBase::fireKeyDownEvent(unsigned int virtualKeyCode,
                                   unsigned int flags,
                                   bool systemKey) {
    return m_wke_web_view ? wkeFireKeyDownEvent(m_wke_web_view, virtualKeyCode,
                                                flags, systemKey)
                          : false;
}

bool WebViewBase::fireKeyPressEvent(unsigned int virtualKeyCode,
                                    unsigned int flags,
                                    bool systemKey) {
    return m_wke_web_view ? wkeFireKeyPressEvent(m_wke_web_view, virtualKeyCode,
                                                 flags, systemKey)
                          : false;
}
...
bool WebViewBase::OnNavigation(wkeWebView webView,
                               wkeNavigationType navigationType,
                               wkeString url) {
    std::wstring url_temp = wkeGetStringW(url);
...
wkeWebView WebViewBase::OnCreateView(wkeWebView webView,
                                     wkeNavigationType navigationType,
                                     const wkeString url,
                                     const wkeWindowFeatures* windowFeatures) {
    std::wstring url_temp = wkeGetStringW(url);
    if (m_create_web_view_callback) {
        return m_create_web_view_callback(navigationType, url_temp,
                                          windowFeatures);
    }
...
void WebViewBase::OnLoadingFinish(wkeWebView webView,
                                  const wkeString url,
                                  wkeLoadingResult result,
                                  const wkeString failedReason) {
    std::wstring url_temp = wkeGetStringW(url);
...
bool WebViewBase::OnLoadUrlBegin(wkeWebView webView,
                                 const utf8* url,
                                 wkeNetJob job) {
...
void WebViewBase::OnLoadUrlEnd(wkeWebView webView,
                               const utf8* url,
                               wkeNetJob job,
                               void* buf,
                               int len) {
...
void WebViewBase::OnLoadUrlFail(wkeWebView webView,
                                const utf8* url,
                                wkeNetJob job) {
...
bool WebViewBase::NetOnResponse(wkeWebView webView,
                                const utf8* url,
                                wkeNetJob job) {
