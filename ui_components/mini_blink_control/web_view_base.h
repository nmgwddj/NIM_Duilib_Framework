...
typedef std::function<bool(wkeNavigationType, std::wstring&)>
    NavigationCallback;
typedef std::function<wkeWebView(wkeNavigationType,
                                 const std::wstring&,
                                 const wkeWindowFeatures*)>
    CreateWebViewCallback;
typedef std::function<
    void(const std::wstring&, wkeLoadingResult, const std::wstring&)>
    LoadingFinishCallback;
typedef std::function<bool(const std::string&, wkeNetJob)> LoadUrlBeginCallback;
typedef std::function<void(const std::string&, wkeNetJob, void*, int)>
    LoadUrlEndCallback;
...
public:
    ...
    wkeWebView CreateWebWindow(wkeWindowType type,
                               HWND parent,
                               int x,
                               int y,
                               int width,
    ...
    bool fireMouseEvent(unsigned int message,
                        int x,
                        int y,
                        unsigned int flags) override;
    bool fireContextMenuEvent(int x, int y, unsigned int flags) override;
    bool fireMouseWheelEvent(int x,
                             int y,
                             int delta,
                             unsigned int flags) override;
    bool fireKeyUpEvent(unsigned int virtualKeyCode,
                        unsigned int flags,
                        bool systemKey) override;
    bool fireKeyDownEvent(unsigned int virtualKeyCode,
                          unsigned int flags,
                          bool systemKey) override;
    bool fireKeyPressEvent(unsigned int virtualKeyCode,
                           unsigned int flags,
                           bool systemKey) override;
    ...
    virtual bool OnNavigation(wkeWebView webView,
                              wkeNavigationType navigationType,
                              wkeString url);

    virtual wkeWebView OnCreateView(wkeWebView webView,
                                    wkeNavigationType navigationType,
                                    const wkeString url,
                                    const wkeWindowFeatures* windowFeatures);

    virtual void OnLoadingFinish(wkeWebView webView,
                                 const wkeString url,
                                 wkeLoadingResult result,
                                 const wkeString failedReason);

    virtual bool OnLoadUrlBegin(wkeWebView webView,
                                const utf8* url,
                                wkeNetJob job);

    virtual void OnLoadUrlEnd(wkeWebView webView,
                              const utf8* url,
                              wkeNetJob job,
                              void* buf,
                              int len);

    virtual void OnLoadUrlFail(wkeWebView webView,
                               const utf8* url,
                               wkeNetJob job);
    ...
    virtual bool NetOnResponse(wkeWebView webView,
                               const utf8* url,
                               wkeNetJob job);

    void AttachOnTitleChanged(const TitleChangedCallback& callback) {
        m_title_changed_callback = callback;
    }

    void AttachOnUrlChanged(const UrlChangedCallback& callback) {
        m_url_changed_callback = callback;
    }

    void AttachOnNavigation(const NavigationCallback& callback) {
        m_navigation_callback = callback;
    }

    void AttachOnCreateView(const CreateWebViewCallback& callback) {
        m_create_web_view_callback = callback;
    }

    void AttachOnLoadingFinish(const LoadingFinishCallback& callback) {
        m_loading_finish_callback = callback;
    }

    void AttachOnLoadUrlBegin(const LoadUrlBeginCallback& callback) {
        m_load_url_begin_callback = callback;
    }

    void AttachOnLoadUrlEnd(const LoadUrlEndCallback& callback) {
        m_load_url_end_callback = callback;
    }

    void AttachOnLoadUrlFail(const LoadUrlFailCallback& callback) {
        m_load_url_fail_callback = callback;
    }

    void AttachOnWindowClosingCallback(const WindowClosingCallback& callback) {
        m_window_closing_callback = callback;
    }

    void AttachOnWindowDestroy(const WindowDestroyCallback& callback) {
        m_window_destroy_callback = callback;
    }

    void AttachNetOnResponse(const ResponseCallback& callback) {
