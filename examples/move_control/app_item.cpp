...
void AppItemUi::FixPos(int step, int index) {
    ...
    ui::UiRect marginRect = {(index_ % EACH_LINE) * APP_WIDTH,
                             (index_ / EACH_LINE) * APP_HEIGHT, 0, 0};
...
LRESULT AppWindow::OnClose(UINT uMsg,
                           WPARAM wParam,
                           LPARAM lParam,
                           BOOL& bHandled) {
    //清理hook
...
void AppWindow::AdjustPos() {
    ...
    ::SetWindowPos(GetHWND(), NULL, ptCursor.x - rcCorner.left,
                   ptCursor.y - rcCorner.top, -1, -1,
...
void AppWindow::InstallHook() {
    if (mouse_Hook_)
        UnInstallHook();
    mouse_Hook_ =
        SetWindowsHookEx(WH_MOUSE_LL, (HOOKPROC)AppWindow::LowLevelMouseProc,
...
LRESULT CALLBACK AppWindow::LowLevelMouseProc(int nCode,
                                              WPARAM wParam,
                                              LPARAM lParam) {
    if (nCode == HC_ACTION) {
