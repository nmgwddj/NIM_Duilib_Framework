...
MoveControlForm::MoveControlForm(const std::wstring& class_name,
...
void MoveControlForm::InitWindow() {
    ...
    // loadFromDb
    // getFromServer---->后台可以先保存到db，再post个消息出来，界面重新从db
    // load。
    ...
    for (const auto& item : applist) {
        ...
        pAppUi->AttachAllEvents(
            nbase::Bind(&MoveControlForm::OnProcessAppItemDrag, this,
                        std::placeholders::_1));
        if (item._isFrequent) {
...
LRESULT MoveControlForm::OnClose(UINT uMsg,
                                 WPARAM wParam,
                                 LPARAM lParam,
                                 BOOL& bHandled) {
    PostQuitMessage(0L);
...
LRESULT MoveControlForm::OnLButtonUp(UINT uMsg,
                                     WPARAM wParam,
                                     LPARAM lParam,
                                     BOOL& bHandled) {
    ...
    if (!DoAfterDrag(frequent_app_) && !DoAfterDrag(my_app_)) {
        ...
        //从index处开始补缺口
        for (int index = current_item_->getIndex() + 1;
             index < pParent->GetCount(); ++index) {
            AppItemUi* _pItem =
                dynamic_cast<AppItemUi*>(pParent->GetItemAt(index));
            if (_pItem) {
                _pItem->FixPos(+1);
            }
        }
...
void MoveControlForm::ShowCustomWindow(const std::wstring& class_name,
                                       const std::wstring& theme_directory,
                                       const std::wstring& layout_xml) {
    MoveControlForm* window =
        new MoveControlForm(class_name, theme_directory, layout_xml);
...
bool MoveControlForm::OnProcessAppItemDrag(ui::EventArgs* param) {
    switch (param->Type) {
        case kEventMouseMove: {
            ...
            POINT pt = {param->ptMouse.x - rect.left,
                        param->ptMouse.y - rect.top};
...
void MoveControlForm::DoBeforeDrag() {
    ...
    if (current_item_) {
        ...
        //从index处开始补缺口
        for (int index = current_item_->getIndex(); index < pParent->GetCount();
             ++index) {
            AppItemUi* _pItem =
                dynamic_cast<AppItemUi*>(pParent->GetItemAt(index));
            if (_pItem) {
...
void MoveControlForm::DoDraging(POINT pos) {
    ...
    AppWindow* pWindow =
        AppWindow::CreateAppWindow(GetHWND(), pos, current_item_->getAppData());
    ASSERT(pWindow);
}
...
bool MoveControlForm::DoAfterDrag(ui::Box* check) {
    ...
    if (rectBox.IsPointIn(pt)) {
        ...
        if (findIndex < check->GetCount()) {
            ...
            //从index处开始补缺口
            for (int index = findIndex + 1; index < check->GetCount();
                 ++index) {
                AppItemUi* _pItem =
                    dynamic_cast<AppItemUi*>(check->GetItemAt(index));
                if (_pItem) {
                    _pItem->FixPos(+1);
                }
            }
