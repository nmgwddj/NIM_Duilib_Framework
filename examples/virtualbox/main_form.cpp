...
void MainForm::InitWindow() {
    ...
    m_CheckBoxItemCenter =
        dynamic_cast<ui::CheckBox*>(FindControl(L"checkbox_item_center"));
    ...
    m_OptionColumnFix =
        dynamic_cast<ui::Option*>(FindControl(L"option_column_fix"));
    m_EditUpdate = dynamic_cast<ui::RichEdit*>(FindControl(L"edit_update"));
    m_EditTaskName =
        dynamic_cast<ui::RichEdit*>(FindControl(L"edit_task_name"));
    m_EditDelete = dynamic_cast<ui::RichEdit*>(FindControl(L"edit_delete"));
    m_EditChildMargin =
        dynamic_cast<ui::RichEdit*>(FindControl(L"edit_child_margin"));

    m_pRoot->AttachBubbledEvent(
        ui::kEventClick,
        nbase::Bind(&MainForm::OnClicked, this, std::placeholders::_1));
...
bool MainForm::OnClicked(ui::EventArgs* args) {
    ...
    if (sName == L"btn_set_total") {
        if (!m_EditChildMargin->GetText().empty()) {
            m_pTileList->SetAttribute(L"childmargin",
                                      m_EditChildMargin->GetText());
        }
        ...
        m_DataProvider->ChangeTaskName(
            _ttoi(m_EditUpdate->GetText().c_str()) - 1,
