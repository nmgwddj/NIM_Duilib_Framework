...
void ControlForm::InitWindow() {
    ...
    std::string checks[6] = {"check1", "check2", "check3",
                             "check4", "check5", "check6"};
    nim_comp::CheckCombo* check_combo =
        static_cast<nim_comp::CheckCombo*>(FindControl(L"check_combo"));
    for (auto i = 0; i < 6; i++) {
        ...
        std::wstring image_normal = nbase::StringPrintf(
            L"file='../public/checkbox/check_no.png' dest='%d,4,%d,20'", 2, 18);
        std::wstring image_select = nbase::StringPrintf(
            L"file='../public/checkbox/check_yes.png' dest='%d,4,%d,20'", 2,
            18);
    ...
    nim_comp::FilterCombo* filter_combo =
        static_cast<nim_comp::FilterCombo*>(FindControl(L"filter_combo"));
    char buf[16] = {};
    for (auto i = 0; i < 100; i++) {
        nim_comp::ListElementMatch* item = new nim_comp::ListElementMatch;
        item->SetFixedHeight(20);
        // ui::GlobalManager::FillBoxWithCache(item,
        // L"date_export/combo/date_item.xml"); Label *label = new label;
    ...
    /* Load xml file content in global misc thread, and post update RichEdit
     * task to UI thread */
    nbase::ThreadManager::PostTask(
        kThreadGlobalMisc, nbase::Bind(&ControlForm::LoadRichEditData, this));

    /* Post repeat task to update progress value 200 milliseconds once */
    /* Using ToWeakCallback to protect closure when if [ControlForm] was
     * destoryed */
    nbase::ThreadManager::PostRepeatedTask(
        kThreadGlobalMisc, ToWeakCallback([this]() {
            nbase::TimeDelta time_delta = nbase::TimeDelta::FromMicroseconds(
                nbase::Time::Now().ToInternalValue());
            nbase::ThreadManager::PostTask(
                kThreadUI,
                nbase::Bind(&ControlForm::OnProgressValueChagned, this,
                            time_delta.ToMilliseconds() % 100));
        }),
    ...
    settings->AttachClick([this](ui::EventArgs* args) {
        ...
        /* Sub menu example */
        nim_comp::CMenuElementUI* menu_fourth =
            static_cast<nim_comp::CMenuElementUI*>(
                sub_menu->FindControl(L"fourth"));
        ...
        /* About menu */
        nim_comp::CMenuElementUI* menu_about =
            static_cast<nim_comp::CMenuElementUI*>(
                sub_menu->FindControl(L"about"));
        menu_about->AttachClick([this](ui::EventArgs* args) {
            AboutForm* about_form =
                (AboutForm*)(nim_comp::WindowsManager::GetInstance()->GetWindow(
                    AboutForm::kClassName, AboutForm::kClassName));
            if (!about_form) {
                about_form = new AboutForm();
                about_form->Create(GetHWND(), AboutForm::kClassName,
...
LRESULT ControlForm::OnClose(UINT uMsg,
                             WPARAM wParam,
                             LPARAM lParam,
                             BOOL& bHandled) {
    PostQuitMessage(0L);
...
void ControlForm::LoadRichEditData() {
    ...
    std::wstring xml;
    std::wstring controls_xml = ui::GlobalManager::GetResourcePath() +
                                GetWindowResourcePath() + GetSkinFile();
    ...
    nbase::ThreadManager::PostTask(
        kThreadUI, nbase::Bind(&ControlForm::OnResourceFileLoaded, this, xml));
}
...
void ControlForm::OnProgressValueChagned(float value) {
    ...
    auto circleprogress =
        static_cast<ui::Progress*>(FindControl(L"circleprogress"));
    if (circleprogress) {
