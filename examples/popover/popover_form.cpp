...
void PopoverForm::InitWindow() {
    m_pPopoverLayer =
        static_cast<nim_comp::PopoverLayer*>(FindControl(L"popover_layer"));

    btn_updatetitle = static_cast<ui::Button*>(FindControl(L"btn_updatetitle"));
    btn_updatecontent =
        static_cast<ui::Button*>(FindControl(L"btn_updatecontent"));
...
LRESULT PopoverForm::OnClose(UINT uMsg,
                             WPARAM wParam,
                             LPARAM lParam,
                             BOOL& bHandled) {
    PostQuitMessage(0L);
...
void PopoverForm::TestAllPlacement() {
    ...
    auto create_popover = [this](PLACE_TEST_BUTTONS button) {
        auto popover = m_pPopoverLayer->CreatePopover(
            button.anchor, button.content, L"button.content", L"", L"",
            nim_comp::kPlaceOuter | button.placement,
            nim_comp::kButtonOk | nim_comp::kButtonCancel |
                nim_comp::kButtonClose,
            nim_comp::kTriggerClick,
            nim_comp::kTriggerClickLayer | nim_comp::kTriggerConfirm);

        popover->BindResultCallback(
            [this, button](const nim_comp::PopoverResult& result) {
    ...
    auto create_alert = [this](PLACE_TEST_BUTTONS button) {
        auto popover = m_pPopoverLayer->CreateAlert(
            button.anchor, button.content, L"", L"", L"",
            nim_comp::kPlaceOuter | button.placement,
            nim_comp::kButtonOk | nim_comp::kButtonCancel |
                nim_comp::kButtonClose,
            nim_comp::kTriggerClick, nim_comp::kTriggerConfirm);
    ...
    auto create_notification = [this](PLACE_TEST_BUTTONS button) {
        auto popover = m_pPopoverLayer->CreateNotification(
            button.anchor, button.content, L"", nim_comp::kIconInfo,
            nim_comp::kPlaceOuter | button.placement);
    ...
    auto create_tooltip = [this](PLACE_TEST_BUTTONS button) {
        auto popover = m_pPopoverLayer->CreateToolltip(
            button.anchor, button.content,
            nim_comp::kPlaceOuter | button.placement);
...
void PopoverForm::TestOthers() {
    {
        auto popover = m_pPopoverLayer->CreateNotification(
            nullptr, L"Title", L"", nim_comp::kIconNone,
            nim_comp::kPlaceOuter | nim_comp::kPlaceRight);

        m_pPopoverLayer->ShowNotification(popover);

        btn_updatetitle->AttachClick(
            ToWeakCallback([this, popover](ui::EventArgs* args) {
