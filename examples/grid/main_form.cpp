...
void MainForm::InitWindow() {
    ...
    std::wstring header[] = {L"代码",   L"名称",     L"类型",     L"长度",
                             L"顺序号", L"编辑模式", L"是否显示", L"参数类型",
                             L"参数值", L"默认值"};
    ...
    RichEdit* re_fixed_row =
        static_cast<RichEdit*>(FindControl(L"re_fixed_row"));
    RichEdit* re_fixed_col =
        static_cast<RichEdit*>(FindControl(L"re_fixed_col"));
    ...
    re_row->AttachBubbledEvent(kEventReturn,
                               [this, grid, re_row](ui::EventArgs* args) {
    ...
    re_col->AttachBubbledEvent(kEventReturn,
                               [this, grid, re_col](ui::EventArgs* args) {
    ...
    re_fixed_row->AttachBubbledEvent(
        kEventReturn, [this, grid, re_fixed_row](ui::EventArgs* args) {
            std::wstring text = re_fixed_row->GetText();
            if (!text.empty()) {
                int fixed = 0;
                if (nbase::StringToInt(text, &fixed) &&
                    fixed != grid->GetFixedRowCount()) {
    ...
    re_fixed_col->AttachBubbledEvent(
        kEventReturn, [this, grid, re_fixed_col](ui::EventArgs* args) {
            std::wstring text = re_fixed_col->GetText();
            if (!text.empty()) {
                int fixed = 0;
                if (nbase::StringToInt(text, &fixed) &&
                    fixed != grid->GetFixedColCount()) {
    ...
    Button* btn_remove_row =
        static_cast<Button*>(FindControl(L"btn_remove_row"));
    Button* btn_add_col = static_cast<Button*>(FindControl(L"btn_add_col"));
    Button* btn_remove_col =
        static_cast<Button*>(FindControl(L"btn_remove_col"));
    Button* btn_import_excel =
        static_cast<Button*>(FindControl(L"btn_import_excel"));
...
LRESULT MainForm::OnClose(UINT uMsg,
                          WPARAM wParam,
                          LPARAM lParam,
                          BOOL& bHandled) {
    PostQuitMessage(0L);
