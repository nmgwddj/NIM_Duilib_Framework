...
MyGridBody::MyGridBody(Grid* pGrid)
    : GridBody(pGrid) {
    printf("MyGridBody::MyGridBody\n");
    m_pGrid->AttachScrollChange(nbase::Bind(&MyGridBody::OnScrollPosChanged,
...
void MyGridBody::SetColCount(int count) {
    ...
    for (size_t i = 1; i < header->size(); i++) {
        ...
        combo->SetMargin(ui::UiRect(margin_left + HEADER_COMBO_MARGIN_LEFT,
                                    header_height / 2, 0, 0));
...
GridHeaderItem* MyGridBody::AddCol(std::wstring text, int width) {
    ...
    if (item) {
        if (col_index > 0) {
            ...
            Combo* combo = new Combo;
            combo->SetFixedWidth(col_width - HEADER_COMBO_MARGIN_LEFT * 2);
            combo->SetFixedHeight(GetHeaderHeight() / 2 -
                                  HEADER_COMBO_MARGIN_BOTTOM);
            combo->SetMargin(ui::UiRect(margin_left + HEADER_COMBO_MARGIN_LEFT,
                                        GetHeaderHeight() / 2, 0, 0));
            combo->SetClass(L"combo2");
            combo->SetAttribute(L"cursortype", L"hand");
            combo->SetFloat(true);
            item->control_ = combo;
...
void MyGridBody::ResetHeanderComboPos() {
    ...
    if (pHeader) {
        ...
        for (size_t i = 0; i < pHeader->size(); i++) {
            ...
            pControl->SetFixedWidth(m_hLayout[i] -
                                    HEADER_COMBO_MARGIN_LEFT * 2);
