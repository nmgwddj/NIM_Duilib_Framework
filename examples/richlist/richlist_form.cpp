...
void RichlistForm::InitWindow() {
    ...
    // 监听列表中点击选择子项的事件
    list_->AttachSelect(
        nbase::Bind(&RichlistForm::OnSelected, this, std::placeholders::_1));
}
...
bool RichlistForm::OnSelected(ui::EventArgs* args) {
    ...
    auto message = nbase::StringPrintf(
        L"您选择了索引为 %d 的子项，上一次选择子项索引为 %d", current, old);
    nim_comp::ShowMsgBox(GetHWND(), nim_comp::MsgboxCallback(), message, false,
                         L"提示", false);

    return true;
}

LRESULT RichlistForm::OnClose(UINT uMsg,
                              WPARAM wParam,
                              LPARAM lParam,
                              BOOL& bHandled) {
    PostQuitMessage(0L);
