...
STDMETHODIMP SdkDataObject::GetData(FORMATETC* pformatetcIn,
    ...
    for (int i = 0; i < nSize; ++i) {
        ...
        if ((pformatetcIn->tymed & dataEntry.m_formatEtc->tymed) &&
            ...
            return CopyMedium(pmedium, dataEntry.m_stgMedium,
                              dataEntry.m_formatEtc);
        }
    }
...
STDMETHODIMP SdkDataObject::GetDataHere(FORMATETC* pformatetc,
                                        STGMEDIUM* pmedium) {
...
STDMETHODIMP SdkDataObject::GetCanonicalFormatEtc(FORMATETC* pformatetcIn,
...
STDMETHODIMP SdkDataObject::DAdvise(FORMATETC* pformatetc,
                                    DWORD advf,
                                    IAdviseSink* pAdvSnk,
                                    DWORD* pdwConnection) {
    UNREFERENCED_PARAMETER(pformatetc);
...
HRESULT SdkDataObject::CopyMedium(STGMEDIUM* pMedDest,
    ...
    switch (pMedSrc->tymed) {
        ...
        case TYMED_ENHMF:
            ...
            break;
        case TYMED_FILE:
            pMedDest->lpszFileName = (LPOLESTR)OleDuplicateData(
...
HRESULT SdkDataObject::SetBlob(CLIPFORMAT cf, const void* pvBlob, UINT cbBlob) {
    ...
    if (SUCCEEDED(hr)) {
        ...
        // The STGMEDIUM structure is used to define how to handle a global
        // memory transfer. This structure includes a flag, tymed, which
        // indicates the medium to be used, and a union comprising pointers and
        // a handle for getting whichever medium is specified in tymed.
        STGMEDIUM medium = {};
...
STDMETHODIMP SdkDropSource::QueryInterface(
    /* [in] */ REFIID riid,
...
STDMETHODIMP SdkDropSource::QueryContinueDrag(/* [in] */ BOOL fEscapePressed,
...
HRESULT SdkDragSourceHelper::InitializeFromBitmap(HBITMAP hBitmap,
                                                  POINT& pt_offset,
                                                  SIZE& bitmap_size,
...
HRESULT SdkDragSourceHelper::InitializeFromWindow(HWND hwnd,
                                                  POINT& pt,
                                                  IDataObject* pDataObject) {
    if (m_pDragSourceHelper == NULL)
        return E_FAIL;
