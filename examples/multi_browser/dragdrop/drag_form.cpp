...
LRESULT CALLBACK DragForm::LowLevelMouseProc(int nCode,
                                             WPARAM wParam,
                                             LPARAM lParam) {
    if (nCode == HC_ACTION) {
        if (wParam == WM_MOUSEMOVE && ::GetKeyState(VK_LBUTTON) < 0) {
            ...
            if (NULL != pMouseStruct) {
                if (NULL != s_drag_form) {
                    ...
                    ui::UiRect rc(pMouseStruct->pt.x - s_point_offset.x,
                                  pMouseStruct->pt.y - s_point_offset.y, 0, 0);
...
void DragForm::CreateCustomDragImage(HBITMAP bitmap, POINT pt_offset) {
    ASSERT(NULL != bitmap);
    s_mouse_hook =
        SetWindowsHookEx(WH_MOUSE_LL, (HOOKPROC)DragForm::LowLevelMouseProc,
...
DragForm* DragForm::CreateDragForm(HBITMAP bitmap, POINT pt_offset) {
    ...
    HWND hwnd = drag_form->Create(
        NULL, kClassName, UI_WNDSTYLE_FRAME,
