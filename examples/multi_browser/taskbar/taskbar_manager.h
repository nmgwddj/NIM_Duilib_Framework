...
public:
    /**
     * @param[in] bind_control
     * 与TaskbarTabItem绑定在一起的提供缩略图和预览图的控件的指针 构造函数
     */
...
/** @class TaskbarManager
 * @brief
 * 管理某个会话窗口在任务栏显示的预览效果(只在Win7及以上系统有),其中包含多个TaskbarItem
 * @copyright (c) 2016, NetEase Inc. All rights reserved
 * @author Redrain
 * @date 2019/3/20
 */
...
public:
     ...
     * @brief
     * TaskbarManager的事件委托类，只有继承该类的类可以使用TaskbarManager功能
     * @date 2019/3/20
     */
    class ITaskbarDelegate {
    public:
     ...
     * @return bool true 成功，false 失败
     */
    bool SetTabOrder(const TaskbarTabItem& tab_item,
     ...
     * @return HBITMAP	生成的位图
     */
    HBITMAP GenerateBindControlBitmap(ui::Control* control,
     ...
     * @return HBITMAP	生成的位图
     */
    HBITMAP ResizeBitmap(int dest_width,
                         int dest_height,
                         HDC src_dc,
                         int src_x,
                         int src_y,
