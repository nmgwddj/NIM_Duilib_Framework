...
void TaskbarTabItem::Init(const std::wstring& taskbar_title,
    ...
    HRESULT ret = S_OK;
    BOOL truth = TRUE;
    ret |= DwmSetWindowAttribute(m_hWnd, DWMWA_HAS_ICONIC_BITMAP, &truth,
                                 sizeof(truth));
    ret |= DwmSetWindowAttribute(m_hWnd, DWMWA_FORCE_ICONIC_REPRESENTATION,
                                 &truth, sizeof(truth));
    if (ret != S_OK) {
...
void TaskbarTabItem::OnSendThumbnail(int width, int height) {
    ...
    HBITMAP bitmap = taskbar_manager_->GenerateBindControlBitmap(bind_control_,
                                                                 width, height);
...
void TaskbarTabItem::OnSendPreview() {
    ...
    HBITMAP bitmap =
        taskbar_manager_->GenerateBindControlBitmapWithForm(bind_control_);
...
void TaskbarManager::Init(ITaskbarDelegate* taskbar_delegate) {
    ...
    if (taskbar_list_) {
        ...
        BOOL truth = FALSE;
        DwmSetWindowAttribute(taskbar_delegate_->GetHandle(),
                              DWMWA_HAS_ICONIC_BITMAP, &truth, sizeof(truth));
        DwmSetWindowAttribute(taskbar_delegate_->GetHandle(),
                              DWMWA_FORCE_ICONIC_REPRESENTATION, &truth,
                              sizeof(truth));
    }
}
...
bool TaskbarManager::RegisterTab(TaskbarTabItem& tab_item) {
    if (taskbar_list_ && NULL == tab_item.GetTaskbarManager()) {
        if (S_OK == taskbar_list_->RegisterTab(
                        tab_item.GetHWND(), taskbar_delegate_->GetHandle())) {
...
bool TaskbarManager::SetTabOrder(const TaskbarTabItem& tab_item,
                                 const TaskbarTabItem& tab_item_insert_before) {
    if (taskbar_list_) {
        return (S_OK ==
                taskbar_list_->SetTabOrder(tab_item.GetHWND(),
                                           tab_item_insert_before.GetHWND()));
    } else
        return false;
}
...
bool TaskbarManager::SetTabActive(const TaskbarTabItem& tab_item) {
    if (taskbar_list_) {
        return (S_OK ==
                taskbar_list_->SetTabActive(tab_item.GetHWND(),
                                            taskbar_delegate_->GetHandle(), 0));
    } else
        return false;
}

HBITMAP TaskbarManager::GenerateBindControlBitmapWithForm(
    ui::Control* control) {
    ASSERT(NULL != control);
    if (NULL == control)
        return NULL;
    ...
    if (::IsIconic(taskbar_delegate_->GetHandle()))  //当前是最小化状态
    {
        ...
        if (placement.flags == WPF_RESTORETOMAXIMIZED)  //最小化前是最大化状态
        {
            ...
            ::GetMonitorInfo(::MonitorFromWindow(taskbar_delegate_->GetHandle(),
            ...
            check_wnd_size =
                true;  //少数情况下，WINDOWPLACEMENT::rcNormalPosition不正确
        }
    } else
    ...
    // 2.把窗口双缓冲的位图画到内存dc
    render->BitBlt(0, 0, window_width, window_height,
                   taskbar_delegate_->GetRenderDC());
...
HBITMAP TaskbarManager::GenerateBindControlBitmap(ui::Control* control,
    ...
    return ResizeBitmap(dest_width, dest_height, render->GetDC(),
                        rcControl.left, rcControl.top, rcControl.GetWidth(),
                        rcControl.GetHeight());
}

HBITMAP TaskbarManager::ResizeBitmap(int dest_width,
                                     int dest_height,
                                     HDC src_dc,
                                     int src_x,
                                     int src_y,
    ...
    if (render->Resize(dest_width, dest_height)) {
        ...
        if (src_scale >= dest_scale) {
            ...
            scale_height =
                (int)(dest_width * (float)src_height / (float)src_width);
        } else {
            scale_height = dest_height;
            scale_width =
                (int)(dest_height * (float)src_width / (float)src_height);
        }

        render->AlphaBlend((dest_width - scale_width) / 2,
                           (dest_height - scale_height) / 2, scale_width,
                           scale_height, src_dc, src_x, src_y, src_width,
                           src_height);
    }
