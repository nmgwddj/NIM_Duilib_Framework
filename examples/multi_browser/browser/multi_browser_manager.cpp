...
BrowserBox* MultiBrowserManager::CreateBorwserBox(
    MultiBrowserForm* browser_form,
    const std::string& id,
    ...
    if (enable_merge_) {
        if (!browser_form) {
            ...
            HWND hwnd = browser_form->Create(NULL, L"MultiBrowser",
                                             UI_WNDSTYLE_FRAME, 0, false);
            if (hwnd == NULL) {
                browser_form = NULL;
                return NULL;
            }
...
void MultiBrowserManager::RemoveBorwserBox(std::string id,
                                           const BrowserBox* box /*=NULL*/) {
...
void MultiBrowserManager::SetEnableMerge(bool enable) {
    ...
    if (enable_merge_) {
        ...
        MultiBrowserForm* merge_form =
            box_map_.begin()->second->GetBrowserForm();
        ...
        for (auto it_box : box_map_) {
            ...
            if (NULL != sort_form) {
                if (first_sort) {
                    ...
                    sort_form->SetPos(rect_old_form, true, SWP_NOSIZE, NULL,
                                      true);
                }
            }
        }
...
bool MultiBrowserManager::DoDragBorwserBox(BrowserBox* browser_box,
                                           HBITMAP bitmap,
                                           POINT pt_offset) {
    if (!enable_merge_)
        return false;
    ...
    // 此函数会阻塞直到拖拽完成
    DWORD dwEffect;
    HRESULT hr = ::DoDragDrop(data_object, drop_src,
...
SdkDataObject* MultiBrowserManager::CreateDragDataObject(HBITMAP bitmap,
                                                         POINT pt_offset) {
    ...
    if (use_custom_drag_image_) {
        ...
        HBITMAP hBitmap =
            (HBITMAP)OleDuplicateData(bitmap, fmtetc.cfFormat, NULL);
        ...
        dragSrcHelper.InitializeFromBitmap(hBitmap, pt_offset, bitmap_size,
                                           data_object, RGB(255, 0, 255));
    }

    return data_object;
}

void MultiBrowserManager::OnBeforeDragBorwserBox(BrowserBox* browser_box,
                                                 HBITMAP bitmap,
                                                 POINT pt_offset) {
    ...
    drag_browser_form->OnBeforeDragBoxCallback(
        nbase::UTF8ToUTF16(draging_box_->GetId()));
...
void MultiBrowserManager::OnAfterDragBorwserBox() {
    ...
    // 如果没有被拖拽到另一个浏览器窗口里
        ...
        // 如果有多个浏览器盒子,
        // 就把浏览器盒子脱离原浏览器窗口，附加到新的浏览器窗口，拖拽成功
        else {
            drag_browser_form->OnAfterDragBoxCallback(true);
            ...
            if (drag_browser_form->DetachBox(draging_box_)) {
                ...
                if (hwnd != NULL) {
                    if (browser_form->AttachBox(draging_box_)) {
                        ...
                        ui::UiRect rect(pt_mouse.x + kDragFormXOffset,
                                        pt_mouse.y + kDragFormYOffset, 0, 0);
