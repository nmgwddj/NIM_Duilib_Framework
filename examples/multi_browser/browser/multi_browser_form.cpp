...
void MultiBrowserForm::InitWindow() {
    m_pRoot->AttachBubbledEvent(
        ui::kEventClick,
        nbase::Bind(&MultiBrowserForm::OnClicked, this, std::placeholders::_1));
    ...
    edit_url_->AttachReturn(
        nbase::Bind(&MultiBrowserForm::OnReturn, this, std::placeholders::_1));

    tab_list_ = static_cast<ListBox*>(FindControl(L"tab_list"));
    borwser_box_tab_ = static_cast<TabBox*>(FindControl(L"browser_box_tab"));

    tab_list_->AttachSelect(nbase::Bind(&MultiBrowserForm::OnTabItemSelected,
                                        this, std::placeholders::_1));

    InitDragDrop();
}

LRESULT MultiBrowserForm::HandleMessage(UINT uMsg,
                                        WPARAM wParam,
                                        LPARAM lParam) {
    if (uMsg == WM_SIZE) {
...
LRESULT MultiBrowserForm::OnClose(UINT uMsg,
                                  WPARAM wParam,
                                  LPARAM lParam,
                                  BOOL& bHandled) {
    ...
    // 如果是其他原因触发了WM_CLOSE
        ...
        if (browser_count > 0 && NULL != active_browser_box_) {
            ...
            // 如果包含多个浏览器盒子，就询问用户
                ...
                ShowMsgBox(this->GetHWND(), cb, L"STRID_CEF_BROWSER_CLOSING",
                           true, L"STRING_TIPS", true, L"STRING_OK", true,
                           L"STRING_NO", true);
            }
...
bool MultiBrowserForm::OnClicked(ui::EventArgs* arg) {
    ...
    if (name == L"btn_max_restore") {
        ...
        nbase::TimeDelta time_delta = nbase::TimeDelta::FromMicroseconds(
            nbase::Time::Now().ToInternalValue());
        std::string timeStamp =
            nbase::StringPrintf("%I64u", time_delta.ToMilliseconds());
        MultiBrowserManager::GetInstance()->CreateBorwserBox(this, timeStamp,
                                                             L"");
...
bool MultiBrowserForm::OnReturn(ui::EventArgs* arg) {
    ...
    if (name == L"edit_url") {
        ...
        std::string timeStamp =
            nbase::StringPrintf("%I64u", time_delta.ToMilliseconds());
        // 新建标签页
        MultiBrowserManager::GetInstance()->CreateBorwserBox(
            this, timeStamp, edit_url_->GetText());
    }

    return false;
}

BrowserBox* MultiBrowserForm::CreateBox(const std::string& browser_id,
                                        std::wstring url) {
    ...
    tab_item->AttachAllEvents(nbase::Bind(
        &MultiBrowserForm::OnProcessTabItemDrag, this, std::placeholders::_1));
    tab_item->AttachButtonDown(nbase::Bind(&MultiBrowserForm::OnTabItemSelected,
                                           this, std::placeholders::_1));
    tab_item->InitControl(browser_id);
    Button* btn_item_close =
        (Button*)tab_item->FindSubControl(L"tab_item_close");
    btn_item_close->AttachClick(nbase::Bind(&MultiBrowserForm::OnTabItemClose,
                                            this, std::placeholders::_1,
                                            browser_id));

    BrowserBox* browser_box = new BrowserBox(browser_id);
    borwser_box_tab_->Add(browser_box);
    GlobalManager::FillBoxWithCache(
        browser_box, L"multi_browser/browser_box.xml",
        nbase::Bind(&BrowserBox::CreateControl, browser_box,
                    std::placeholders::_1));
...
bool MultiBrowserForm::IsActiveBox(const BrowserBox* browser_box) {
    ASSERT(NULL != browser_box);
    return (browser_box == active_browser_box_ &&
            ::GetForegroundWindow() == GetHWND() && !::IsIconic(GetHWND()) &&
            IsWindowVisible(GetHWND()));
}

bool MultiBrowserForm::IsActiveBox(const std::wstring& browser_id) {
    ASSERT(!browser_id.empty());
    return (::GetForegroundWindow() == GetHWND() && !::IsIconic(GetHWND()) &&
            IsWindowVisible(GetHWND()) &&
            FindBox(browser_id) == active_browser_box_);
}
...
void MultiBrowserForm::OnBeforeDragBoxCallback(const std::wstring& browser_id) {
    ...
    // 否则隐藏被拖拽的浏览器盒子和标签
        ...
        if (index > 0)
            ...
            index++;
        BrowserTabItem* new_tab_item =
            static_cast<BrowserTabItem*>(tab_list_->GetItemAt(index));
...
bool MultiBrowserForm::OnTabItemSelected(ui::EventArgs* param) {
    if (kEventSelect == param->Type) {
        ...
        BrowserTabItem* tab_item =
            dynamic_cast<BrowserTabItem*>(param->pSender);
        if (tab_item) {
...
bool MultiBrowserForm::OnTabItemClose(ui::EventArgs* param,
...
void MultiBrowserForm::SetTabItemName(const std::wstring& browser_id,
...
void MultiBrowserForm::SetURL(const std::string& browser_id,
