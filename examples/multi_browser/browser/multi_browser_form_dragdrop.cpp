...
bool MultiBrowserForm::InitDragDrop() {
    ...
    if (FAILED(CoCreateInstance(CLSID_DragDropHelper, NULL,
                                CLSCTX_INPROC_SERVER, IID_IDropTargetHelper,
                                (void**)&drop_helper_))) {
        return false;
    }
...
HRESULT MultiBrowserForm::DragEnter(IDataObject* pDataObject,
                                    DWORD grfKeyState,
                                    POINTL pt,
                                    DWORD* pdwEffect) {
    if (NULL == drop_helper_)
    ...
    if (!MultiBrowserManager::GetInstance()->IsDragingBorwserBox()) {
        if (NULL != active_browser_box_) {
            // active_browser_box_->DragEnter(pDataObject, grfKeyState, pt,
            // pdwEffect);
            ActiveWindow();
        }
    } else {
    ...
    drop_helper_->DragEnter(this->GetHWND(), pDataObject, (LPPOINT)&pt,
                            *pdwEffect);
    return S_OK;
}

HRESULT MultiBrowserForm::DragOver(DWORD grfKeyState,
                                   POINTL pt,
                                   DWORD* pdwEffect) {
    if (NULL == drop_helper_)
        return S_OK;
...
HRESULT MultiBrowserForm::Drop(IDataObject* pDataObj,
                               DWORD grfKeyState,
                               POINTL pt,
        ...
        case kEventMouseMove: {
            ...
            if (!is_drag_state_ && (cx > 5 || cy > 5)) {
                ...
                if (nim_comp::CefManager::GetInstance()->IsEnableOffsetRender())
                    bitmap = GenerateBoxOffsetRenderBitmap(
                        borwser_box_tab_->GetPos(true));
                else
                ...
                StdClosure cb = [=] {
                    MultiBrowserManager::GetInstance()->DoDragBorwserBox(
                        active_browser_box_, bitmap, pt);
                };
...
HBITMAP MultiBrowserForm::GenerateBoxOffsetRenderBitmap(
    const UiRect& src_rect) {
    ...
    if (render->Resize(kDragImageWidth, kDragImageHeight)) {
        ...
        if (scale >= 1.0) {
            ...
            dest_height =
                (int)(kDragImageWidth * (float)src_height / (float)src_width);
        } else {
            dest_height = kDragImageHeight;
            dest_width =
                (int)(kDragImageHeight * (float)src_width / (float)src_height);
        }

        render->AlphaBlend(
            (kDragImageWidth - dest_width) / 2, 0, dest_width, dest_height,
            this->GetRenderContext()->GetDC(), src_rect.left, src_rect.top,
...
HBITMAP MultiBrowserForm::GenerateBoxWindowBitmap() {
    ...
    HBITMAP cef_bitmap =
        CreateCompatibleBitmap(cef_window_dc, src_width, src_height);
    ...
    if (render->Resize(kDragImageWidth, kDragImageHeight)) {
        ...
        render->AlphaBlend((kDragImageWidth - dest_width) / 2, 0, dest_width,
                           dest_height, cef_Dc, src_rect.left, src_rect.top,
