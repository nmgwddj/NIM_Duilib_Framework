...
public:
     ...
     * 浏览器窗口，如果为NULL则创建一个新的窗口，否则在这个窗口里新建浏览器盒子
     * @param[in] id 某个浏览器盒子的唯一标识
     * @param[in] url 初始化URL
     * @return BorwserBox*	浏览器盒子指针
     */
    BrowserBox* CreateBorwserBox(MultiBrowserForm* browser_form,
                                 const std::string& id,
...
public:
     ...
     * 设置是否使用自定义的拖拽缩略图
     * @param[in] use
     *true:使用自定义拖拽缩略图，会使用低级键盘钩子，一些安全软件会拦截
     ...
     */
    bool DoDragBorwserBox(BrowserBox* browser_box,
                          HBITMAP bitmap,
                          POINT pt_offset);
...
private:
     ...
     * @return void	无返回值
     */
    void OnBeforeDragBorwserBox(BrowserBox* browser_box,
                                HBITMAP bitmap,
                                POINT pt_offset);
...
private:
    ...
    MultiBrowserForm*
        drop_browser_form_;  // 拖入浏览器盒子的浏览器窗口，用于附加拖拽来的浏览器盒子
