...
void MainThread::Init() {
    ...
    ui::GlobalManager::Startup(app_dir + L"resources\\",
                               ui::CreateControlCallback(), true);

    // 创建一个默认带有阴影的居中窗口
    DateTimeForm* window = new DateTimeForm();
    window->Create(NULL, DateTimeForm::kClassName.c_str(),
                   WS_OVERLAPPEDWINDOW & ~WS_MAXIMIZEBOX, 0, false);
    window->CenterWindow();
    window->ShowWindow();
}
