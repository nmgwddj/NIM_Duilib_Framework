...
void DateTimeWnd::InitDateTime(DateTime* pOwner) {
    ...
    if (m_hWnd == NULL) {
        Create(m_pOwner->m_pWindow->GetHWND(), NULL, WS_CHILD, 0, false,
               m_pOwner->GetPos());
        SetWindowFont(m_hWnd, GlobalManager::GetFont(m_pOwner->GetFont()),
                      TRUE);
    }
...
LRESULT DateTimeWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (uMsg == WM_CREATE) {
        ...
        bHandled = FALSE;
    if (!bHandled)
        return this->CallWindowProc(uMsg, wParam, lParam);
    return lRes;
}
...
void DateTime::UpdateText() {
    if (m_nDTUpdateFlag == NIM_DTS_DELETE) {
        ...
        std::stringstream ss;
        ss << std::put_time(&tmSystemDate,
                            nbase::UTF16ToUTF8(m_sFormat).c_str());
...
void DateTime::SetAttribute(const std::wstring& strName,
                            const std::wstring& strValue) {
    if (strName == L"format")
        SetFormat(strValue);
    else if (strName == L"datetimeformat")
        SetDateTimeFormat(strValue);
    else
